...
\vspace{-0.4cm}
\begin{verbatim}
   snlc_sim.exe <inputFile> GENRANGE_REDSHIFT .05 -1  # (a) update only zmin
         or
   snlc_sim.exe <inputFile> GENRANGE_REDSHIFT -1  1.3 # (b) update only zmax
\end{verbatim}
\vspace{-0.2cm}
and note that setting the $-1$ value in a sim-inpult file results in abort.
