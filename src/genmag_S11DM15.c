...
int init_genmag_S11DM15(char *VERSION, int OPTMASK ) {
  ...
  S11DM15_SEDMODEL.MINLAM = S11DM15_SEDMODEL.LAM[0] ;
  S11DM15_SEDMODEL.MAXLAM = S11DM15_SEDMODEL.LAM[NLAM-1] ;
  S11DM15_SEDMODEL.MINDAY = S11DM15_SEDMODEL.DAY[0] ;
  S11DM15_SEDMODEL.MAXDAY = S11DM15_SEDMODEL.DAY[NDAY-1] ;

  // store SEDMODL struct values that are used for error checking 
  SEDMODEL.MINLAM_ALL = S11DM15_SEDMODEL.MINLAM ;
  SEDMODEL.MAXLAM_ALL = S11DM15_SEDMODEL.MAXLAM ;

  printf("\t LAM(MIN,MAX,STEP)= %.0f, %.0f, %.0f \n"
	 ,S11DM15_SEDMODEL.MINLAM
	 ,S11DM15_SEDMODEL.MAXLAM
	 ,S11DM15_SEDMODEL.LAMSTEP );

  printf("\t DAY(MIN,MAX,STEP)= %.0f, %.0f, %.1f \n"
	 ,S11DM15_SEDMODEL.MINDAY
	 ,S11DM15_SEDMODEL.MAXDAY
...
void genmag_S11DM15( int ifilt_obs  // (I) absolute filter index
  ...
  for ( epobs=0; epobs < Nobs; epobs++ ) {
    ...
    if ( Trest/s < S11DM15_SEDMODEL.MINDAY+.0001 ) { continue ; }
    if ( Trest/s > S11DM15_SEDMODEL.MAXDAY-.0001 ) { continue ; }
...
double  INTEG_SEDFLUX_S11DM15(int ifilt_obs, double z, double Tobs ) {
  ...
  for ( ilamobs=0; ilamobs < NLAMFILT; ilamobs++ ) {
    ...
    LAMDIF  = LAMSED - S11DM15_SEDMODEL.MINLAM ;
...
double INTEG_BFLUX_S11DM15(double Trest) {
  ...
  DAYDIF    = Trest - S11DM15_SEDMODEL.MINDAY ; 
