...
\end{verbatim}
% $
If this file is not specified, then default $\simeffspec=1$.
...
and applies only for fields named 'DEEP1' and 'DEEP2'.
The 'FIELD:' option allows for field-dependent maps,
and the FIELD key must appear before VARNAMES key.
...
may not be properly modeled. To investigate the first 
case (e.g., systematics) the sim-input parameter 
``{\tt MAGSHIFT\_SPECEFF: <shift>}'' 
shifts the peak magnitude used to determine $\simeffspec=0$
...
# decide how to handle extrapolations for all maps:
