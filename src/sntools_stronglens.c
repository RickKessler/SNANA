...
void init_stronglens(char *MODEL_FILE) {
  ...
  if( INPUTS_STRONGLENS.ICOL_LENSID < 0 )
    { sprintf(MISSING_VAR, "%s", INPUTS_STRONGLENS.VARNAME_LENSID); }
  else if( INPUTS_STRONGLENS.ICOL_ZLENS < 0 )
    { sprintf(MISSING_VAR, "%s", INPUTS_STRONGLENS.VARNAME_ZLENS); }

  // June 30 LOGMASS_LENS is optional 

  else if( INPUTS_STRONGLENS.ICOL_ZSRC < 0 )
    { sprintf(MISSING_VAR, "%s", INPUTS_STRONGLENS.VARNAME_ZSRC); }
  else if( INPUTS_STRONGLENS.ICOL_NIMG < 0 )
    { sprintf(MISSING_VAR, "%s", INPUTS_STRONGLENS.VARNAME_NIMG); }
  else if( INPUTS_STRONGLENS.ICOL_XIMG_SRC < 0 )
    { sprintf(MISSING_VAR, "%s", INPUTS_STRONGLENS.VARNAME_XIMG_SRC); }
  else if( INPUTS_STRONGLENS.ICOL_YIMG_SRC < 0 )
    { sprintf(MISSING_VAR, "%s", INPUTS_STRONGLENS.VARNAME_YIMG_SRC); }

  // XGAL_SRC and YGAL_SEC are optional

  else if( INPUTS_STRONGLENS.ICOL_MAGNIF < 0 )
    { sprintf(MISSING_VAR, "%s", INPUTS_STRONGLENS.VARNAME_MAGNIF); } 
  else if( INPUTS_STRONGLENS.ICOL_DELAY < 0 )
    { sprintf(MISSING_VAR, "%s", INPUTS_STRONGLENS.VARNAME_DELAY); }
