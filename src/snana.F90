...
! to see flags in C code, "grep WRITE_MASK sndata.h"
    ...
    if ( ISNLC_IMGNUM(1) >= 0     ) WRITE_FLAG = WRITE_FLAG + 8192  ! Aug 2026
...
! Jul 30 2206: MXWR_TEXT = 400 -> 1000
! -------------
    ...
    INTEGER, PARAMETER :: MXWR_TEXT = 1000  
...
! --> avoid crazy large errors that result in bad FLUXUM/FLUXERRSUM.
    ...
    REAL*8  FLAMERR_MEDIAN, RATIO, WGT_IGNORE
    INTEGER ispec, NLAMBIN, ilam, NBIN_REJECT
    INTEGER LEN_TOBS, LEN_SNR, LEN_LAMMIN, LEN_LAMMAX
    CHARACTER  & 
          STR_TOBS*20, STR_SNR*20,  FNAM*32,  & 
          STR_LAMMIN*20, STR_LAMMAX*20

    EXTERNAL ARRAYSTAT_MEDIAN
! ----------- BEGIN --------

    FNAM       = 'WRITE_SIMLIB_TAKE_SPECTRUM' // char(0)
    WGT_IGNORE = -9.0
