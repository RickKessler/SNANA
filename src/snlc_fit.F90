...
! 
! Generic SNIa lightcurve fitter.
! "Generic" means that any SNIa model can be fit to the data.
! This code applies a known model, but does NOT train SN models.
! 
! -------------------------------------------------------
! This file patches in public analysis utilities
! (HBOOK, READING DATA, etc ...)
! 
! Contents:
!   FITPAR_INI   : one-time init
!   FITPAR_INI2  : one-time init after 1st event is read
!   FITPAR_PREP  : prepare each SN prior to fit (was FITPAR_INI)
!   FITPAR_ANA   : analyze fit results after each lightcurve fit
!   FITPAR_END   : called once after all fits
! 
...
! ------------
! NEW Jan 25 2014
! Define two sets of arrays:
!  1: _ALL stores all epochs using the fixed 'ep' index
!  2: _FIT stores only the epochs used in the fit; so each index
!     can change its location on the next iteration. This sparse
!     array is needed for SNLCPAK.
! 
! Note that EPLIST_FIT is used in FCN to loop over only
! the epochs to be fit.
! 
...
! for I4EP_XXX
        ...
        ,IEP_DETNUM          = 10  &  ! Detector/CCD number
...
! 
        ,RESTLAMBDA_MODEL(2)    &  ! central filter range defined by SN model
...
! R4BNAND_  for filter-dependent quantities
! R4SN_     for SN-dependent quantities (scalars)
...
! define covariance matrices for SN model correlations (NOT for fit)
...
! define photoZ variables for photoZ fit.
...
! 
! ? DOFIT_PHOTOZ_HOST=> DOFIT_PHOTOZ_GAUSS
...
! define prior info from external SNCID_LIST_FILE
...
! option to compute Zmax/Vmax for 1/Vmax method (Jun 2013)
    CHARACTER MAGLIM_VMAX*60   ! I: e.g., 'r 21.5  i 22.0'
    INTEGER   OPT_VMAX         ! I: integer mask of options

! Note that _REJECT refer to epochs after 1st fit iteration ;
! these epochs are rejected before 2nd fit.
! FITWIN_XXX refer to cuts on final fit parameters, and are apply to SN.
! Jun 2015: FITWIN_AV -> FITWIN_COLOR
...
! ----------------------
! define all of the PHOTOZ options here in one place
! (except for INISTP_PHOTOZ ande INIVAL_PHOTOZ)
...
! define varibales to fudge the data errors for particular
! TREST-range and passbands. Note that FUDGE_MAGERR_MODEL
! globally replaces the model error, while FUDGE_FITERR_MAXFRAC
! is added to chi2-error in the specified passbands and Trest range.
...
! ------
    ...
    NAMELIST / FITINP /  & 
         ...
         ,USESIM_PEAKMJD, USESIM_DLMAG, USESIM_INIVAL  & 
...
! Created May 2014
! Misc arrays used to initialize and fill FITRES table(s).
! These arrays either don't exist elsewhere (e..g, REDUCED
! covariance), or they replace existing arrays with BIDON
...
! variables to define subset of fitted bands;
...
!           before doing more specialize fits such as rest-frame mags.
    ...
    REAL  & 
         FITVAL_ALLFILT(20)   &  ! MXFITPAR = 12, so 20 gives extra room
        ,FITERR_ALLFILT(20)
...
! 
! Aug 2 2019: pass full model name (including path) to INIT_GENMAG_SNOOPY,
!             instead of passing only the model name without path
! 
! Jun 17 2020: if OPT_PHOTOZ==2, set ERRMAX_BAD(zPHOT) = 1.0E-8
! 
! -------------------------------------
...
! set FITANA_CUTWIN_XXX for cuts that might change for photoZ fits.
! This allows using the same FITANA_CUTWIN_XXX array in FITANA
    ...
    ENDDO
! -------
! Default is rest-frame model; override below for obs-frame
    LOBS_FITMODEL  = .FALSE.
    LREST_FITMODEL = .TRUE.
    LSED_FITMODEL  = .FALSE.
...
! might be changed below, for example, for SALT2 model
...
! set default cut-windows to be wide open
...
! -------------------------------------------------------
! Covariance matrix:
! construct name of covar_File for this version;
...
! check option to fit obsMag 2 different ways
    CALL FITMAGDIF_INI()

! check option to include IDEAL fit with FLUX=TRUEFLUX (no Poisson fluctuations)
    CALL SIMFIT_IDEAL_INI()

! ------------------------------------------------------
! set FITMODEL_INDEX, FITMODEL_PATH and FITMODEL_VERSION
...
! note that mB is computed from fitpars, and does not float in fit
...
! fetch valid lambda-range from mlcs-function; note that the range
...
! Mar 2013: abort if model filter is not defined in kcor table.
...
! -----------
! set optional filter-dependent cuts on TREST
...
! print cuts to reject epochs at 2nd epoch
...
! ------------------
! check FUDGE_FITERR to see if some flux-errors should be
! blown up. Set logical(s) DOFUDGE_FITERR(ifilt_obs) to use later.
...
! -------------------------------------------
! Define extra "TABLEVAR" variables for output tables

    CALL FITINI_TABLEVAR

! check for file with interpolated MAGS at specified MJDs.

    IF ( N_INTERP_MJDLIST .GT. 0 ) THEN
       CALL DMP_SNMJD_INTERP(0,0)
    ENDIF

! check for option to compuare Zmax/Vmax
    CALL VMAX_INI()
...
! ----
    ...
    RETURN
  END SUBROUTINE FITPAR_INI

! ===========================================
    SUBROUTINE FITPAR_INI2(IERR)
! 
! Created May 2012 by R.Kessler
! One-time Initialization after 1st event is read
! but before the fit is performed.
! 
! 
...
! ------------- BEGIN ----------------
    ...
    FNAM = 'FITPAR_INI2'
! ------------------------------------------------
! Determine which parameters are floated in fit.
! Use namelist input since the global INISTP array has
! not yet been filled.
...
! ------------------- BEGIN ---------------------
 ...
 40     format(T8,'COV(mB,x1,c)-invertability fixed for ', I6,' events')
    ...
    RETURN
  END SUBROUTINE FITPAR_END

! ==========================================
    SUBROUTINE FITPAR_PREP ( iter, IERR )
! 
! For each "isn" supernova, this user-routine is used to
! initialize global variables used as input to the fitter:
! 
! NFITPAR             = number of fit parameters
! PARNAME_STORE(1:NFITPAR)  = name of each parameters (<= 10 bytes)
! INIVAL(1:NFITPAR)   = initial value
! INISTP(1:NFITPAR)   = initial step size (0 => fixed parameter!)
! 
! 
! Useful notes:
! 
! 
! It is highly recommened that the first fit parameter
! is INIVAL(1) = DBLE(ISN)  with INISTP(1) = 0.0
! You will need this index in FCNSNLC to get information
! about a particular SN.
! Another useful init is to set your initial MJD_AT_PEAK equal  to
! SNLC_SEARCH_PEAKMJD(isn) from Masao's LC fitter during the survey.
! 
! 
! Fill MAG_XTMW_REF(ifilt_obs)  so that  USRFUN
! can make the same approx MilkyWay correction that was
! applied to data.
! 
! 
! Mar 19 2024: set MASK_PHOTOZ_SOURCE
! 
! ----------------------------------------
...
! init Trest-related variables that are set after fit
    IF ( FIRST_ITER ) THEN
      TRESTMIN_FIT        = -999.
      TRESTMAX_FIT        = -999.
      TRESTRANGE_FIT      = -999.
...
! load redshift to use in spec-z fit
...
26       format(T5,'Z-WARNING for CID=',A,  & 
    ...
    ENDIF
! ------------------------------
! set global REDSHIFT_FIT to use for things like
! getting rest-frame filters and Trest values.
...
! ---------------------------------
! set redshift-arg for LFITFUN_FILT (below)
! For photoz fit set Z8TEST=-9 so that z-dependent tests are
! skipped ; they are applied in PHOTOZ_ANA (after 1st fit) instead.

    Z8TEST = Z8
    IF ( DOFIT_PHOTOZ ) THEN
      Z8TEST = -9.0

! reset DROPFILTER array in first iteration only
      if ( FIRST_ITER ) then

! Mar 19 2024: set mask of which OPT_PHOTOZ option (bit) is actually used.
        ...
        if ( LPZ_Q ) then
!           use z_spec prior or quantiles; ignore zPHOT point estimate
          if ( LZSPEC ) then
...
! skip initial DROP-FILTER test if cheat-flag is set
          IF ( DOFIT_PHOTOZ_CHEAT ) GOTO 220

! check option to exclude certain lambda regions on first
! iteration (default is to ignore UV filters).
! Note that any filters dropped here can get added back before
! 2nd iteration depending on the photoZ-fit value.
...
! ---------------------------------
! count how many filters to fit;
! a fit filter must:
!   -  be specified by the user (DOFIT_FILT)
!   -  exist in the lightcurve.
!   -  transform to rest-frame lambda within RESTLAMBDA_FITRANGE(2)
! 
...
! store USE_FILT array from previous iteration in order
! flag any changes in the 2nd iteration.
...
! Jan 2025: for photo-z fit, read BANDLIST to force fitting same bands again
...
! set USE_FILT based on user-request, existence,
! and valid rest-frame filter.
...
! print message if observer-filter is dropped or added on 2nd iteration
...
! set range of observer filters to evaluate for each obs filter.
! This is needed for the color transformation and for U -> U-BX+B
...
! init EP_XXX arrays ( before calling FITINI_ADJUST)
    CALL FITINI_EPVAR(iter)

! set INIVAL and INISTP arrays.
    CALL FITINI_PARVAL(iter)

! Check 1st iter override from SNCID_LIST_FILE
    IF ( FIRST_ITER ) CALL FITINI_LIST_FILE(iter)

! init linear FITCOV arrays
...
! check for photoZ fit (after FITINI_COV)
    IF ( DOFIT_PHOTOZ ) CALL FITINI_PHOTOZ(iter, IERR)
    IF ( IERR .NE. 0 ) RETURN

! adjust fitpar based on initial Fdata/Fmodel ratios.
...
! check option to fix all parameters in fit
...
! init filter-dependent variables.
    ...
    RETURN
  END SUBROUTINE FITINI_EPVAR


! ===================================================
    SUBROUTINE FITPAR_ANA ( isn, iter, ERRFLAG )
! 
! User-routine to analyze fit results after each SN.
! Includes incrementing histograms, statistics, etc ...
! 
! Fit results are in
! 
!   FITPAR(1:NFITPAR)
!   FITERR(1:NFITERR)
! 
! Recommend making (REAL*8) equivalence to local
! variables with more appropriate names.
! 
! Returns ERRFLAG as follows:
!  =  0 => OK; keep SN
!  = >0 => discard fit (call CDTOPDIR)
...
!   DELCHI2_REJECT are set. Fixes long-standing bug.
! 
! Aug 28 2020: if ERRFLAG=0 then set CUTFLAG_SNANA += 2 so that
!              correct CUTFLAG_SNANA shows up in FITRES table.
! 
! May 18 2021: skip crazyflux check of OPT_SNCID_LIST > 0
! Jul 23 2024: apply user defined 1st-iteration cuts on FITPROB, shape, color;
!              see FITWIN_[FITPROB,SHAPE,COLOR]_ITER1
! 
...
! 
! transfer I4/R4EP_ALL arrays to sparse I4/R4EP_FIT arrays (Jan 25 2014)
    CALL LOAD_EPFIT()

! for photoz fit, check for repeat iteration or discard fit
    IF ( DOFIT_PHOTOZ ) THEN
...
! --------------------------------------------
! Jan 2016: fit iteration again if cov matrix has not converged
...
! --------------------------------------------
! May 2018 check option to repeat entire fit with MINOS
...
! set default of NFIT=1 to plot (before calling FITRESTMAG or FITMAGDIF)
! This call is not needed, but avoids confusion with NFIT=0 inside the
! sntools_output routines.
    IF ( ITER .EQ. 1 ) THEN
      CALL SNLCPAK_NFIT(1)   ! 5/01/2014
    ENDIF

! check optional rest-mag  (Feb 2012)
    CALL FITRESTMAG_CALC(ITER)

! check optional magdif test (Mar 2013)
    CALL FITMAGDIF_CALC(ITER)

! check optional IDEAL fit with FLUX=TRUEFLUX (Aug 2017)
...
! ----- BELOW IS LAST ITERATION -------

! check for special variable computation for SIMSED models;
    CALL SIMSED_RECON()

! ------------------------------------------------------
! compute SIMCHI2 on last iteration
!       IF ( LSIM_SNANA .and. LAST ) then
!  4.22.2019: disable because of problem with wrong host redshift
!          CALL GET_SIMCHI2_CHEAT()
!       ENDIF
...
! ---------------------------------------
! Use fitted flux to update ERROR CALC from ZPT,PSF and SKY

    if ( LTRACE ) CALL DMPTRACE("CALLING UPD_FLUXERRCALC")
    CALL UPD_FLUXERRCALC()

! -------------------------------
! Nov 2018: Check option to fix non-invertible COV(mB,x1,c)
    CALL FIX_COVAR_LCFIT()

! ----------------------
! check option to compute Zmax for 1/Vmax method
! (after FITANA_STORE ad before SNTABLE calls)
    do i = 1, NFILT_VMAX
       CALL VMAX_ANA(i)
    enddo

! ----------------------------------------
! on last iteration, check if fit is OK
...
! - - - - - - -
! Aug 28 2020: if we get here, set CUTFLAG_SNANA bit for fit success.
    CUTFLAG_SNANA  = IBSET(CUTFLAG_SNANA,1) ! for SNANA tree

! ---------------------------------------------------------
! book plots for this SN; all plots are vs. relative epoch
...
! check for single-epoch dump of FLUX & MAG at specified MJD
! See input namelist "SNMJD_LIST_FILE"
...
! xxxxxxxxxxxxxxxxxxx
!      print*, ' xxx '
!      print*, ' xxx -------- CRAZYFITERR iter=', iter
!      print*, ' xxx ERR(c,s,d,t0) = ',
!     &     ERR_COLOR, ERR_SHAPE, ERR_DIST, ERR_PKMJD
! xxxxxxxxxxxxxxxxxxx

! if we are fitting only 1 filter, then don't bother
! checking crazy error

    if ( .NOT. CHECK_CRAZYFITERR ) then
       RETURN
    endif

! check for NaN on every fit iteration
    LNAN = .FALSE.
...
! don't do any more until last iteration
...
!  max is a guess
    ...
    RETURN
  END FUNCTION CRAZYFITERR

! =====================================
    LOGICAL FUNCTION FIRST_ITERATION(ITER)

! Created Aug 2017
! Return TRUE on first iteration of fit.
...
! Created Jun 28, 2012
! If SIMSED model has parameters S2c and S2x1,
! compute SIMMB and SIMx0.
...
! for SIMSED model with S2x1 and S2c,
! compute SIM_SALT2mb and SIM_SALT2x0
...
100   CONTINUE
    ...
    RETURN
  END SUBROUTINE SIMSED_RECON

! ======================================
    SUBROUTINE FIX_COVAR_LCFIT()

! Nov 2018
! Check option to fix COV(mB,x1,c) if not invertible.
! Use algorithm for Marriner 2011 (SALT2mu code),
! which is update_covMatrix() function in sntools.c
! 
! Uses &FITINP OPT_COVAR_LCFIT mask, with mask options
! defined below as parameters.
! 
! There are two options here to fix COV to ensure invertibility:
! 1. M11 (Marriner 2011):
!    Diagonal COV is ERROR^2, and off-diag is from Minuit's MNEMAT
!    Beware that MNEMAT diagonals correpond to parabolic error,
!    not MINOS error.
! 
! 2. RED: use reduced covariance from MNEMAT and MINOS errors to
!    to construct COV matrix.
! 
! 
...
! load list of fit-param IPAR used to construct local COV
    NPAR = 3
    IPAR_LIST(1) = IPAR_X0
    IPAR_LIST(2) = IPAR_SHAPE
    IPAR_LIST(3) = IPAR_COLOR

! define jacobian factor for x0err
    X0    = LCVAL_STORE(IPAR_X0)
    SF    = -2.5/(x0*LOGTEN)

! set diagonal COV using stored errors ; don't use diagonal FITERRMAT
! since it's not necessarily ERR^2.
...
! ----------------------------------
! prepare arguments for update_covmatrix
...
!       print*,' xxx LDMP(1,2) = ', LDMP1, LDMP2, OPTMASK
...
! --- check dump options ----
 ...
 20      format(T8,'Fix COV(mB,x1,c) for CID ', A)
    ...
    RETURN
  END SUBROUTINE FIX_COVAR_LCFIT

! ======================================
    SUBROUTINE UPD_FLUXERRCALC()

! Feb 24, 2012
! Loop over fitted epochs and call FLUXERRCALC
! with fitted flux rather than the FLUXCAL from
! the data file.
! 
! Nov 27, 2012: remove ISN argument from subroutine and from
!                FLUXERRCALC call.
! 
! -------------------------
...
200   CONTINUE
    RETURN
  END SUBROUTINE UPD_FLUXERRCALC

! ==================================
    SUBROUTINE SETCUTS_FITWIN_TREST

! Jan 2012
! By default set each FITWIN_TREST_FILT = global FITWIN_TREST
! Then check user-namelist strings FITWIN_TREST_FILTER
! for tighter TREST-windows.
! 
! Initial use is to limit the SNACC-filter Trest-range
! because the FUDGE_FITERR options allow downweighting
! but not removing epochs. The FITWIN_TREST_FILTER option
! allow removing epochs in specific filters.
! 
! May 26, 2012:
!  fix aweful bug. First do-loop to init FITWIN_TREST_FILT
!  is now up to MXFILT_ALL=80 instead of MXFILT_OBS=62.
!  Previously FITWIN_TREST_FILT with IFILT_OBS > 62 were
!  not initialized, and thus these filters were likely
!  dropped on 2nd fit iteration.
! 
! ------------
...
! first set each filter-Trest window to the global default.
! c   DO ifilt_obs = 1, MXFILT_OBS !!! aweful bug fixed May 2012
    DO ifilt_obs = 1, MXFILT_ALL
      FITWIN_TREST_FILT(1,ifilt_obs) = FITWIN_TREST(1)
      FITWIN_TREST_FILT(2,ifilt_obs) = FITWIN_TREST(2)
    ENDDO

! count how many FITWIN_TREST_FILTER cuts have been  specified.
...
! make sure that number of elements is a multiple of
! three (filters, TMIN, TMAX)
...
100   CONTINUE
    ...
    RETURN
  END SUBROUTINE SETCUTS_FITWIN_TREST

! ===================================================
    SUBROUTINE SEDMODEL_INI(IFLAG,IERR)
! 
! Created Nov 09, 2010 by R.Kessler
! 
! Called from FITPAR_INI to initialize model(s) based
! on a SED spectral surface. This init includes
! primary reference, filters, and the model-dependent
! INIT_GENMAG_XXX() function.
! 
! Models supported:
!  - SALT2
!  - SIMSED
! 
! 
!  IFLAG = 0 => called before any SN have been fitted (init stage)
!  IFLAG > 0 => called just before fitting each SN
! 
!    HISTORY
! 
! Jun 24 2018: pass NONE argument to INIT_GENMAG_SALT2
! 
! Jul 09 2019: call set_UVLAM_EXTRAPFLUX_SEDMODEL
! 
! Apr 20 2020: fix dimenstion of TMP_FITMODEL to allow paths
! 
! Jul 30 2024:
!  remove "ZMAX = PHOTOZ_RANGE_KCOR(2)" because PHOTOZ_RANGE_KCOR isn't set yet.
!  This results in zmax=0 and undefined log10(zmax)  ... not clear how this worked at all.
! 
! ---------------------------------------------------------
...
! reset SEDMODEL to init and/or erase previous instance
    ISTAT = RESET_SEDMODEL()

! init primary spectrum
...
! set entire redshift range if same filters are used for each SN;
! for SN-dependent filters, set exact z-range (and 1 Z-bin)
! to avoid lots of uncecessary CPU time.
...
! ----------------------------------------
! now do general model-dependent init.
! TMP_FITMODEL has the special end-string char(0) need for C functions.
...
!          ISTAT = INIT_GENMAG_SIMSED( TMP_FITMODEL, LEN_NAME )
...
! Created Feb 2, 2014
! 
! Call FCNSNLC with  IFLAG = FCNFLAG_USESIM to replace the
! fitted parameters with the true simulated parameters.
! Store resulting chi2 in global SIMCHI2_CHEAT.
! This works fine for 4-parameter fits, but there is a
! subtle issue for 5-par (photo-z) fits; if the true
! redshift (ZSIM) is very far away from the fitted ZPHOT
! then the wrong filters are used and the program will abort.
! For example, ZPHOT=.4 will use z-band, but if ZSIM=0.1
! then z-band goes outside the 9200 range of SALT2 and
! will abort.
! 
! If any obs-frame filter is undefined in the rest-frame
! [ based on range of LAMOBS/(1+ZSIM) ] then skip chi2
! computation and set SIMCHI2_CHEAT = -1 * NFILT_UNDEFINED
! 
...
! first check that each filter used in the fit
! is defined by the model in the rest frame.
...
! bail if there are any undefined filters in the rest-frame.
    if ( NFILT_UNDEFINED > 0 ) THEN
       SIMCHI2_CHEAT = -1.0 * DBLE(NFILT_UNDEFINED)
       return
    endif

! if we get here, filters are OK so compute SIMCHI2_CHEAT.
...
! Analyze PHOTOZ fit.
! 
! On ITER=0, init & print ZDROP for each filter.
!    Also remove SNANA-Trest cuts and store cuts in PHOTOZ-cut variables
! 
! On 1st iteration, check for filters to drop.
! On 2nd iteration, do nothing but fill global variables.
! 
! If filter(s) are dropped, set ERRFLAG = -1
! so that the 1st iteration is repeated (LREPEAT_ITER=T)
! 
! Fill global variables
!  PHOTODZ_MIN(iter)   MIN distance to model-boundary over all filters,
!  PHOTODZ1Z_MIN(iter)
!  IFILT_NEARDROP     ! filter-index closest to being dropped
! 
! 
! April 2024: check FITWIN_PROB_ITER
...
!    Nov 2024: preserve original PHOTODZ_REJECT in case it gets altered
!              for subset of zSpec
...
! bail if we just did 2-band FITRESTMAG since it makes no sense to repeat.
...
! check LREPEAT after PHOTOZ_VAL/ERR are defined
...
! note that negative DZ always means outside the ZMIN/ZMAX range
! defined by the model.
...
! apply cut to see if this filter should be tossed;
...
! set cut logical only on first iteration
...
! 4/24/2011: check if a filter will be added
...
!          print*,'  xxxx ITER=', ITER,
!     &          '   USE_FILT(', cfilt1(1:1), ') = ', USE_FILT(ifilt_obs),
!     &          '   LZTEST=', LZTEST,
!     &          '   DROP=', PHOTOZ_DROPFILTER(ifilt_obs)
...
! On last iteration, set NEARDROP-filter index to negative
! if it was dropped. This is to identify dropped filters
! in the analysis-ntuple (ntid 7788)
...
! if filters are dropped, then check if Trest-related
! SNANA cuts are still satisfied. If not, set ERRFLAG
! to SKIP this turkey. Use the loose snana cuts instead of the
! tighter (final) cuts.
...
! if we don't have enough measurements after filter-drop, bail ASAP.
      if ( NTLIST .LT. CUTWIN_NEPOCH(1) ) then
        ERRFLAG = ERRFLAG_ZFIT_DROPFILTCUTS
        if ( STDOUT_UPDATE ) then
          write(6,186) NTLIST
186         format(I2,' obs -> FAIL' )
        endif
        return
      endif

! get Nfilt passing Trestmin & Trestmax cuts.
...
! require at least enough filters to have a chance at NFILT_SNRMAX cut
...
! ----------------------
!        print*,' xxxx TRESTMIN, TRESTMAX = ',
!     &          TRESTMIN_FIT, TRESTMAX_FIT
!        print*,' xxxx NFILT(TRESTMIN,TRESTMAX) = ',
!     &          NFILT_TRESTMIN_FIT, NFILT_TRESTMAX_FIT
! ----------------------
...
300   CONTINUE
    ...
    RETURN
  END FUNCTION LTEST_ZPHFILTER

! ============
    REAL FUNCTION TRESTzCUT(OPT, z, Trest)

! Created Aug 19, 2009
! Returns either (1+z)*Trest or  1/(1+z) times Trest
! 
! OPT = 1 : return TRESTzCUT that is smaller than Trest
! OPT = 2 : return TRESTzCUT that is larger than Trest
! 
...
! ----------- BEGIN ----------
    ...
    RETURN
  END FUNCTION TRESTzCUT

! ============================================================
    SUBROUTINE PHOTOZ_SAVECUT(name,  & 
              cutwin_orig, cutwin_snana, cutwin_save )

! Created Aug 15, 2009 by R.Kessler
! 
! Save SNANA cut-window in CUTWIN_SAVE; then open
! cutwin_orig to be wide open so that cut is not
! applied in snana.
...
30    format(T8,'Nominal ',A10,' cut after  photoZ fit: ',  & 
    ...
    RETURN
  END SUBROUTINE PHOTOZ_SAVECUT

! ====================================================
    SUBROUTINE SET_ZHD_PHOTOZ()

! Created Jan 26 2018
! Set redshift globals for output tables:
!    SNLC_zHELIO[_ERR]
!    SNLC_zCMB[_ERR]
!    SNLC_zHD[_ERR]
! 
! For zHD, translate heliocentric fitted photo into cmb frame,
! and include ZPECERR in zHDERR
! 
...
! -------------- BEGIN -------------
    ...
    RETURN
  END SUBROUTINE SET_ZHD_PHOTOZ

! ====================================================
    SUBROUTINE FITANA_MARG(isn,DOPDFPLOT,ERRFLAG)
! 
! Shell to call MARG_DRIVER
...
! store filter-dependent chi2 before continuing
! These are just the fit chi2 from minimization.
...
! skip marg if chi2(sigma) fails cut since it won't change after marg.
...
! store marginalized chi2 per filter
    ...
    RETURN
  END SUBROUTINE FITANA_MARG


! ============================================
    SUBROUTINE FCNSNLC(NVAR,GRAD, CHI2TOT, XVAL,IFLAG, USRFUN )
! 
! User function to be minimized (chi2, log-likelihood, etc ...).
! "USRFUN" is an optional function, for example from MC.
! YOU are responsible to sync XVAL(1:NFITPAR) in this function with
! INIVAL(1:NFITPAR) in FITPAR_PREP and with FITPAR(1_NFITPAR)
! in FITPAR_ANA.
! 
! The prior-chi2 "CHI2INI" is used for minimization.
! However, the stored chi2 "FCN_FITCHI2(0)" does NOT include
! CHI2INI so that the probability is based on the data and
! model, and not on the prior.
! 
! 
! 
! The data to compare the function to are in KEEP,SNCUTS
! in snana.car. A few useful variables are
! 
!   SNLC_CID         ! CID vs. sparse SN index
!   SNLC_NEPOCH_STORE      ! Number of Epochs
! 
!   SNLC_RA          ! RA   vs. SN
!   SNLC_DECL        ! OBDECL vs. SN
!   SNLC_REDSHIFT    ! redshift used for analysis
! 
!   SNLC_XTINC_MW(MXFILT_ALL)  ! interstellar extinction
! 
!   SNLC_FLUXCAL(MXFILT_OBS,MXEPOCH)
!   SNLC_FLUXCAL_ERRSTAT(MXFILT_OBS,MXEPOCH)
!   SNLC_FLUXCAL_ERRSYST(MXFILT_OBS,MXEPOCH)
!   SNLC_FLUXCAL_ERRTOT(MXFILT_OBS,MXEPOCH)
! 
!   chi2 = D_i  COV_ij D_j
! 
! 
...
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
! 
! Mar 1 2017: load R4EP(iep,JEP_MAGDIF)
! 
! Jun 23 2019: protect I4EP_ALL array bound
! Jul 13 2019:
!   + compute RATIO_ERR only if LFLAG_USER=T (to reduce compute time)
!   + apply SNR_RAW cut to compute R4SN_FFSUM_XXX
! 
! Feb 28 2020:
...
! check for photoZ fit that constrains DLMAG to input cosmology.
...
! bail on large CHI2INI unless we are dumping.
...
!    protect model against invalid rest-frame lambda values
...
! compute data SNR with/without fudged error
      ...
      Nfitdata_filt(0)     = Nfitdata_filt(0) + 1
      LASTFILT  = Nfitdata_filt(ifilt) .EQ. NEPOCH_FIT(ifilt)

! set local dump logicals = namelist flag .and. (debug option)
...
! print non-zero chi2prior contributions
...
! =====================
! get flux (observer frame) from model
...
! ---------------------------------------------
! get data-model chi2 ;
! if USE_FITCOV=T, then sum diagonal elements of cov matrix.
...
! always tack on chi2 from log(sigma) term
        chi2tot  = chi2tot + delchi2_sigma

      ENDIF

! if fast-flag is set, quit when chi2 is too big;
! this corresponds to tiny pdf in integral.

      if ( LFLAG_FAST .and.  chi2tot .GT. FITCHI2_QUIT ) return

! check dump-flag again; print every epoch here
...
! check for epochs to reject at end of 1st iteration only (note LAST=T here)
...
! ----------------------
! compute chi2tot from covariance matrix
...
!  check option to re-compute covariance inside minuit function
      if ( OPT_COVAR_FLUX .EQ. 2 ) then
         DO ipar = 1, NFITPAR_MN
           XVAL4COV(ipar) = XVAL(ipar)  ! used for COV opt
         ENDDO

!    modify x1 for SALT2 model.
         if  ( FITMODEL_INDEX .EQ. MODEL_SALT2 ) THEN
           x1 = XVAL4COV(IPAR_SHAPE)
           XVAL4COV(IPAR_SHAPE)  = SALT2xx1(ITER,x1)
         endif

!  re-compute COVARIANCE and its inverse => COVMAT2
!  1st arg OPT=0 suppresses printing.
         CALL FITINI_COV(0,iter,XVAL4COV,IERR)
      endif

! loop over COV_INV elements and compute chi2
...
!   check option to include determinant term log[det(COV_INC)]
      IF (DO_LOGDET) THEN
...
! print more info for large delchi2
...
166       format(T5,' xxx MJD=',F7.1, 2x, 'FLUX[DATA,MODEL] = ',  & 
    ...
    RETURN
  END SUBROUTINE  PRINT_FCNCHI2_MATRIX

! ==============================================================
    DOUBLE PRECISION FUNCTION FCNCHI2_PRIOR(XVAL,CHI2PRIOR)
! 
! Created Apr 03, 2009 by R.Kessler
! Evaluate prior-chi2 and return total prior-chi2.
! 
! Apr 3 2018: include color-prior chi2
! 
...
! init CHI2PRIOR and check for crazy fit params
...
! strip off XVAL parameters into local variables.
...
! priors for all models to protect against crazy MINUIT excursions

    DIF     = PEAKMJD  - INIVAL(IPAR_PEAKMJD)
    CHI2PRIOR(IPAR_PEAKMJD)  & 
              = CHI2_PRIOR(ipar_PEAKMJD,DIF)

! protect against crazy COLOR values
...
! protect against against absurdly small & large distances
! (because MINUIT sometimes tries really crazy values)
...
! Jan 2025: add prior on each param based on fitpar read from external file
...
40    format(' z=',F5.3, 3x,'sigz=',F5.3, 3x,'PROBZ= ',F5.3,3x,  & 
    ...
    CALL flush(6)
    RETURN
  END SUBROUTINE DUMP_ZPHOT_PRIOR
...
! [moved code out of FCNSNLC to clean up and fix bug]
! 
! Return chi2 contribution from error-term, relatiave to last interation.
! The nominal term is -2ln(FLUXERR) but instead here we compute
! 
!      FCNCHI2_SIGMA = -2ln(FLUXERR/LAST_FLUXERR)
! 
! so that this is nominally ~zero if the flux-error does not
! change much between iterations.
! 
! LAST_FLUXERR is either defined as total error (default) or only using the data error (user option)
! To control this option, see user input OPT_CHI2_SIGMA and MASK_CHI2_SIGMA_* parameters
! 
! Also fix bug by returning zero if 1st-iteration FLUXERR=0 ...
! this can happen if a new filter is added in the 2nd iteration.
...
!      ifilt_obs = I4EP_ALL(iepoch,IEP_IFILT_OBS)  ! for debug only
!      cfilt     = filtdef_string(ifilt_obs:ifilt_obs) ! for debug only
...
! 
! 
! Jan 16, 2012: fix long-standing patchy bug NOAVWARP;
!               test-code had always been included,
!               thus blowing up the errors.
! 
! Jul 2013: change name of MU argument to DIST
! 
! Sep 22 2013: add args RV and OPT_MWCOLORLAW to get_mwxt8().
! 
...
! check for special case where IFILT_OBS is really a rest-filter
...
! get approx MW extinction (for printout only ... not for calculation)
    AVMW     = RVMW * mwebv_model ;
    PARDUM(1) = 0.0
    MAG_XTMW = GALextinct(RVMW, AVMW, LAMAVG, 94, PARDUM,  & 
                     FNAM//char(0), 8);

! if we have observer-frame model, then skip all
! the rest-frame stuff.

    IF ( LOBS_FITMODEL ) GOTO 333

! get nearest and 2nd nearest filter in rest frame.
! Note that IFILT_REST1[2] are absolute (NOT sparse) filter indices.
...
! check if KCOR13 (based on warping other neighbor) should be
! wgt-averaged with nominal KCOR12.
! First that order is 3-1-2  or  2-1-3.
...
! update rest-frame map for photoZ fit (if IFILT is defined)
...
! check option for rest-frame mags only with no K-correction
...
! expand filter ranges if Landolt option is set.
...
! -------------------------------------------
! get MAG_REST1 = magnitude in rest frame for closest filter
!     MAG_REST1 = mag in rest-frame for 2nd closest filter (for color)
! Use "genmag_xxx" function that are used in simulation.
...
! note that genmag_mlcs starts at 0, so subtract 1 from index.
...
! loop over all needed obs-filters;
! multiple filters are needed for Landolot color transf.
...
! mag error is from the nominal filter ... ignore mag_err
! from other filters used for Landolt color trans.
...
! -----------------------------------
! check option for rest-frame mag only
! Include host extinction to get rest-frame colors.
...
! check for option to smooth rest-frame model
! across filter boundaries
...
! set temp arrays MAG_BESSELL and MAG_LANDOLT
...
! -----------------------------------------------------------
! If Landolt option is set, then assume MAG_REST is in Landolt system
! and convert to Bessell system before applying Kcor to observer frame.

    IF ( USE_LANDOLT_REST ) THEN  ! rest-frame conversions

! If U-band is needed (NEEDU), first get synthetic BX-B needed to
! convert Landolt-U back to synth Bessell.
! Store BX-B in the memory slot for Landolt-BX.
...
! now convert to synthetic Bessell.
...
! convert synthetic U into reported U by subtracting BX-B :
...
! ----------------------------------------------------
! Now MAG_REST is in synthetic Bessell system;
! apply host extinction for rest-frame models.
...
! --------------------------------------------
! determine K-correction; note that rest-frame mags
! include host-galaxy extinction.
...
! check filter bit-mask to see if this IFILT needs to be evaluated.

      LTMP = FILTBTEST(MSKTMP8,ifilt2_obs)

      IF ( .not. LTMP ) goto 155

! get closest rest-frame filters for this observer "ifilt2_obs"
      OPT = OPT_FILTOBS
      DO inear = 1, 3
...
! Add up all the pieces for observer mag prediction.
...
! check for option to subtract MWgalaxy extinction from model
...
! ---------------------------------------------
! load output args for function
! note that MAG_OBS is loaded below the 501-continue
...
! determine model error as contribution from model and K-corr.
...
! -------------------------------------------------
! if observer-frame is Bessell, and Landolt option is set,
! then transform observer-frame Bessell mags (MAG_BESSELL)
! to Landolt.
...
! transfer MAG_OBS_TMP to temporary MAG_BESSELL_OBS array
! => input argument for LANDOLOT_CONVERT below.
...
! abort if Landolt correction makes more than 3 mag change.
! Check only if SHAPE and COLOR have sane values,
! and also require reasonable Trest.
! 
! 
...
!  strip off final observer-frame mag

    MAG_OBS      = MAG_OBS_TMP(ifilt_obs)

! ------------------------------------------
! Convert to calibrated flux if MAG_OBS has reasonable value;
! otherwise set flux to zero.
...
! allow any MAG for TEST option
    if ( FITMODEL_INDEX .EQ. MODEL_FIXMAG ) LTMP = .TRUE.

    if ( LTMP ) then
       arg   = -0.4 * (MAG_OBS - ZP_FLUXCAL)
       FLUX  =  TEN8**(ARG)
    else
       FLUX = 0.0
    endif
...
! write rest-frame info

! unextincted mag in rest frame
...
! write rest color, AVwarp and Kcor
...
! now write single line with CID keyword ;
! grep "CID" > bla creates easy-to-read column-formatted file
! Columns are
!   1. CID
!   2. obsFilt
!   3. restColor
!   4. MJD
!   5. Trest
!   6. Unextincted mag in rest-frame
!   7. rest-frame mag
!   8. rest-frame color used for SED warping
!   9. AV warp
!  10. Kcor
!  11. observed mag
...
! ===========================
! Now check dump for observer-frame model
...
! 
! Created Aug 13, 2009 by R.Kessler
! 
! Return "DISTANCE" estimate based on model;
...
! Use namelist input FITMODEL_NAME to initialize global
! variables
!   FITMODEL_PATH, FITMODEL_VERSION, FITMODEL_INDEX
! 
! Feb 22 2017:
!   + allow FITMODEL_NAME to include path; see EXTRACT_MODELNAME().
! 
...
! first  hard-wird generic name for each model
...
! Special case for backward compatibility: for mlcs, change to mlcs2k2 ...
! remove this dumb logic if/when a new MLCS model is provided by the ESSENCE group.
...
! Feb 22 2017:
!  check if FITMODEL_NAME includes path.
!  Note that C function return termination string
...
! May 31 2019 hack: if SALT3, switch to SALT2 to avoid lots of code refactor.
    if ( FITMODEL_INDEX == MODEL_SALT3 ) FITMODEL_INDEX=MODEL_SALT2

! abort if FITMODEL_INDEX has not been found.
...
! ----------------------------
! finally, if FITMODEL_PATH is not already set above
! with EXTRACT_MODELNAME, then set it here to default path.
    ...
    RETURN
  END SUBROUTINE FITMODEL_INI

! ========================
    SUBROUTINE GET_FITFLUX(ep, FIT_FLUXCAL, FIT_FLUXCAL_ERR, REJECT)

! Created Mar 17 2021
! Interface for snana to fetch fitted FLUXCAL and error;
...
! this before or during fit !!!
! 
! OPT = 0  : return nominal function: observer-frame flux
! OPT = 1  : return rest-frame flux
!                (i.e.; no K-cor, no dL, include AV)
! 
! OPT = 2  : return rest-frame flux based on cosmology and XMW
!              (for training)
! OPT = 3  : same as OPT=2, but subtract host-extinction too
! 
! OPT = 6  : return rest-frame mag-error instead of flux
! OPT = 10 : return K-correction only
! OPT = 60 : return error on observer-frame flux
! OPT = 99 : return simulated observer flux with no smearing (LSIM=T only)
! 
! 
!     HISTORY
!  ~~~~~~~~~~~~
...
! 
! Sep 29, 2007
! Returns RV for fitter-function USRFUN
! 
! Aug 1, 2009: RV8=1000 (instead of RV_MWDUST) if AV/RV=0
! 
! ----------------------------------------
...
! Created Aug 26,2009 by R.Kessler
! 
! Return rest-filter weights to allow a smooth transition
! across filters. At a transition wavelength (i.e., 3900 A
! between U & B), WGT1 = WGT2 = 0.5. If more than
! "LAMREST_MODEL_SMOOTH" Anstroms away from transition
! wavelength, return WGT=1 and WGT=0
! 
! These weights can then be used to smooth out the
...
! get distance (Ansgrroms) from boundary
    LAMDIST = abs(LAMREST - LAMBOUNDARY)

! bail if too far from boundary
    IF ( LAMDIST .GT. LAMREST_MODEL_SMOOTH ) RETURN

! use SMMOOTH_STEPFUN function normalized to be
! zero  at LAMBOUNDARY - LAMREST_MODEL_SMOTH, and
! one   at LAMBOUNDARY + LAMREST_MODEL_SMOTH
...
! 
! Return wgt average of data/model for point on lightcurve.
! This is used for adjusting initial "fluxnorms" for fit.
! Uses EP_XXX arrays, and assumes that they have been
! filled with a call to FCNSNLC with IFLAG=FCNFLAG_USER
! 
! Aug 15, 2012: remove obsolet ISN arg.
! Jan 25, 2014: use new I4/R4EP_ALL arrays
! ---------------------
...
! exclude early or late times to be safe
 ...
 100  CONTINUE
    ...
    RETURN
  END FUNCTION DATA_MODEL_RATIO

! ===========================================
    SUBROUTINE RDFITNML(IERR)
! 
! 
! Read &FITINP namelist.
! 
! HISTORY
! ---------------------
! 
...
! set defgaults to no rejected epochs at 2nd iteration
...
! hard-wire a few very loose cuts on SALT2-fitted errors
! to reject crazy fits
...
! setup default grid w.r.t. peakmjd-estimate
...
! Set option for covariance matrix.
! -9 => use default for each model (Aug 28, 2012)
!  0 => use diagonal elements only
!  1 => use covariance matrix evaluated between iterations
!  2 => use covaraince matrix evaluated each fit trial (slower)
    OPT_COVAR      = -99 ! legacy variable
    OPT_COVAR_FLUX = -9  ! new variable name, Nov 9 2018

! option for covariance errors on Galactic-extinction
    OPT_XTMW_ERR      = -9 ! Sep 22: legacy variable off
    OPT_COVAR_MWXTERR =  1 ! Aug 28, 2012: default is ON

    FUDGE_MAG_COVERR = ' '

! extinction options (also see GET_SNXT8 in snana.car):
!  1 => use default extinction from CCM89 law
!  2 => use Saurabh's parametrized extinction from alphs,beta,zeta

    OPT_SNXT     = OPT_SNXT_CCM89

! set algorithm for picking rest-frame color
!   1 => use two closest rest-frame filters
!   5 => use closest filter, and then use color corresponding
!        to Saurabh's lookup (use this option for sync-tests only)

    OPT_NEARFILT   = OPT_FILTOBS  ! obsolete : Feb 3 2012

! set option for Kcor errors; use SJ option (=2) as default
...
! set default Landolt option OFF
    OPT_LANDOLT = 0
    do i = 1, 10
...
! c      CALL READ_DEFAULT_PRIORS ! removed Aug 13 2015

! ---------------------
! read the 'snana' namelist file for the FITINP namelist
...
! first, allow user to set integer option or logical flag.
   ...
   ! But note PRIOR_ZERRSCALE=1000.01 by default
       OPT_PHOTOZ = IBSET(OPT_PHOTOZ,BIT_PHOTOZ_GAUSS)
    endif

! Set non-zero step-size for PHOTOZ or DLMAG,
...
902   C1ERR = 'Could not find &FITINP namelist in'
    ...
    RETURN
  END SUBROUTINE RDFITNML


! =================================
    SUBROUTINE PARSE_NML_FILTLISTS
! 
! Created Nov 24 by R.Kessler
! Prcoess namelist strings FILTLIST_XXX and NEARFILT_IGNORE_REST,
! and fill integer arrays.
! 
! Check every character to allow spaces or no spaces
! betweeh filters: i.e, 'gri' and 'g r i' are both allowed.
! 
! Apr 14, 2013:
!   Allow fitting peak-mags in extra filters. For example,
!   to fit ugriz and also compute peak mags in B & V,
!      FILTLIST_FIT   = 'ugriz+BV'
! 
! Apr 17 2017: check ABORT_ON_BADFILTER
...
!  skip DUMP options after 20 filters
      if ( i .GE. 20 ) GOTO 100
...
100   CONTINUE
    ...
    RETURN
  END SUBROUTINE PARSE_NML_FILTLISTS
...
! --- keep legacy variables TREST_REJECT and MJD_REJECT
...
! xxx mark delete Aug 14 2024   IF ( i .GT. ilast ) THEN
    ...
    RETURN
  END SUBROUTINE FITNML_OVERRIDE


! ==========================================
    SUBROUTINE FITINI_PARVAL ( iter )
! 
! Init INIVAL_ and INISTP arrays.
! 
...
! init everything to BIDON value or to value of last iteration
...
!  Jun 2021: set tiny z errors too
...
! - - - - shape/stretch - - - -
! note that INIVAL(IPAR_SHAPE,IPAR_SHAPE2) depend
! on the model and are evaluated below.
    ipar            =  IPAR_SHAPE
...
! -------------------------------------------
! check for valid model and set model-dependent parameters
...
! Mar 24, 2012: set bound on AV and DELTA (for photoz fit)
...
! Dec 2023: check for M2 component
       if ( NSHAPEPAR > 1 ) then
...
! for SALT2, DLMAG is replaced by x0:
...
! note that X08min <-> MU8MAX and vice-versa
...
! if user fixes DLMAG, then fix X0.
...
! make sure that alpha & beta are defined for photoZ fit.
...
! set bounds on color and stretch
! Mar 24, 2012: reduce range from +-2
! Dec 5 2013: reduce max color frmo +2 to +1.
! Aug 30 2021: max color -> 2 to fit redder events
...
! check if user has specified any INITIAL values.
! To fix a parameter value, must also set INISTP_XXX = 0.0
...
! AV & COLOR are the same parameter index;
! so allow one or the other.
...
! May 2022: check option to set all of the INIVAL to sim-values
!    Note that fit floats all params
...
420         format('   Set INIVAL = SIM value (USESIM_INIVAL=',L2,  & 
    ...
    ENDIF
! --------------------------------------------
! store SIMVAL and check option to use sim values in fit.
...
! now check if want to fix a parameter with the simulated value.
...
! --------------
! set FITVAL on 1st interation since this array
! is used in the ADJUST & FITINI_XXXX routines.
! (exception for option that fixes parameters)
    ...
    RETURN
  END SUBROUTINE FITINI_PARVAL 


! ==========================================
    SUBROUTINE FITINI_PHOTOZ ( iter , IERR)
! 
! Created Jul 11,2006 by R.Kessler
! Used only if DOFIT_PHOTOZ=T.
! Estimate initial PHOTOZ value and fill INIVAL(IPAR_zPHOT)
! 
! * if there is a valid host, then INIVAL = Zhost
! 
! * if no host and 1st iteration, loop over redshift in bins
!   of 0.05 with AV=0 and LUMIPAR=0; find min CHI2.
! 
! * if LREPEAT, set INIVAL = PHOTOZ from previous fit
! 
! Jan 2024: wrap Quantile debug prints under LDMP_Q flag
! May 2024:
!   + add IERR argument based on init_zPDF_spline
!   + fix bug and use MASK_PHOTOZ_SOURCE instead of OPT_PHOTOZ
! 
...
! set global MINUIT bound each time since it may
! get over-written in FITINI_PARVAL

    ZMIN = PHOTOZ_BOUND(1)
    ZMAX = PHOTOZ_RANGE_KCOR(2)
    ZMAX = min ( ZMAX, PHOTOZ_BOUND(2) )

! reset photoz step, unless this is a 2-band 'FITRESTMAG' fit (Nov 28, 2014)
...
! ----------------------------------------
! set initial value and step size.
...
! skip search grid if initial params are going to be over-written
! from fit params in external FITRES file
    if ( USE_INIVAL_SNCID_FILE ) RETURN

! -------------------------------------------------
! if we get here, then we have just the SN with no hostZ to lean on;
! search course grid to find min chi2.
...
! -----------------------------------------------------
! call FCNSNLC with INIVAL and IFLAG = FCNFLAG_USER (last-call)
! to set EP_XXX arrays with initial conditions.
! This is needed to check initial data/model normalization.

! open up the x0 bound for the course grid search (Feb 4, 2014)
    IF ( FITMODEL_INDEX .EQ. MODEL_SALT2 ) THEN
       INIBND(1,IPAR_X0) = 1.0E-15
       INIBND(2,IPAR_X0) = 10000.
    ENDIF

! Use initial guess to get starting CHI2GUESS
    z       = INIVAL(IPAR_zPHOT)
    s       = INIVAL(IPAR_SHAPE)
    c       = INIVAL(IPAR_COLOR)
...
890      format(T5,'Finished ', A,' for CID=',A,  & 
    ...
    RETURN
  END SUBROUTINE FITINI_PHOTOZ 

! =============================
    SUBROUTINE PRINT_INIPAR_ZPHOT(string,INIPAR_VAL,chi2)
! Created July 2012
! FITINI_PHOTOZ print utility to show
!     initial-[string]: zPhot,s,c,x0 = [values]
! - - - -
...
! ----------- BEGIN --------
 ...
 44   format(T5,'initial-',A, 2x, 'CHI2 = ',G12.4 )
    ...
    RETURN
  END SUBROUTINE PRINT_INIPAR_ZPHOT
...
! Created Jul 2019
! Estimate initial all 5 photo-z params using MCMC  method.
! The goal is to significantly reduce init time compared with
! the COURSEGRID method, and maybe even do a better job.
! Works only for SALT2 model.
! 
...
!     NEVAL_MCMC_PHOTOZ = int(PARLIST_MCMC_PHOTOZ(1)) ! already done
...
! for each param, get initial start value and step size
    DO i = 1, NPAR
       IPAR = IPAR_LIST(i)
...
! store initial chi2min in case we don't find better
    CHI2MIN = CHI2
    DO i = 1, NPAR
       IPAR = IPAR_LIST(i)
       INIVAL_SAVE(IPAR) = INIVAL(IPAR)
...
! call function to evaluate chi2 for this INIVAL trial
       CALL FCNSNLC(NFITPAR_MN,GRAD,CHI2,INIVAL,IFLAG,USRFUN)
...
! reduce step size, and move to current values with min chi2
          NTMP = 0
          DO i = 1, NPAR
             IPAR = IPAR_LIST(i)
...
! restore INIVAL_SAVE at mon chi2 to global INIVAL
    DO i = 1, NPAR
       IPAR = IPAR_LIST(i)
       INIVAL(IPAR) = INIVAL_SAVE(IPAR)
    ENDDO
...
! 
! Call by FITINI_PHOTOZ to set color (IPAR=IPAR_COLOR) or
! stretch (IPAR=IPAR_SHAPE) bins. These bins are used
...
! local args
    ...
    LOGICAL DO_SALT2, DO_COLOR, DO_SHAPE, DO_PEAKMJD
...
! 
! Re-Created April 6, 2009 by R.Kessler
! 
! Compute data/model ratio and adjust either MU or x0
! so that initial model matches data better.
! This is much simpler than original version.
! IERR  = 0 -> OK
! IERR != 0 -> bad adjustment; should skip fit
! 
! Aug 30 2017: use function FIRST_ITERATION instead of test on ITER.
!              --> accomodate SIMFIT_IDEAL logic
! 
! Mar 4 2020
!  + modify PKMJD sweep from -2, 2 in 1 day bins, to -4 to +4 in 2 day bins.
!  + remove CHI2TEST on 5*NEPOCH
! 
! Aug 4 2021
!  + add optional color grid search with INIVAL_GRIDSEARCH_COLOR - D. Jones
! 
! Jul 23 2024: add IERR arg
! 
! ----------------------------------------
...
! process only 1st fit iteration
...
! load inital PEAK MJD and re-run FCNSNLC to update EP_FLUX_MODEL
...
! -----------------------------------------------------
! Now check Data vs. Model;
! The data/model flux ratio-average will serve as
! an estimator for peakmag(data)-peakmag(model).
...
! -------------------------------------------
! check chi2 at end of adjustment
...
! and minimized PEAKMJD
    ...
    RETURN
  END SUBROUTINE FITINI_ADJUST 

! ===========================================
    SUBROUTINE FORCE_BANDLIST_SNCID_FILE()

! Created Jan 2025
! Force filters used in previous fit and read from argument of SNCID_LIST_FILE.
...
! load global info to use these bands
...
! disable feature to add/drop filters
    ...
    RETURN
  END SUBROUTINE FORCE_BANDLIST_SNCID_FILE

! ==========================
    SUBROUTINE FITINI_LIST_FILE(iter)

! Created Apr 29 2022 by R.Kessler
! If &SNLCINP SNCID_LIST_FILE is set, check for previous SALT2/SALT3 fit
! values to use as INIVAL when iter=1.
! If INISTP(ipar)=0 [fixed parameter value],
! apply previous SALT2 fit value for all iterations.
! 
...
! construct list of params to check in SNCID_LIST file
...
663     format('check fit pars in argument of SNCID_LIST_FILE')
    ...
    RETURN
  END SUBROUTINE FITINI_LIST_FILE

! =======================================
    DOUBLE PRECISION FUNCTION SET_PRIOR_FITPAR_SIG(IPAR,DVALSIG)

! return Gaussian sigma to use as prior; this sigma is based
! on previous LC fit, and intended to use for systmatics to
...
! hard-wire scale on previous fitted sigma to avoid suppressing systematic
! while still avoiding crazy outliers. This is not well tuned so may
! need more study.
    SCALE_SIGMA = 1.0

! SIG_FLOOR logic is desgined for SALT2/3, and maybe works for BayeSN ??
! But set a trap for BayeSN just to alert the user
    IF ( FITMODEL_INDEX .EQ. MODEL_BAYESN ) THEN
      c1err = 'Must check SIG_FLOOR logic below before '  & 
                // 'bypassing this abort.'
      c2err = 'Does BAYESN need different SIG_FLOOR values ?'
      CALL MADABORT ( "SET_PRIOR_FITPAR_SIG", c1err, c2err )
    ENDIF

    SIG_FLOOR  = .01
    MAGERR_MIN = 0.03  ! guess
    IF ( IPAR == IPAR_COLOR ) THEN
       SIG_FLOOR = MAGERR_MIN/3.1  ! beta*cERR >= MAGERR_MIN
    ELSE IF ( IPAR == IPAR_SHAPE ) THEN
       SIG_FLOOR = MAGERR_MIN/0.15  ! alpha*sERR >= MAGERR_MIN
    ELSE IF ( IPAR == IPAR_DLMAG ) THEN  ! MU or x0
       SIG_FLOOR = MAGERR_MIN
    ELSE IF ( IPAR == IPAR_ZPHOT ) THEN
       SIG_FLOOR = 0.02   ! guess
    ENDIF

! few debug option to play with scale
...
! Aug 27 2025 : avoid too-small PRIOR_SIG
    if ( PRIOR_SIG < SIG_FLOOR ) PRIOR_SIG = SIG_FLOOR

    SET_PRIOR_FITPAR_SIG = PRIOR_SIG
    RETURN
  END FUNCTION SET_PRIOR_FITPAR_SIG

! =====================================
    SUBROUTINE FITINI_TABLEVAR

! -----------------------------------------------
! define extra "PLOTVAR" variables to plot along
! with fit values. These are all plotted as a
! function of ISN.
! 
! Jul 21, 2009: fix index bug inside LSIM if-block;
!               increment NPLOTPAR at end of bloc, instead of at start.
! 
! Sep 2, 2010: check NPLOTPAR array bound before filter loop
! Feb 13,2012: check NFILTDEF_FITRESTMAG
! 
! Sep 29, 2012: call CWNT_DUPLICATE_FILTER(i)  to check for duplicate
!               filter for case-insensitive ntuple.
! 
! Jun 24 2013: always ignore FILTER_REPLACE filters for plotting.
! 
! Mar 5 2020: ad NPTFIT_REJECT (due to DELCHI2)
! 
! -----------------
...
! Oct 27, 2011:
! get fixed number of filters to include in plots and ntuple,
! and create mapping between this fitted subset and
! the complete list of NFILTEDEF_SURVEY
! 
...
! - - - - - - - - - - - - - - - - - - - - -
! check filter-REMAP option (Feb 2017)
...
! ----------------------
! define names of extra calculated parameters to plot;
! these parnames are used only in HFFITRES

    NPLOTPAR = IPAR_MAX

! make sure we don't exceed bound before filling arrays
...
! init everything, even for unused filters.
...
! check for duplicate filters such as g,G or r,R ...
! perfectly allowed in snana, but ntuples cannot distinguish
...
! plot peakMags in rest-frame.
! The number of rest-frame filters can be different
! than the number of observer-frame filters,
! so do NOT put this part into the 555 loop above.
...
! pointer to assoc NPLOTVAR with ifilt-rest
       PARPTR_IFILT_REST(ifilt_rst) = NPLOTPAR
      ENDDO
    ENDIF

! Feb 2012: Optional FITRESTMAGs
...
! April 2013: optional peak-mag(s) in other observer-frame filters
!             e.g.: FILTLIST_FIT = 'ugriz+BV' -> include B&V peak mags
...
! Dec 5, 2006: extra Z plots for PHOTOZ fits.
...
663     format('Exceeds MXFITSTORE = ', I4 )
    ...
    RETURN
  END SUBROUTINE FITINI_TABLEVAR
...
! ---------------------------
! init FITCOV arrays used in FCNSNLC and also used
! to get covariance matrix.
! MSKOPT: bit 1 => print  info to stdout
! ITER = 1, 2, ... = fit iteration.
! 
! 
! Feb 2020: add OPT_COVAR_MWXT==2 option to include only DIAG part.
! May 2021: pass param lists to gencovar_SALT2
! Jul 2024: load PARLIST_SN(4:5)
! 
! ----------------------------------------------------------
...
! -------------------------------
!  Fill R4EP_XXX and I4EP_XXX arrays.
...
! -------------------------------------------------
! set flag to use diagonal elements only if
! OPT_COVAR_FLUX <= 0 or if this is the 1st iteration
...
! 670   format(T8,'Use ',I3,' x ', I3,' Covariance computed ', A)
...
! alway init covariance matrix to zero
...
! ----------------------------------------------------
! get COVMAT1 covariance matrix on 2nd iteration
! This linear array is converted below into a 2-dim array.
! This is the model-covariance only.
...
! print rho-fudge formula by calling with args = 0
...
! -------------------------------------------------------------
! fill the 2d matrix, COVMAT2, and store diagonal errors.
...
! store diag errors in mag space
       if ( irow .EQ. icol ) then
...
!        CALL DMP_COVAR(iter)  ! for DEBUG only

     FUDGE_SQERR = FUDGE_MODELERR_SCALE**2

! now convert COVMAT2 model-magerr^2 to  model-fluxerr^2,
! and add data errors. Note that COVMAT2 changes here.

! COVMAT1/COVMAT2 is the model-magerr covariance.
! Below convert to FLUXERR covariance and add the following
!  - XTMW covariance
!  - model-err cov
!  - data error covariance
! 
! Also fill the 2d matrix, COVMAT2
...
! get the model covariance
        ...
        ELSE IF ( LDIAG ) THEN
! note that FUDGE_MODELERR_SCALE has already been
! applied to R4EP_MODELFLUX_ERR
           ERR1 = R4EP_ALL(ep_row,JEP_MODELFLUX_ERR)
           COV_MODEL = ERR1 * ERR1
       ENDIF

! check option to turn off model error.
       IF ( FUDGE_MAGERR_MODEL .EQ. 0.0 ) COV_MODEL = 0.0

! check for XTMW covariance
...
! redefine COVMAT2 in fluxerr space
       COVMAT2(irow,icol) = COV_MODEL + COV_MWXT + COV_DATA

312    CONTINUE
311    CONTINUE

!        CALL DMP_COVAR(iter)  ! for DEBUG only DDDDDDDD

! invert COVMAT2 matrix.

     CALL INVERTMATRIX( MXFIT_DATA, NFITDATA, COVMAT2)

! !!! Now COVMAT2 is the inverted COV matrix !!!

! check for min value of inverted matrix
...
! Created Sep 28, 2012
! 
! Fill R4EP_ and I4EP_ arrays so that first NFITDATA elements
! are the ones to fit; the next NFITDATA_REJECT elements
! are rejected epochs. This sorting is done so that FCNSLNC
! can efficiently compute chi2,  and still be able to record
! rejected epochs [when LAST=T] for plotting.
! When IREJ=0, store epochs to fit;
! when IREJ=1, tack on rejected epochs.
! 
! Feb 26 2015: load CHI2FLUX_SIM for simulation (LSIM_SNANA=T).
! 
! -------------------------
...
! -------------------------------
! save previous R4ALL values in R4_LAST array in case
! previous iteration values are needed; e.g. FCNCHI2_SIGMA
...
! find sparse index 'ifilt' that corresponds to IFILT_OBS
       ifilt = IFILTDEF_INVMAP_SURVEY(ifilt_obs)
...
! immediately skip epochs that fail cuts, or epochs whose
! flux-error is negative

      IF ( .not. MJDSELECT(imjd,0) ) GOTO 101

! check if this epoch is rejected
...
! count rejected epochs within TREST window
...
! increment number of epochs (fit + rejected)
...
! get local flux and error from model (non-zero only if ITER > 1 )
...
! load FITCOV arrays
      ...
      I4EP_ALL(ep,IEP_DETNUM)      = ISNLC_DETNUM(ep)
      I4EP_ALL(ep,IEP_IMGNUM)      = ISNLC_IMGNUM(ep)
      BANDEP_ALL(ep) = FILTDEF_STRING(IFILT_OBS:IFILT_OBS)

! Get nearest and 2nd nearest rest filter
! Also subtract index offsets
...
! set error on MilkyWay Galactic extinction
...
! - - - - - -
 ...
 670       format('NFIT_DATA=',I4, '  exceeds MXFIT_DATA=',I4, 3x,  & 
    ...
    RETURN
  END SUBROUTINE LOAD_EPALL

! ==========================================
    SUBROUTINE LOAD_EPFIT()

! Creatd Jan 25 2014
! Transfer R4EP_ALL -> sparse R4EP_FIT
...
! load sparse _FIT arrays
 ...
 400  CONTINUE
    ...
    RETURN
  END SUBROUTINE LOAD_EPFIT

! ================================
    SUBROUTINE PRINT_ITERINFO(CCID,ITER,COMMENT)

! print line as formated as
!  [CID](ITER=[ITER]):  [COMMENT]
! 
...
! Created Jul 20 2011 by R.K
! Return MAXFRAC such that fudged error is MAXFRAC * PEAKFLUX
! Based on FUDGE_FITERR_XXX parameters.
! Evaluate MAXFRAC separeately from global FUDGEALL_XXX options
! and also from FUDGE_FITERR_XXX options.
! Return maximum of these two MAXFRAC  values  so that we
! essentially OR the two options.
! 
! Feb 28 2020:
!   FIRST_ITER now includes !LREPEAT_ITER. Restore old bug
!   with RESTORE_DES3YR.
! 
! -------------
...
! ---------
! check options that depend on TREST, LAMREST and filter
    ...
    RETURN
  END FUNCTION GET_FUDGE_FITERR_MAXFRAC 

! ==========================================
    SUBROUTINE FITINI_PRIORS()
! 
! One time init:
! Initialize -2 * ln(prob) for prior on each fit variable.
! Fills PRIOR_CHI2GRID array so that chi2-contribution
! can be looked up via interpolation in CHI2_PRIOR function.
! 
! Priors initialized for:
!  - AV     (exponential convoluted with Gaussian)
!  - MJD    (gaussian)
!  - photoZ (Gaussian -- added Dec 30, 2006)
! 
! Apr 2 2018: call FITINI_FLATPRIOR for color
! 
...
! initalize all chi2-prior to zero.
...
! initialize prior ranges to zero
...
! Set peak-MJD PRIOR for all models, and even if OPT_PRIOR=0.
! This is mainly to protect against crazy MJD values,
! and it is not meant to bias the fit.
...
! - - - - - -  -
! now turn on prior(s) specific to model
...
! so that chi2-contribution can be looked up via
! interpolation in CHI2_PRIOR function.
! 
...
! set up binning used to store PHOTOZ prior on grid.
...
50    CONTINUE
    ...
    RETURN
  END SUBROUTINE FITINI_ZPRIOR

! ==========================================
    SUBROUTINE FITINI_MUPRIOR(LSTAT)
! 
! Created Aug 7, 2009 by R.Kessler
! 
! Initialize DISTANCE-MODULUS prior,
! Fills PRIOR_CHI2GRID array with Gaussian with sigma=1.0
! so that chi2-contribution can be looked up via
! interpolation in CHI2_PRIOR function.
! 
! Prior chi2 =  [ (MU - MUREF) / scale*MUERR ]^2 .
! 
! where scale = PRIOR_MUERRSCALE scales the error.
! 
! WARNING: this should be used only for photoZ fits
!          to get a better estimate of redshift.
!          ==> do NOT use this for cosmology !!!
! 
! ------------------------
    ...
    REAL*8 TMP, pull_BINSIZE, PULL, MU
...
! set up binning used to store DLMAG prior on grid.
...
! 
! Created Aug 21, 2006 by R.Kessler
! 
! Initialize MJD prior.
! Fills PRIOR_CHI2GRID array so that chi2-contribution
! can be looked up via interpolation in CHI2_PRIOR function.
! 
! Prior chi2 =  [ (MJD - INIMJD) / PRIOR_MJDSIG ]^2 .
! 
! This is designed to protect against crazy MJD values that
! result in FLux(model) = 0 and reasonable chi2 ...
! The MJD prior will give a big chi2 when MJD wanders off.
! 
! Note that MJD really mean PEAKMJD here.
! 
! ------------------------
...
! set up binning used to store MJD prior on grid.
...
! flare the chi2 at the edges so that extrapolation
! has much larger slope. Note that CHI2 function and
! its derivative are still continuous at the MIN[MAX]DEF_PKMJD2
! boundary.
...
50    CONTINUE
    ...
    RETURN
  END SUBROUTINE FITINI_PKMJDPRIOR

! ==========================================
    SUBROUTINE FITINI_FLATPRIOR(IPAR,LSTAT)
! 
! Created Aug 2,  2007 by R.Kessler
! 
! Initialize prior that is flat over defined range,
! and then has guassian roll-off.
! Fills PRIOR_CHI2GRID array so that chi2-contribution
! can be looked up via interpolation in CHI2_PRIOR function.
! 
! Works for DELTA, PHOTOZ, RV
! 
! Apr  2 2018: check PRIOR_COLOR_RANGE
...
! set up binning used to store prior on grid.
...
! keep track of min/max bin in FLATPRIOR range;
...
! allow non-trivial priors to multiply flat prior
...
50    CONTINUE
    ...
    RETURN
  END SUBROUTINE FITINI_FLATPRIOR
...
! can be looked up via interpolation in CHI2_PRIOR function.
! 
! Jul 27,2007: check option for ESSENCE/WV07 prior
! 
! Jun 22, 2009: check OPT_PRIOR_AV for flat-prior option
! 
! Apr 28, 2011: set C2ERR when PRIOR_AVRES < 0
! 
! Sep 08, 2011: FLOAT() -> DBLE()
!               REAL*8 AVRES = DBLE(PRIOR_AVRES)
!               to have consistent double precision usage.
! 
! ------------------------
...
! determine how many exponentials used in prior.


! check for flat prior
...
! set up binning used to store AV prior on grid.
...
! setup binning for Gaussian smear: bin size = smaller of
! 1/10 of sigma or AV1_binsize.
! First AV2 bin has positive AV; last AV2 bin is 3-sigma
! past range.
...
! now convolve exponential AV-prior with gaussian smear
! (to allow for negative AV)
! AV1 includes unphysical AV due to smearing;
! AV2 includes only physical AV
...
! start with loop over AV that includes negative values.
...
! now loop over physical AV2 > 0, and go past defined range to
! accoount for smearing from beyond defined range.
...
! large AVEXP is a flag to have no prior on AV
...
! -----------------------------------------------------------
! Now check if chi2 > 1400 (at negative AV) which causes prob
! to goto zero due to floating point overflow.
! To avoid discontinuity in chi2, the chi2 is extrapolated
! from a safe region.

! get chi2max from most neg AV in 1st bin

    chi2max = PRIOR_chi2grid(1,ipar_av)

! look for first bin with chi2 < chi2max
...
! get slope from two neighbor chi2 points
...
! Loop over AV bins again, and replace chi2 in overflow region
! with extrapolated chi2.
...
! define variables for polynomial interpolation
...
! Created Jan 2025
! Return Gauss prior chi2 for OPT_SNCID_LIST += 4 option;
...
! --------- BEGIN ------------
    ...
    RETURN
  END FUNCTION CHI2_PRIOR_SNCID_FILE

! =================================
    DOUBLE PRECISION FUNCTION PRIOR_ZPULL(ZSN)
! 
! Created April 20, 2007 by R.Kessler
! 
! Returns ZPULL = (Z - Zref) / err
! to be used in chi2-fit function.
! 
! Note namelist option
!  OPT_PHOTOZ += 1  => always use host-galaxy photoZ
!  OPT_PHOTOZ += 2  => use spectro Z when available.
!  OPT_PHOTOZ += 4  => use quantiles instead of zphot point estimate
! 
! Jan 27 2018:
!  + use DOFIT_PHOTOZ_HOST to determine LPULL
! 
! Mar 02 2022: load global PRIOR_PHOTOZ[_ERR]
! Apr 16 2024: set zerr floor to E-3 instead of E-5 to avoid pathologically large prior-chi2.
! 
! Dec 20 2024: PRIOR_PHOTOZ[_ERR] is loaded before fit in FITINI_PHOTOZ
! ------------------------------------------------
...
! local
    ...
    REAL*8  & 
         muerr, mudif, muref, mu_w0, mu_wa, mu_om  & 
        ,W_SAVE, OMAT_SAVE, sqdif, RHO_W0WA  & 
...
! to get distance error.

    sigmu_w0 = mu_w0-muref
    sigmu_wa = mu_wa-muref
    sigmu_om = mu_om-muref
    sqdif = sigmu_w0*sigmu_w0 + sigmu_wa*sigmu_wa + sigmu_om*sigmu_om

! Nov 7 2025: very rough correction for w0/wa correlation
    RHO_W0WA = -0.8
    sqdif = sqdif + 2.0*sigmu_w0*sigmu_wa*RHO_W0WA

    muerr = sqrt(sqdif)
    mudif = MUSN - muref

! inclucde user MUERRSCALE in evaluating the pull.
...
! 
! Apply quality cuts to fit results AFTER all fit-iterations
! have finished.
! Returns LCUTS = T if fit is acceptable.
! 
! Cuts on:
! 
!  * min Trest
!  * max Trest
!  * TGAPMAX & T0GAPMAX
!  * NEPOCH_FIT per filter
!  * valid LUMIPAR range
!  * all but 1 error is from MINOS
!  * DLMAG error is from MINOS
!  * ZPULL (for PHOTOZ fit only)
!  * fit prob
!  * SNRMAX & SNRMAX2
!  * SIGMACHI2
!  * FitPar and covariances
! 
! 
! Sep 17 2016: cut on fitted covariances; see CUTWIN_FITCOV
! 
! Mar 10 2017: cut on CUTWIN_FITERRz1
! 
! Jan 4 2021: check OPT_SNCID_LIST option to accept everything.
! 
! ------------------------------------------------
...
! ------------------------------------------------
! check Tobs[rest]_min,max using fit PEAKMJD instead
! of SEARCH_PEAKMJD. Use same "cutwin_Tobs[rest]*" parameters
! that snana uses.

! loop over epochs used in fit and make TLIST array
...
! apply SNRSUM cut, July 2024
    LTMP = LCUTVAR(SNRSUM_FIT, CUTBIT_SNRSUM, LVBOSE)
    IF ( .NOT. LTMP ) LCUTS = .FALSE.

! -------------------------------------------------------
! Check that Trestmin/max are withing FITWIN_TREST,
! but allow slop in TRESTCUT_EXTEND.

! is this redundant with Trestmin[max]_FIT cuts above ??
! Feb 2014: skip this for photo-z fits.
...
! - - - -
!  Dec 18 2017: add TRESTRANGE cut
      xval     = TrestRange_FIT
...
! -------------------------------------------------------
! cut on fitted COVariances (Sep 2016)
...
! Mar 10 2017 check cut on sigma/(1+z)
...
! -------------------------------------------------------
! cuts for PHOTOZ fits only: Zpull
...
! Cut on sigma-chi2
    ...
    IF( LVBOSE ) print*,CLINE
! -------------
! 7/13/2008 - cut on number of passbands passing SNR
...
! May 25, 2009 - cut on number of passbands passing 2nd SNR
...
! cut on Nfilt passing Trestmin & Trestmin
...
! -----------------------------------------------
! Allow only 1 of the errors to fail MINOS errors;
! 2 or more parabolic or BAD => skip
...
! count bad-errors => number of fitpars with invalid MINOS error
! (these errors are either parabolic or crazy-valued)
...
! ----------------------------------------------
! require that DLMAG has MINOS-error (if requested)
...
! 
! store info in  FITVAL_STORE and FITERR_STORE arrays.
! These arrays are used for FITRES table output.
! 
! Feb 20 2017: refactor using new calls to SET_FITANA_STORE;
!              accomodates SNTABLE_FILTER_REMAP option.
! 
! --------------------------------------------------
...
! re-evaluate Trest-related variables

! set global redshift variable & local peakmjd
    REDSHIFT_FIT = LCVAL_STORE(IPAR_zPHOT)
    z1 = 1. + REDSHIFT_FIT

    PEAKMJD      = LCVAL_STORE(IPAR_PEAKMJD)

! loop over epochs used in fit: rejected epochs are excluded here.
...
! -------------------------------------------------
! get peakmags in obs and rest frames.
! Make sure to get ALL of the rest-frame filters,
! even if there is no obs filter than maps on
...
! compute number of fitted filters that pass global user-SNR cut
! This accounts for fudged errors so that deweighted passbands
! are not counted in NFILT_SNRMAX_FIT.
...
! fill SNRMAX_SORT_FIT to get sorted SNRMAX for fitted bands.
...
! -----------------------------------------------
! store extra info in FITVAL_STORE array so that
! all gets plotted the same way as fit-parameters.

    if ( LTRACE ) CALL DMPTRACE("START 555 LOOP in FITANA_STORE")

    NPAR = PARPTR_CHI2

! store global fit parameters: Ndof, chi2 and prob(chi2)
! Note index here is NPAR instead of ipar
...
! set store-masks. first number refers to storing VAL or error.
! Second number is to take average(4) or sum(8) for REMAPed filters.
...
! now load rest-frame peakmags; use pointer to find IPAR.
! Note that number of rest-frame filters could be more
! than the number of obs-frame filters.
...
! Feb 2012: optional things to store for special fits
...
! store DELMU = MU(fit) - MU(Zspec)
! If REDSHIFT < 0 (undefined) then use photoz
...
! Make sure to always increment NPAR.
...
! store SALT2 mB in IPAR_MB index ..
! do NOT increment NPAR here
    IF ( FITMODEL_INDEX .EQ. MODEL_SALT2 ) THEN
      CALL SALT2_STORE()
    ENDIF
...
! Created Feb 2017
! 
! Copy element of ARRAY into either FITVAL_STORE or FITERR_STORE,
! which are used to fill the SNTABLE.
! 
! For nominal jobs, load ARRAY(ifilt) into FITVAL[ERR]_STORE[IPAR].
! But for FILTER_REMAP_TABLE, then figure out which original
...
! -----------------------------------------------------
! if we get here, then figure out the FILTER-REMAP

    IFILT_REMAP = IFILT

! loop over original filters to see which one(s)
! were remapped onto IFILT_REMAP
...
! xxxxxxxxxxxxxxxxxxx
!        write(6,66) IFILT_ORIG, IFILTOBS_ORIG, IFILTOBS_REMAP,
!     &             IFILT_REMAP_TMP, ARRAY(IFILT_ORIG)
! 6      format(T2,'xxx IFILT_ORIG=',I2,2x,
!     &          'IFILTOBS[ORIG,REMAP]=',2I3,' IFILT_REMAP_TMP=',I2,
!     &          '  ARRAY=',F10.2 )
! xxxxxxxxxxxxxxxxxxx
...
500   CONTINUE
    ...
    RETURN
  END SUBROUTINE  SET_FITANA_STORE

! ==================================
    SUBROUTINE FITERR_ADJUST()
! 
!  Created Apr 25, 2007 by R.Kessler
! 
!  If MINOS returns rediculously tiny errors,
!  then set error to a reasonable artificial error.
!  This is needed for the marginalization to work
!  properly.
! 
!  Feb 7, 2008:
!    - set FITERR(IPAR) in addition to FITERR_MINUS[PLUS]
! 
!    - if photoZ error is too big, trim errors
!      so that +-4 sigma does not give unphysical value
! 
! Dec 16, 2011:
!   photoZ error must be at least 40% of color error.
! 
! ------------------------
...
!  [code moved from FCNSNLS]
! 
! SALT2xx1 is the x1 value used in the SALT2  model-error
! calculation. This value is either the same x1 value in
! the fit (if DOCHI2_SIGMA=T) or it is fixed to the x1-value
! from the previous iteration.
! 
! 
! 
! Aug 30 2017: use FIRST_ITERTION(ITER) instead of ITER=1 .
!              Needed for SIMFIT_IDEAL option.
! 
! ---------------------
...
! Created Mar 2018
! 
! Return redshift (zz) for SALT2 error calculation.
! Motivation is for photoz fit, fix zz = z(previous iteration),
! and use zz for SALT2 error calculation.
! 
! Mar 24 2020: fix logic using local DOFIT_zFIX; needed for FITRESTMAG.
! 
! ---------------------
...
!      FIRST_ITER = FIRST_ITERATION(ITER)  ! v10_76b and earlier
    ...
    RETURN
  END FUNCTION SALT2zz

! ========================================
    SUBROUTINE SALT2_STORE()
! 
! April 24, 2009 R.Kessler
! 
! Compute mB* and its error, and store in parameter
! slot IPAR_MB.  Assumes same primary system for
! each survey/filter system.
! 
! 
! Nov 6, 2009:  mB -> mB - FILT_MAGPRIMARY(IFILT_BESS_B)
!               SALT2 mB* is now the same for Vega & BD17 primaries
! 
! Jan 28, 2010: undo above after making more general fix in genmag_SALT2.
! 
! Feb 09, 2010: set redshift to 10pc (see new SAVE_Z)
! 
! Jul 27, 2010: z = ZAT10PC instead of 10.*Zat10pc.
!               No idea why there was a factor of 10 ... perhaps a debug ?
! 
! Aug 11, 2010: huge simplification: just use SALT2mbcalc and get
!               rid of all the stuff for MODELMAB_CALC
! 
! Sep 17 2016: load FITERRMAT
! Feb 16 2017: compute MBerr analytically (fix problem found by ChrisD)
! 
! -----------------------------------------------------------
...
! store mB and it error, and its COV
    ...
    RETURN
  END SUBROUTINE SALT2_STORE

! =====================================
    SUBROUTINE SET_REDSHIFT_FIT(ITER)

! Created Aug 12, 2009 by R.Kessler
! 
! For each event, called from FITPAR_PREP to set initial REDSHIFT_FIT
...
! leave best Z  above ; note that initial photoZ gets modified
! in FITINI_PHOTOZ before the fit starts.
       endif

! use fitted photoZ (from 1st iteration) on 2nd iteration,
! or if 1st iteration is repeated.
...
! June 2025: abort on invalid redshift (likely missing in data file)
    if ( REDSHIFT_FIT < 0.0 .and. OPT_PHOTOZ == 0 ) then
       write(c1err,660) REDSHIFT_FIT, SNLC_CCID
660      format('Undefined REDSHIFT_FIT=', F6.3,' for CID=',A8 )
       c2err = 'Check REDSHIFT info in data file'
       CALL MADABORT("SET_REDSHIFT_FIT", c1err, c2err )
    endif

    RETURN
  END SUBROUTINE SET_REDSHIFT_FIT

! ========================================
    SUBROUTINE PHOTOZ_STORE(NPAR)
! 
! Store PHOTOZ-related variables
! 
! Mar 2024: use MASK_PHOTOZ_SOURCE instead of user-input OPT_PHOTOZ
!           because MASK_PHOTOZ_SOURCE is guaranteed to have only
...
! extract fit photoZ and determine total error.
...
! ---------------------
! Compute photoZ-error from intrinsic MAG-dispersion.
...
! -----------
! for photoZ fit, MU is fixed to initial photoZ.
! Here we change the 'fixed' DLMAG parameter to correspond
! to the final (fitted) photoZ. This is just for internal
! consistency and it does not affect the fit or analysis
! in any way.
    ...
    RETURN
  END SUBROUTINE PHOTOZ_STORE

! ===================================================
    SUBROUTINE FITANA_SUMMARY ( isn )
! 
! print summary of fit vs. filter.
! 
! --------------------------------------------------
...
! create sparse arrays for printing results-vs-filter on one line.
...
! print no more than 6 filters to avoid a messsy output
    NFTMP = MIN(NFILT,6)
...
! write Trest fit range relative to PEAKMJD(fit)
...
20      format(T10,'LCPROBCHI2 WARNING: Ndof=',I2,' for ifilt=',I2 )
    ...
    RETURN
  END FUNCTION LCPROBCHI2

! ====================================
    SUBROUTINE FPKRAT(ifilt_obs)

! Created May 15, 2008 by R.Kessler
! 
! Fill EP_FPKRAT[ERR] = flux/peakFlux in rest frame.
! Loop over all epochs & filters inside here.
! 
! Mar 06, 2009:
!   Correct peakFlux for average data/model ratio so that
!   peakFlux is not way off if the model is off.
!   See new namelist varaible TREST_PEAKKRENORM
!   Fills new array FPKRAW to keep track of raw
!   (uncorrected) Flux/peakFlux ratio.
! 
!   Pass 'ifilt_obs' arg and process only this filter
! 
! Mar 3, 2010: apply K-correction only if kcor != NULLVALL;
!              ==> give something reasonable for SALT2
! 
! Mar 5, 2010: fix aweful bug, argument of PEAKFLUX_OBS_MODEL
!              is IFILT_OBS instead of IFILT. Previous code worked
!              by accident for SDSS, but not for any other survey.
! 
! Dec 14, 2011: avoid divide-by-zero for Fraterr calc
! 
...
! now K-correct from obs-frame back to rest-frame
...
! Amplify error away from peak;
! error scale is 1 at peak, and 3 at 8 days away from peak.

         ratio_err   = ratio_err * sqrt(abs(Trest)+1.)

! compute weight at this epoch
         wgt         = 1. / (ratio_err**2)

! now add additional weight based on
! proximity to peak
...
! --------
! Check if model peakFlux gets renormalized
...
! adjust all of the FPKRAT values.
 ...
 70     CONTINUE
    ...
    RETURN
  END SUBROUTINE FPKRAT

! =====================================
    SUBROUTINE RADIUS_CALC()

! Created Jan 2024
! Compute population radius RADIUS_POP in space of stretch and color.
...
! -----------------------
! 
! Created Jan 2008 by R.Kessler
! Calculate Lightcurve parmaters at MJD using best fit model.
! Returns FLUX, MAG and KCOR
! 
...
! -------
!      print*,' OLD/NEW= ', SNGL(SQSIG_FLUX/U12),
!     &   '  COV=',U12, '  at TREF=',sngl(TREF),'  ifilt=',ifilt
! ------
...
! convert flux to MAG.
    ...
    if ( Flux .GT. 1.0E-20 ) then
      mag    = -2.5 * LOG10(Flux) + MAGOFF + ZP_FLUXCAL
...
! get the K-correction
    ...
    RETURN
  END SUBROUTINE MODELMAG_CALC 

! ======================================
    SUBROUTINE PEAKMAG_CALC ( IFILTDEF, CFRAME )
! 
! Created Jan, 2008
! Just calls MODELMAG_CALC with PKMJD = PEAKMJD
! 
! Feb 20 2017: convert input IFILTDEF to sparse index IFILT,
!              and fill arrays with sparse IFILT.
! 
! ----------------------------------------------
...
! ----------------- BEGIN --------------
    ...
    RETURN
  END SUBROUTINE PEAKMAG_CALC 

! =====================================
    REAL*8 FUNCTION COVFLUXFUN(OPT, ifilt1,ifilt2, T1,T2,LDMP )

! 
! Created April 6, 2008 by R.Kessler
! 
! Get the model-Flux covariance between any two epochs/filters
...
! 
!                           dF1   dF2
! U12       = \sum_{k,l}  ------ ------ * FITERRMAT(k,l)
!                         dPAR_k dPAR_l
! 
!  where PAR_k = fit-parameter with index 'k'.
!  and F1,F2 are fluxes at days with index 1,2.
! 
! If ifilt1=ifilt2 and T1=T2,  then U12 = square of model
! uncertainty at this epoch.
! 
! Mar 26 2014: add new LDMP argument
! 
! -------------------------------
...
301   CONTINUE
    ...
    RETURN
  END FUNCTION COVFLUXFUN

! ======================================
    DOUBLE PRECISION FUNCTION COVFUDGE_DATA( irow, icol )
! 
! Created Feb 23 2015 by R.Kessler
! Compute fudged data-covariance from &SNLCINP  namelist
...
! Created Sep 20, 2006
! 
! Return fudged correlation coefficients for systematic studies
! on model errors.
...
! check option to print fudge-formual

    LDMP = ( irow .LE. 0 .and. irow .LE. 0 )
    IF ( LDMP ) THEN

!          CALL PRBANNER("RHOFUDGE: FUDGE RHO = COV[i,j]/sig_i*sig_j")

       IF ( FUDGE_COVAR .EQ. 0.0 ) THEN
!             print*,'      No fudge for RHO '
...
! bail if fudge is not requested
...
! as FUDGE_COVAR spans -1 to 1, |RHO| spans 0 to 1
! Positive FUDGE_COVAR increases |RHO| up to 1.0 ;
! Negative FUDGE_COVAR decreases |RHO| down to zero.
...
!  make sure that |RHOFUDGE| < 1 to avoid singular matrix.
    ...
    RETURN
  END FUNCTION RHOFUDGE_MODEL

! ==================================================
    SUBROUTINE VMAX_INI

! Created Jun 2013 by R.Kessler
! 
! Initialization for 1/Vmax method.
! Parse user inputs  MAGLIM_VMAX and OPT_VMAX
! 
...
! ---------------------------------
! translate user string MAGLIM_VMAX
 ...
 20   CONTINUE
    ...
    RETURN
  END SUBROUTINE VMAX_INI

! ==================================================
    SUBROUTINE DMPVMAX(CCID,CFILT,NITER,ZMAX,MAGDIF,MAG)

! utility to dump one line for VERBOSE option
...
! Created Jun 2013 by R.Kessler
! 
! Compute Zmax/Vmax for 1/Vmax method.
! 
! Jun 28 2013:
...
! init all global output to crazy values.
...
! strip off a few globals into local variables
    IFILT_OBS = IFILTOBS_LIST_VMAX(ifilt_Vmax)
    MAGLIM    = MAGLIM_LIST_VMAX(ifilt_Vmax)
    PEAKMJD   = LCVAL_STORE(IPAR_PEAKMJD)
    CFILT     = FILTDEF_STRING(IFILT_OBS:IFILT_OBS)

! find brightest epoch, both observed and best-fit.
    DO j = 1, 2
...
! bail if we have an undefined MAG to work with
...
! save fit quantities that need to be changed to find zmax
    z_save = LCVAL_STORE(IPAR_zPHOT)
    d_save = LCVAL_STORE(IPAR_DLMAG)  ! DLMAG or x0

! need color and shape parameter for distance(zmax) calc.
...
! first MAG-DIF for iterative loop
...
! modify zmax and distance before re-computing model mag at zmax;
! for each iteration,
! (zmax/z)^2  = FLux(z)/FLUXLIM = 10**[0.4*(MAGLIM - mag(z))]
       ...
       IF ( LVBOSE  ) THEN
          CALL DMPVMAX(SNLC_CCID,CFILT,NITER,ZMAX,MAGDIF,MAG)
!             print*,'          x0 = ' , LCVAL_STORE(IPAR_DLMAG)
       ENDIF
...
! 
! Created Mar 2013
! Prepare fits for 1-filter only, all other filters,
! and all filters.
...
! get filter index (absolute and sparse) for test filter.
...
! --------
 ...
 100  CONTINUE
    ...
    RETURN
  END SUBROUTINE FITMAGDIF_INI

! =====================================
    SUBROUTINE FITMAGDIF_PREP(ITER)

! March, 2013
! Called before fit-iteration ITER to prepare fit-iteration ITER
! for fitting either FILTER_FITMAGDIF or all filters except this one.
! 
! -----------------------
...
! bail on the nominal ALLFILT interations
    IF ( ITER .LE. NFIT_ITERATION_ALLFILT ) RETURN

! =============================================
! store ALLFILT results just before doing first 2-band fit

    IF ( ITER .EQ. NFIT_ITERATION_ALLFILT + 1 ) THEN

! store original USE_FILT(IFILT) and FITPAR to keep track of which
! filters can be used.
...
! make sure that test filter is on the list; otherwise bail out.

    IF ( .not. USE_ALLFILT(IFILTOBS_FITMAGDIF)  ) RETURN

! reset global list of filters to fit.
    FILTLIST_FIT_USE = ''
    NFILT_OBS_USEFIT = 0

! always restore INIVAL and INISTP
...
! always restore FUDGE_FITERR params.

    FUDGE_FITERR_PASSBANDS = 'NULL'
    FUDGE_FITERR_MAXFRAC   = 0.0

! -----------------
! if done with all test fits, do nominal fit again so that
! analysis variables correspond to nominal fit.
...
! fix all params except for the overall scale/distance parameter
...
! use FUDGE_FITER_XXX parameters to de-wieght test filter
! to effectively exclude it.
 ...
 40   format(T5,'FITMAGDIF_PREP(CID=',A,' ITER=',I1,') -> fit  ', A)
    ...
    RETURN
  END SUBROUTINE FITMAGDIF_PREP


! =====================================
    SUBROUTINE FITMAGDIF_CALC(ITER)

! Computations for stored quantities.
! May 8 2013: compute and store FITPROB_1FILT & FITPROB_XFILT
...
! bail if test filter is not part of fit.
    IF ( .not. USE_ALLFILT(IFILTOBS_FITMAGDIF)  ) RETURN

! strip off useful info
    IFLAG    = IFLAG_FITMAGDIF(ITER)
...
! after both fits, compute mag difference & error.
...
! print one-line summary.
 ...
 50      format(T5,'FITMAGDIF(CCID=',A,') : ', A,'(pred-meas) = ',  & 
    ...
    RETURN
  END SUBROUTINE FITMAGDIF_CALC

! ========================================
    SUBROUTINE GET_FITMAGDIF(MAGDIF, MAGERR, FITPROB )

! Created Mar 2013.
! Return final results via subroutine args.
...
! Created Aug 29 2017
! 
! One-time init for IDEAL FIT with FLUX=TRUEFLUX (sims only)
...
! ---------- BEGIN --------------
    ...
    RETURN
  END SUBROUTINE SIMFIT_IDEAL_INI

! ===============================================
    SUBROUTINE SIMFIT_IDEAL_PREP(ITER)

! Created Aug 29 2017
! Called for each event before fit; prepare to add IDEAL fit.
! 
! ITER=0 --> before fitting begings
! ITER>0 --> before fit iteration ITER
! 
! 
...
! check pre-scale on ITER=0 (before fitting begins)

    IF ( ITER .EQ. 0 ) THEN

! check debug flag to NOT change fluxes, but still run the
! fit twice to see if we get the same results.
...
! check to restore nominal FLUXCAL for normal fit with Poisson fluctuations
...
!       print*,' xxx IDEAL_PREP for NCAL=', NCALL_SNANA_DRIVER

    RETURN
  END SUBROUTINE SIMFIT_IDEAL_PREP

! =======================================
    SUBROUTINE SET_IDEAL_FLUXCAL(OPT)

! Created Aug 30 2017
! OPT > 0 --> set SNLC_FLUXCAL = TRUEFLUX
! OPT < 0 --> restore original FLUXCAL
! 
...
! save measured MWEBV, then MWEBV-> TRUE MWEBV
    ...
    ELSE
!        restore original MWEBV & FLUXCAL
...
201       CONTINUE
    ...
    RETURN
  END SUBROUTINE SET_IDEAL_FLUXCAL

! ================================================
    SUBROUTINE SIMFIT_IDEAL_CALC(ITER)

! Created Aug 29 2017
! Process IDEAL fit after each fit ITER.
...
! compute mB from x0 (Jan 28 2018)
    ...
    RETURN
  END SUBROUTINE SIMFIT_IDEAL_CALC
...
! 
! One-time init.
! if FITLTER_RESTMAG is set, then add this filter
! to suryey-filter list and re-init kcor file to
! include this filter. Called only once.
! 
! --------
...
! -------------
! init number of LC plots per SN to prepare SNLCPAK utility
...
! add FILTLIST_FITRESTMAG to list of SURVEY_FILTERS

! skip this filter if it is already defined as a survey filter.
       IFILT     = IFILTDEF_INVMAP_SURVEY(IFILTDEF)
       IF ( IFILT .GT. 0 ) THEN
         write(6,20) CFILT
...
! -------- adjust NFIT_ITERATION ----------------

! add NFIT_ITERATION for each new filter
    NFIT_ITERATION_ALLFILT = NFIT_ITERATION
    NFIT_ITERATION      =  & 
...
! load IFILTDEF_ITER_FITRESTMAG array to store which filter
! to process for each fit-iteration
...
! -------------------------------------------------
! re-read KCOR file to include FILTLIST_FITRESTMAG
! (user must make sure that FILTLIST_FITRESTMAG is in the kcor file)

    CALL READ_CALIB_WRAPPER(IERR)

    CALL CDTOPDIR_OUTPUT(STDOUT_UPDATE)  ! back to topdir

! check that added filter(s) are all in the KCOR file.
! Abort if any filter is missing.
...
! set flag so that SURVEY_FILTERS are NOT re-read
! if another version header is read.
    FREEZE_SURVEY_FILTERS = .TRUE.

    RETURN
  END SUBROUTINE FITRESTMAG_INI
...
! Feb 12, 2012
! Called before fit-iteration ITER to prepare fit-iteration ITER
! for fitting FILTLIST_FITRESTMAG. Do nothing for first 1/2 of iterations
! to get t0 and stretch. For 2nd half of iterations keep only
! the two observer bands that bracket FILTLIST_FITRESTMAG.
! 
! 
! Apr 24 2014:
!  Re-do SNRMAX logic; apply cut to mask filters before calling
!  BRACKET_IFILT_OBS() function, rather than applying SNRMAX cut
!  to the filters found by BRACKET_IFILT_OBS.  New logic is more
!  efficient. Should fix problem at z ~ 0.4 in which g&r are
!  BRACKET filters but then g fails SNRMAX cut; now it will find
!  r&i instead.
! 
! Dec 3 2014: for photo-z fit, set Z=ZPHOT instead of SNLC_REDSHIFT.
! 
! Mar 26 2020
!  Refactor to use new BRACKET_FILTERLIST with DLAM_TOL so that
!  more than 2 bands can be included if some bands have very
!  similar central wavelengths.
! 
! -----------------------
...
! default flag is NOT 2-band fit;
! setting IFILTOBS_BRACKET below --> 2-band fit
...
! always restore INIVAL and INISTP
! Note that 1st IPAR_ITER param is not restored.
...
! -----------------------------------------------------------
! if done with all 2-band fits, do nominal fit again so that
! analysis variables correspond to nominal fit rather than
! to the last 2-band fit.
...
! set tolerance for including bands close in wavelength
    DLAM_TOL = DLAMTOL_FITRESTMAG

! =========================================

! strip off filter to process for this fit-iteration
...
! for remainder of the iterations (i.e., 2-band fits)
!  - float only color and distance/x0 (i.e,, fix PKMJD and LUMIPAR)
!  - keep only observer bands that bracket FILTLIST_FITRESTMAG
!  - fix redshift (for photo-z fits)
...
! ---------------------------------------------------
! Find obs-filters that bracket FILTLIST_FITRESTMAG
! Before searching for two nearest bands,  EXCLUDE filter if
!   * it's a FITRESTMAG filter (so it does not find itself)
!   * it fails SNRMAX cut  e.g., if g&r are the closest two bands
!     but g fails SNRMAX cut, the BRACKET function will return
!     ri instead.
! 
! Note that loop is over ALL filters, not just FITRESTMAG filters.
! 
...
! Sep 2013: beware that BRACKET_IFILT_OBS() checks all filters
! selected by snana, not just the ones in FILTLIST_FIT.
! Use EPCUT_SNRMIN to reject extra filters.

       LAMTMP = LAMREST(itry)

! new default, Mar 28 2020
...
! check if IFILTOBS_BRACKET(1:2) are valid
...
! print message for which filter to fit, or to skip.
455   CONTINUE
...
! Turn off filters that do not bracket FILTLIST_FITRESTMAG.
! If either of the BRACKET filters is not defined then
! turn off ALL filters so that nothing passes.
...
602      format('CID=',A8, 2x, 'ITER=',I1,' -> RestFilt=',A1)
    ...
    RETURN
  END SUBROUTINE FITRESTMAG_PREP
...
! -----------------------------------------
! Mar 25 2020
! Return IFILTOBS_BRACKET (indices) of obs-filters that bracket
! LAMREST in the rest-frame.
! In the loop over SURVEY filters, IFILT_OBS = -999 is a flag
! (externally set) to ignore this filter.
! 
! This function originally returned just 2 bands: one below and
! one above the target LAMREST. Now it has been refactored to
! return all filters within DLAM_TOL of closest filters.
! 
! Original motivatoin is to work for LOWZ in which each band
! is broken into several bands covering different time periods.
! 
! 
! Inputs:
!   LAMREST   : rest-frame wavelength to bracket
!   DLAM_TOL  : combine obs filters withing this tolerance
!   Z         : redshift
! 
! Output:
!   IFILTOBS_BRACKET(1) : closest band with LAM/z1 < LAMREST
!   IFILTOBS_BRACKET(2) : closest band with LAM/z1 > LAMREST
!   IFILTOBS_BRACKET(3) : optional additional band within DLAM_TOL
!   IFILTOBS_BRACKET(4) : optional additional band within DLAM_TOL
!   etc ...
! 
! Function returns number of bands in IFILTOBS_BRACKET list.
! If only low or high bands are found, function returns 1.
! Function returnig 2 or larger is flag for success.
! Example, if filters a,b,c all have LAM/z1 < LAMREST, then
! function returns 1. But if there is also filter d with
! LAM/z1>LAMREST, then function returns 4.
! 
! ------------------------------------------
...
!       IF ( DLAM_TOL < 1.0E9 ) RETURN ! xxx REMOVE

    LAMZ_LO = FILTOBS_LAMAVG(IFILTOBS_LO)/Z1
    LAMZ_HI = FILTOBS_LAMAVG(IFILTOBS_HI)/Z1

! if we get here, check for more bands with DLAM_TOL ...
...
! higher-wavelength filter
 ...
 200  CONTINUE
    ...
    RETURN
  END FUNCTION BRACKET_FILTERLIST 

! =====================================
    SUBROUTINE FITRESTMAG_CALC(ITER)
! 
! Feb 2012:
! If ITER corresponds to the last iteration for a rest-filter,
! compute and fill global FITRESTMAG and FITRESTMAG_IERR
! 
! Sep 19 2013: call SNLCPAK to show each single-band fit
! 
! Dec 11, 2014: return if ITER > ITERLAST_2BAND to allow more
!                than 1 fit after the 2-band fits.
! ----------------------------------
...
! get absolute filter index of rest-frame filter
    IFILTDEF = IFILTDEF_ITER_FITRESTMAG(ITER)
    IFILT    = IFILTSP_ITER_FITRESTMAG(ITER)

! ---------------------------------------
! sanity check on filter indices
...
! proceed with calc only on last iteration for this filter.
    IF ( ITER .NE. ITERLAST_FITRESTMAG(ifilt) ) RETURN

! save fitted SHAPEPAR in case we use a fixed SHAPEPAR for restMag
    SHAPE_FIT  = LCVAL_STORE(IPAR_SHAPE)
    COLOR_FIT  = LCVAL_STORE(IPAR_COLOR)

! set LCVAL_STORE to get rest-frame mag
...
! store fit-info in local variables (before they get restored!)
    CFILT   = FILTDEF_STRING(IFILTDEF:IFILTDEF)
    SHAPE   = DBLE( LCVAL_STORE(IPAR_SHAPE) )
    COLOR   = DBLE( LCVAL_STORE(IPAR_COLOR)   )

! put this filter on the 'EVAL' list
...
! WARNING: Below is hard-wired for SALT2 ... need to fix for mlcs/snoopy.
...
! -----------------------------------------------------
! for SALT2, get K-corrected FLUX_REST (Jan 2014)
...
! --------------------------
! check option for single-band plot (Sep 2013)

    IF ( DOPLOT_SNLC() ) THEN

       NLCPLOT_FITRESTMAG = NLCPLOT_FITRESTMAG + 1
       IF ( NLCPLOT_FITRESTMAG .EQ. 1 ) THEN
! problem: if next fit fails we have already committed
!          to NFUNPLOT plots ???
    ...
    RETURN
  END SUBROUTINE FITRESTMAG_CALC
...
! 
! For each epoch, compute K-correction and load EP_FLUX_REST array.
! 
...
! ----------------- BEGIN -----------------
! 
! determine which of the IFILTOBS_BRACKET is closest to IFILTDEF(rest)
! Note that the rest-frame filter is defined as an obs-filter.
...
! ----------- BEGIN ----------
    ...
    RETURN
  END SUBROUTINE GET_FITRESTMAG


! =======================================
    LOGICAL FUNCTION LFITFUN_FILT(ifilt_obs,ZTEST)
! 
! Return T if this filter should be included in fit.
! 
! ZTEST > 0 => make z-dependent tests
! ZTEST < 0 => skip z-dependent tests
! 
! Jan 2025: add logic for USE_PRIOR_SNCID_FILE
! 
! ---------------------
...
! did user request this filter.
    if ( .NOT. DOFIT_FILT(ifilt_obs) ) RETURN

! are there any epochs with this filter.
    ifilt    = IFILTDEF_INVMAP_SURVEY(ifilt_obs)
...
! does this observer filter transform to a valid
! rest-frame lambda range
...
! does this observer filter transform to a valid rest-frame filter
...
! Created Feb 10, 2012
! 
! Init stuff related to OPT_LANDOLT.
! Pulled out of FITPAR_PREP.
! 
! May 2012: if BX is defined in the KCOR file but is not
!           a SURVEY_FILTER, then define X here as a SURVEY_FILTER.
!           This part was moved from RDKCOR.
! 
! ------------------------
...
! check for Marriner's "LANDOLT" option
! OPT_LANDOLT bits (lsb=0):
!   bit0 : Landolt => Bessell in rest-frame (BD17)
!   bit1 : Bessell => Landolt in obs-frame  (BD17)
!   bit2 : idem but in Vega system
...
! -----------------------------------------------
! if BX filter is not a survey filter,
! then add it to the SURVEY_FILTERS list and call RDKCOR again.
...
! -----------------------------------
! make sure that BX filters is defined
    C2err =  & 
...
! check for VEGA or BD17 as reference;
! make crude sanity check on FILT_MAGPRIMARY to avoid
! mixing the wrong things.
    ...
    RETURN
  END SUBROUTINE LANDOLT_PREP

! =======================================
    SUBROUTINE LANDOLT_SNMAG_CHECK(OPT,Trest,AVwarp)
! 
! Compute SN mags at t=0 for Bessell and Landolt;
! compare with external calculations.
! 
! -----------------------------------------
...
! hard-wire primary mags from John Marriner.
...
! now invert to get back Bessell from Landolt.
...
! convert synthetic mags into reported BESS mags.
    DO ifilt = IFILT_BESS_U, IFILT_BESS_BX
...
20      format(T3,A, 2x,4(F9.4,3x) )
    ...
    RETURN
  END SUBROUTINE LANDOLT_SNMAG_CHECK

! ====================================
    SUBROUTINE SET_IFILT_OBS(LPRINT)
! 
! Sets up appropriate observer-filter ranges before fit begins
! so that USRFUN need not compute filter ranges during each
! call.
! 
! Set the following FILTCOM varaibles for this SN:
!   IFILT_OBS_EVAL_MASK(ifilt_obs)  => mask of obs filters to evaluate
! 
!  Example: U-band needs to evaluate obs U,B,V,BX
!   => IFILT_OBS_EVAL_MASK(IFILT_BESS_U) = bits 6,7,8,11
! 
! 
! Apr 22 2022: pass LPRINT logical flag as input argument
! --------------------------------------
...
! check if any of the OBS filters are in the Bessell90 system
    LBESS_OBS = .FALSE.
    DO ifilt = 1, NFILTDEF_SURVEY
       ifilt_obs = IFILTDEF_MAP_SURVEY(ifilt)
...
! set default mask to evaluate only observer filter
! (ie.. no color transformations).
...
! -----------------------------------------------------
! check option to correct U with BX-B
! Note that UCOR is used automatically with OPT_LANDOLT;
! this is to allow UCOR_BXB without the LANDOLT option
...
! -----------------------------------------------------
! check options that involve color transformations.
...
44      format(T5,'CID ',A,' : FIT OBSERVER FILTERS ', A )
    ENDIF
! ----------------------------------------
!  print obs filters for each filter when mutliple filters
!  are needed per band, such as for the Landolt correction
...
! 
! Created Jul 18, 2008
! (code moved from FITPAR_INI)
! 
! Feb 19 2010: abort if MAXFRAC>0, but TREST-range or LAM-range
!              is zero.
! 
! Mar 1 2017: do NOT return after FUDGEALL_MAXFRAC -->
!             allow mixing with other options.
! ---------------------------------
...
! ----------------------------------------------------
! sanity checks to prevent setting bad FUDGEs
...
661   format('FUDGE_FITERR_MAXFRAC = ',F8.2)
    ...
    RETURN
  END SUBROUTINE INIT_FUDGE_FITERR

! ==========================================
    SUBROUTINE DMP_DICTFILE(IFLAG)
! 
! Created Apr 13, 2009 by R.Kessler
! 
! Write SALT2 results to "dictFile" ...
! the particular format needed by their
! "hubblefit" program. Note that SNANA does
! NOT use this DICTFILE for anything;
! this is just a convenience option to
! use the original SALT2 hubblefit with
! the output of snlc_fit.exe
! 
! IFLAG = 0 => open file; write header
! IFLAG > 0 => update with next SN
...
! init dictFile and  write header
...
888   CONTINUE
    RETURN
  END SUBROUTINE DMP_DICTFILE


! =======================================
    SUBROUTINE DMP_COVAR(iter)
! 
! Created Jun 2, 2011 by R.Kessler
! 
! Dump non-zero elements of COVMAT2 matrix
...
662   format(T3,'COVMAT Number of non-zero elements: ', I8)
    ...
    RETURN
  END SUBROUTINE DMP_COVAR

! ========================================
    SUBROUTINE DMP_SNMJD_INTERP(IFLAG,OPTMJD)
! 
! For specified MJDs, write MAG(obs & rest) , FLUX & KCOR
! to special dump file. List has been read from
! SNMJD_LIST_FILE. Output file has same name as input,
! but with .OUT extension.
! 
! March 24, 2008:
!       Add OPTMJD (see below)
! 
! May 28, 2008: fix dumb bug so that PEAKMJD dump happens only once.
! 
! May 18, 2012: use new GET_INTERP_MJDLIST utility
! Aug 15, 2012: protect printing info for undefined IFILT_OBS.
! 
! Jul 31 2015: write fitres [column] format to make it easier to parse.
! 
! Nov 19 2019: remove NVAR key
! Dec 04 2019: remove obsolete IFILTOBS_RST to avoid negative index
! 
! ------------
...
! IFLAG = 0 => open output file
! IFLAG > 0 => update file with SN if MJD is specified
! IFLAG < 0 => close file.
...
! avoid repeating an MJD ... mainly to avoid multiple PEAKMJD entries.
      IF ( MJD8 .EQ. MJD8_LAST ) GOTO 200


! get observer mags,flux,kcor at this MJD
...
! Make sure to write out PEAKMJD even if it's fixed.
...
92    CONTINUE
    ...
    RETURN
  END SUBROUTINE DMP_SNMJD_INTERP


! ===============================
    SUBROUTINE PREP_FITRES_TABLEVAR()

! Created Sep 2012 by R.Kessler
! Prepare some FITRES table variables.
! 
! Apr 29, 2013: fill CWNT_COV
! May 14 2014: overhaual using keep, TABLEVARCOM.
! 
! 
...
! ----------------------------------
! load MINUIT reduced correlation matrix into linear array
...
! fill SNRMAX and XTMW arrays with only fitted bands.
...
200   CONTINUE
    ...
    RETURN
  END SUBROUTINE PREP_FITRES_TABLEVAR


! ========================================
    SUBROUTINE TEST_TRANSFORM(Z4)
! 
! Test rest->observer frame transformation.
! For well-defined filter set, this transformation
! is simply a K-correction on a warped spectrum.
! For John's color transformation, it's a little more
! complicated.
! 
...
!      write(6,18) MAG_REST_TEST
! 8    format(T3,'REST UBVRI mags: ', 5F8.4 )
...
! c         MAG8 = -2.5*DLOG10(FLUX8/ADUSCALE)
       MAG8 = -2.5*DLOG10(FLUX8) + ZP_FLUXCAL
...
! 
! Pack light curve & best-fit function for snlc_fit.exe.
! Also pack optional info such as marginalized distributions.
! 
! ---------------------------------------------
...
! -------------------------------------------
! load data flux, reject flags, chi2 ...
...
!  load each filter with the same PEAK-MJD value
...
!         VMODELFLUX(NOBS)     = R4EP_FIT(i,JEP_MODELFLUX)       ! Mar 2025
!         VMODELFLUX_ERR(NOBS) = R4EP_FIT(i,JEP_MODELFLUX_ERR)   ! Mar 2025
...
!      pack generated SIM fluxes (May 20 2016)
      CALL SNLCPAK_DATA(CCID, NOBS, VMJD, VTOBS,  & 
           VSIMFLUXOBS, VZERO,    &  ! VZERO = zero error for each true flux
           VFILTOBS, SNLCPAK_EPFLAG_FLUXSIM, LENCCID)
    ENDIF

! Jan 2014: store rest-frame filter and flux (mlcs,snoopy ...)
! Jan 2020: add KCOR and AVwarp
...
! fill output structure and write to disk
...
! Create strings and pass them to generic wrapper SNLCPAK_DISPLAYTEXT()
! 
...
! tack on optional FITMAGDIF result (May 2013)
...
! ---------------------------------
! now distance (MU and or PHOTOZ) and chi2
...
! glue the pieces together into one string.
    ...
    RETURN
  END SUBROUTINE SNLCPAK_DISPLAYTEXT_FIT
...
! routines need to create fitres table and NN table
...
! 
! ID    = table identifier
! IFLAG = IFLAG_INI or IFLAG_ANA
! 
! 
!     HISTORY
! July 19 2016: call TABLE_STRING_TERMINATION(+1/-1)
!               to protect strings for ROOT.
! 
! Apr 18 2022: pass ID and IFLAG args
! -----------------------------------------
...
! Created Feb 1, 2013
! 
! Initialize variables for snfitres-analysis table.
! Call generic function SNTABLE_ADDCOL() that loads a table
! for each selected format: CERNLIB, ROOT, HDF5 ...
! The VARLIST string is constructed so that it trivially
! works for ROOT; will need extra parsing (in SNTABLE_ADDCOL)
! for root and hdf5.
! 
...
!  when stretch and color are both fixed instead of floated.
! 
! Mar 02 2022: add zPRIOR and zPRIOR_ERR
! Jan 10 2025: add FILTLIST_FIT_USE
! --------------
...
! Jan 2025: add list of bands used in fit;
!   (40 should be enough for joint LSST+ROMAN+EUCLID)
...
! - - - - fit parameters and errors (skip BIDON values)
...
! ----------------------------------------------------
! check for additional IDEAL fit with FLUX = TRUEFLUX
...
! - - - - - -
! add Covariance status (Jan 2016) from MNSTAT call
!  MNSTAT_COV  = 2 -> was forced to be pos-definite
!              = 3 -> full accurate coc matrix
!  MNSTAT_COV  < 2 -> bad matrix and is rejected
...
! add other [non-PKMJD] covariances and include in optional text table.
! Make sure to skip the comma after the PKMJD-covariance list.
...
! - - - - - -   PHOTOZ - - - - -
    ...
    IF ( DOFIT_PHOTOZ ) THEN
      ...
      VARLIST = 'zPRIOR:F' // char(0)
      CALL SNTABLE_ADDCOL_flt(ID, CBLOCK, PRIOR_PHOTOZ, VARLIST,1,  LENBLOCK, LENLIST)
      VARLIST = 'zPRIORERR:F' // char(0)
      CALL SNTABLE_ADDCOL_flt(ID, CBLOCK, PRIOR_PHOTOZ_ERR, VARLIST,1,  LENBLOCK, LENLIST)

      VARLIST = 'ZPULL:F' // char(0)
      CALL SNTABLE_ADDCOL_flt(ID, CBLOCK, PHOTOZ_ERR_ZPULL,  VARLIST,0,  LENBLOCK, LENLIST)

      VARLIST = 'NEARDROP:I' // char(0)
      CALL SNTABLE_ADDCOL_int(ID, CBLOCK, IFILT_NEARDROP, VARLIST,0,  LENBLOCK, LENLIST)

      VARLIST = 'DZMIN1:F,DZMIN2:F' // char(0)
      CALL SNTABLE_ADDCOL_flt(ID, CBLOCK, PHOTODZ_MIN(1),  VARLIST,0,  LENBLOCK, LENLIST)

      VARLIST = 'DZ1ZMIN1:F,DZ1ZMIN2:F' // char(0)
      CALL SNTABLE_ADDCOL_flt(ID, CBLOCK, PHOTODZ1Z_MIN(1), VARLIST,0, LENBLOCK, LENLIST)
...
! - - - - - Zmax for 1/Vmax option
 ...
 404  CONTINUE
    ...
    RETURN
  END SUBROUTINE INIT_TABLE_SNFITVAR
...
! Created Mar 2018
! Return TRUE if this variable should be suppressed
! from table output.
! 
! Jan 2024: check x2
...
! Init table for epoch-dependent variables.
! (replaces CWNT_HBNAMES_RESID)
! 
! Feb 22 2016: always do the extra EP variables, even for TERSE format.
!              See EXTRA_EPVAR
! 
! Jul 22 2017: add BAND (character for IFILT_OBS)
! Jan 23 2020: add AVWARP for rest-frame models with k-corrections
! 
! ----------------------------------------------
...
! optionl rest-frame info for rest-frame model
...
22     format(I2.2)
    ...
    RETURN
  END SUBROUTINE TABLE_VARLIST_ERRFLAG

! =======================================================
    SUBROUTINE TABLE_VARLIST_COV(FLAGCOV,VARLIST,LENLIST,LENPKMJD)

! Created Sep 2012
! Return covariance VARLIST to pass to SNTABLE_ADDCOL().
! 
! Input:
!   FLAGCOV   = 1, 2 --> reduced, full covariance
! 
! Outputs:
!   VARLIST  = string to pass to SNTABLE_ADDCOL.
!   LENLIST  = length of entire varlist
!   LENPKMJD = length of sub-list with only the PKMJD elements.
! 
! In addition to return arguments, the following globals
! are also computed to help evaluate the COV arrays needed
! to fill the SNTABLE(s).
! 
...
!        ~~~~~~~~~~~
! 
! Apr 29 2013: RHO -> REDCOV and replace IPAR with PARNAME_STORE
!              e.g., RHO34 -> REDCOV_pkmjd_x1
! 
! May 13 2014:
!   - remove terms with BIDON and add FLAGPKMJD flag.
!   - overhaul using new keep, TABLEVARCOM.
! 
! Mar 19 2018: check SKIP_ZPHOT option
! 
! ------------------------------------
...
21    CONTINUE
    ...
    RETURN
  END SUBROUTINE TABLE_VARLIST_COV
...
! Created Jan 2025
! *** FIT DEBUG UTILITY ***
! Prepare table to dump FCN chi2 info per fit step.
! Beware that the output is huge, so use this only for a few events.
! 
! -------------------------------
...
! Mar21 2022: compute model mag for every band at every MJD
        DO ifilt = 1, NFILTDEF_SURVEY
...
! Created Nov 18 2016
! 
! Initialize variables to store SALT2 spectrum for each epoch.
! Store epoch variables as scalars; e.g., MJD, TOBS, FLUXCAL.
! Store spectrum as vector array: LAMLIST and FLUXLIST.
! 
! Initial use is that spectra are needed for chromatic
! corrections for each epoch (for DES).
! 
! Call generic function SNTABLE_ADDCOL() that loads a table
! for each selected format: CERNLIB, ROOT, HDF5 ...
! 
! Mar 21 2022: load all model mags for each obs.
! 
! --------------
...
! - - - -
    ...
    VARNAME = 'DETNUM:I' // char(0)
    CALL SNTABLE_ADDCOL_int(ID, CBLOCK, TBLSPEC_DETNUM, VARNAME, 1,  & 
                         LENBLOCK, 20 )
...
! - - - - -
! Mar 21 2022: write model mag for all bands at each obs
!   initial use is for DCR correction in DES.
...
! Apr 23 2014, R. Kessler
! for input string VARNAME, return IPAR such that
!    VARNAME = PARNAME_STORE(IPAR).
! If VARNAME has 'ERR' at the end, then return negative IPAR value
! 
! Nov 14 2014: fix bug to allow zPHOT as well as z.
...
!      check FITRESTMAG variables
    ...
    RETURN
  END FUNCTION IPAR_PARNAME_STORE
