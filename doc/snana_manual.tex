...
In the Hostlib you can define true, observed and error for up to four properties;
\begin{verbatim}
...
Items in the left columns are the BASENAME, and BASENAME alone will be interpreted as BASENAME\_TRUE.
If BASENAME\_ERR is specified, BASENAME\_OBS is computed as Gauss(BASENAME\_TRUE, BASENAME\_ERR).
Non gaussian errors can be simulated by specifing BASENAME\_OBS and not BASENAME\_ERR.
BASENAME\_ERR can be rescaled by using the sim-input key 
\begin{verbatim}                                                                                                                                             
HOSTLIB_SCALE_PROPERTY_ERR: 0.0(LOGMASS),0.0(LOGSFR) # set errors to zero
HOSTLIB_SCALE_PROPERTY_ERR: 0.8(LOGMASS),0.9(LOGSFR),0.5(LOGsSFR)  # scale each error differently
\end{verbatim}

These values are automatically propagated to the simulated data files, and aftyer light-curve fitting they are propagated to output tables (e.g. SNANA, FITRES).
% ------------------------------------                                                                                                                        
