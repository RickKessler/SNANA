...
\end{Verbatim}
%
It is recommended to use the defaults so that global updates
are easily propagated thru Github updates rather than modifying
numerous sim-input files.
The curvature is computing internally as $\Ok = 1 - \OM - \OL$,
and the radiation term is ignored. Time-varying $w$
...
\end{Verbatim}
For command line overrides and {\tt GENOPT} options for {\tt sim\_SNmix},
remove the colon. The parenetheses are properly handled by
{\tt sim\_SNmix}, but for command-line overrides quotes are needed,
e.g., 
\begin{Verbatim}
...
% ------------------------------------
%   \clearpage
   \subsection{Viewing Lightcurve Fits: {\tt plot\_snana.py} and 
       {\tt mkfitplots.pl} }
   \label{subsec:lcplot}
% ------------------------------------
...
The large {\tt GENSIGMA\_SALT2xxx} values specify flat
distributions that result in two $\delta$-functions per parameter.
...
\begin{itemize}
  ...
  \item the same ``{\tt varname\_pIa}'' logic is applied to variables
        ...
        include all classifier probababilities.
\end{itemize}
%
...
A python wrapper ({\tt cat\_snana\_table.py}) is also available with a 
...
\begin{itemize}
  ...
  \item SNLCPAK: used by {\tt mkfitplots.pl} (\S\ref{subsec:lcplot})
