...
Optioanl flux correlations can be defined using the {\tt REDCOV} 
keys as shown in Fig.~\ref{fig:fluxerrmap_2D}.
...
(e.g., systematic studies using {\tt sim\_SNmix}),
use the following command-line override,
\begin{Verbatim}
    snlc_sim.exe <inputFile> FLUXERRMODEL_REDCOV g:0.3,r:0.5,iz:0.5
\end{Verbatim}
...
\begin{itemize}
  \item {\tt HOSTLIB\_SPECBASIS\_FILE} points to a file with spectral basis
         ...
         {\tt specbasis01}, {\tt specbasis02}, etc ...
...
% ------------------------------------
\subsubsection{Synthetic Host Magnitudes ({\tt +HOSTMAGS})}
\label{sss:hostlib_synmag}
% ------------------------------------

Using {\tt HOSTLIB\_SPECBASIS\_FILE},
synthetic galaxy mags are computed with the command-line option
\begin{verbatim}
     snlc_sim.exe <iputFile> +HOSTMAGS
\end{verbatim}
A new \hostlib\ file is created with the {\tt [band]\_obs} columns appended,
...
This new \hostlib\ can be used to generate with galaxy mags to 
compute additional Poisson noise and to model analmolous 
flux-scatter as a function of local surface brightness.

% ------------------------------------
\subsubsection{Generate List of Host Neighbors ({\tt +HOSTNBR})}
\label{sss:hostlib_NBR}
% ------------------------------------

To simulate the effect of matching SN to the wrong galaxy with the
DLR method,
a list of true galaxy neighbors is needed for each \hostlib\ entry. 
For any \hostlib\ that includes sky coordinates, a supplemental 
column with a comma-separated
list of neighbors can be created with
%
\begin{verbatim}
   snlc_sim.exe <iputFile> +HOSTNBR
           or
   snlc_sim.exe <iputFile> +HOSTNBR   SEPNBR_MAX 10.0  NNBR_WRITE_MAX 10
\end{verbatim}
%
...
each neighbor galaxy, and ordering the galaxy matches by DLR, 
even of the smallest DLR is the wrong match.
{\it (TO DO: define tag for true match)}

To find the galaxy neighbors quickly when using the output \hostlib\ 
in a simulation,
the galaxy identifiers are \hostlib\ row numbers rather than {\tt GALID}.
...
Third option required min number of observations with true flux$>0$/
...
% --------------------------------------
  \subsubsection{{\tt DASHBOARD} Utility}
  \label{sss:sim_dashboard}
% --------------------------------------

There are well over a dozen possible input files to the simulation,
and keeping track can be difficult, especially with include files
and command-line overrides. A brief summary of every input file
can be displayed with
\begin{verbatim}
   snlc_sim.exe mysim.input DASHBOARD
\end{verbatim}
which will print a file summary and then quit. 
Higher-level pipelines can use this feature to better summarize the inputs.
...
Two examples are 1) low-z sample which has no meta-data 
to construct a \simlib\ file, and 2) simulating fake SN
that have already been overlaid on images.
...
so it is recommended to also set {\tt CUTWIN\_NFIELD = 1,1}.
The ``{\simlib}'' model feature in the simulation (\S\ref{subsec:MODEL_SIMLIB}) 
reads these mags.
