...
\end{verbatim}
Beware that increasinging the number of bins can result in significantly longer fit times, 
...
It is best to let the code internally compute {\tt NBINz} and {\tt RANGEz} for each event,
and manual overrides should be done only for testing. Also note that the redshift
grid increments in bins of $\Zphot/(1+\Zphot)$ instead of $\Zphot$, in order to 
search finer $\Zphot$ bins at lower redshifts.
If there is no host prior, then it is sensible to modify 
{\tt NBINz\_COURSE\_PHOTOZ} and {\tt RANGEz\_COURSE\_PHOTOZ}
and re-process a large sample. However, a host prior results in internally altering
these parameters for each event, and therefore overriding the defaults should
be done only for testing individual events and not for a large sample.
