...
The first method, called a ``cosmology-constrained photoZ fit,'' 
...
assuming a particular cosmology: $\mu = \mu(z,\OM,\OL,w_0,w_a)$
where $z$ is floated in the fit, and $\OM,\OL,w_0,w_a$ are
...
The second method, called a ``unconstrained photoZ'' fit,
...
on lightcurve-fit ouput where a distance-modulus prior is used with zero uncertainty.
...
results in a 4-parameter cosmology-constrained-photoZ fit.
...
If the host photo-$z$ is described by a zPDF unpacked from quantiles,
{\tt PRIOR\_ZERRSCALE<100} is simply a flag to use the host zPDF,
but the value is not applied to the zPDF.

To switch from a cosmology-constrained-photoZ fit to a 5-parameter
unconstrained-photoZ fit, set {\tt INISTP\_DLMAG} to any
non-zero value such as 0.1. The use (or non-use) of the
host-photoZ prior is controlled by the value of 
{\tt PRIOR\_ZERRSCALE}. {\tt PRIOR\_MUERRSCALE} controls the
...
\minuit\ stage. 
After using the recommended {\tt OPT\_SETPKMJD=20} (Fmax-clump)
...
If {\tt NBINt\_COURSE\_PHOTOZ>1} (see below), 
...
expecially if {\tt NBIN[par]\_COURSE\_PHOTOZ}
is increased for multiple parameters;
it is therefore good practice to run a CPU benchmark before submitting long jobs.
The ranges for color ({\tt RANGEc\_}) and stretch ({\tt RANGEs\_}) have literal definitions
for SALT2 (c,x1) or BAYESN (AV,theta). 
{\tt RANGEt\_} is w.r.t. the rough estimate determined by {\tt \&SNLCINP} input 
{\tt OPT\_SETPKMJD} in \S~\ref{subsec:SETPKMJD}.
{\tt RANGEz\_} defines a grid over $\Zphot/(1+\Zphot)$ instead of $\Zphot$, in order to 
...
If there is no host prior, {\tt NBINz\_COURSE\_PHOTOZ} and {\tt RANGEz\_COURSE\_PHOTOZ}
are the same for each event and a manual override is applicable to all events.
With a host prior, however, each event has a custom {\tt NBINz\_} and {\tt RANGEz\_}
...
UNDER DEVELOPMENT: The second option (MCMC) processes all 5 parameters simultaneously.
