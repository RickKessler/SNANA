...
{\tt GENMODEL\_OPTMASK} enable options described in \S\ref{sss:LCLIB_options}.
...
\begin{itemize}
  ...
  \item {\tt GENMODEL\_MSKOPT += 8}: 
     ...
     with RA,DEC in LCLIB, and also update MWEBV.
     {\it Beware} that incorrect cadences are used with this option!
     If {\tt DNDB} inputs are not sufficient, the correct strategy
     is to create a SIMLIB cadence library with the
     correct RA,DEC distribution.
\end{itemize}
