...
\newcommand{\crcov}{{\tt create\_covariance.py}}
...
\newcommand{\simeffsub}{\epsilon_{\rm detect}}  % May 7 2026
\newcommand{\simeffspec}{\epsilon_{\rm spec}}
\newcommand{\simeffzhost}{\epsilon_{\rm zHOST}}
...
\begin{verbatim}
  ...
  > $SNANA_DIR/src/snlc_fit.F90   # light curve fitting
  > $SNANA_DIR/src/psnid.F90      # psnid classifier
  > $SNANA_DIR/src/SALT2mu.c      # implement BBC to create hubble diagram
...
# ---------------------------------------------------------------------
* SNID                      required: integer or character ID
* NAME_TRANSIENT            optional: internal survey name
* NAME_IAUC                 optional: circular name
...
% #####################################################
\clearpage
\subsection{Tracking \faGithub ~ GitHub Tags in Analysis Outputs}
\label{subsec:tracking_git_tags}
% #####################################################

%\faGithub ~ GitHub
\newcommand{\git}{GitHub}

For reproducible analysis results,
it is ideally good practice to record a \git\ tag for a final analysis.
In practice, however, different components of an \snana-based analysis 
(e.g., {\tt sim, lcfit, bbc, create\_cov, wfit})
are run at different times and may have different commits, 
meaning that there may not be a single tag+commit reflecting an entire analysis.
While running a full end-to-end analysis with a single \git\ tag+commit
is recommended prior to publishing results, this single-tag 
reanalysis is usually forgotten. 

To enable reproducibility with analysis components using different \git\ tags,
all \snana\ codes write explicit \git\ tag+commit following an \verb|SNANA_VERSION|
key. This \verb|SNANA_VERSION| key is written into stdout, 
and to output files and tables. 
As an example, the Dovekie analysis outputs include
\begin{verbatim}
   SNANA_VERSION: v11_06a-62-g4c10cc4 
\end{verbatim}
...
# Optional keys:
  ...
  TEXPOSE(WIDE):   100 200 ...  # in FIELDs with WIDE substring; per band in FILTERS list
  TEXPOSE(DEEP):   500 600 ...  # in FIELDs with DEEP substring; per band in FILTERS list
\end{verbatim} 
%
...
{\tt TEXPOSE} is used only for the {\tt NONLINEARITY} option (\S??),
and the values are written to a {\tt TEXPOSE} column in PHOT section
of data files.
...
%
  \item {\bf IDEXPT:} arbitrary integer identifier for exposure/visit ID. Related options include:
     \begin{itemize}
       \item {\bf IDEXPT*NEXPOSE:} Exposure/Visit ID \& number of expsoures used to determine saturation
            (\S\ref{sssec:saturation}). Default {\bf NEXPOSE}=1.
       \item {\bf IDEXPT(DETNUM):} Exposure/Visit and detector/ccd number in (). \\
            Default {\bf DETNUM}$=-9$.
       \item {\bf IDEXPT*NEXPOSE(DETNUM):} Exposure/Visit \& Nexpose \& detector/ccd number.
       \item {\bf IDEXPT*NEXPOSE(DETNUM,XPIX,YPIX):} Exposure/Visit \& Nexpose \& detector/ccd number \&
            X,Y pixel coords.
     \end{itemize}
%
  \item {\bf BAND:} single character to specify a filter for this observation.
%
...
\begin{Verbatim}[frame=single]
  ...
  SIMLIB_SURVEY:  xyz     # override survey name in SIMLIB file; e.g., rename survey to be LOWZ
  ...
  SIMLIB_MJD_SHIFT x.y     # shift every MJD; e.g., to overlap range of real images
  ...
  USE_SIMLIB_TAKE_SPECTRUM: 1  # use TAKE_SPECTRUM keys in header 
  USE_SIMLIB_SPECTROGRAPH:  1  # use SPECTROGRAPH keys under each LIBID
  USE_SIMLIB_SALT2:         1  # use SALT2c & SALT2x1 keys in header
  USE_SIMLIB_GROUPID:       1  # pick HOST with RA,DEC near SIMLIB RA,DEC
  USE_SIMLIB_IDEXPT:        1  # write IDEXPT value to IMGNUM column in data file
  USE_SIMLIB_IMGNUM:        1  # same as above 
...
the ratio of calculated-to-true \unc, is included in the output
table for {\tt SNTABLE\_LIST='SNANA+EPOCH'}.
...
are written in fortran, and are stored in {\tt snana.F90}.
...
The simulation includes options to model 3 components of each survey 
search \eff:
(i) image subtraction pipelines characterized by \eff\ vs. S/N or mag,
(ii) \spec-confirmation \eff\ and 
(iii) for {\specy}-{\nospec} SNe, the ``{\tt zHOST}'' efficiency 
for obtaining an accurate host-galaxy redshift.
%
Some explicit examples of sim-input options are provided in the
subsections below.
To ensure correct settings, it is {\em essential} to always check that the
simulated distributions matches the data, and to check separately for the
\specy-confirmed and \nospec\ sub-samples.


Publicly released efficiency files are stored in the directory  
\verb|$SNDATA_ROOT/models/searcheff|,
and the file-name typically includes the name of the survey.

{\bf LEGACY WARNING:} as of \SNANA\ version \verb|v12_01e|,
the {\tt SEARCHEFF} efficiency code was refactored to significantly
expand capabilities, mainly to allow the spec-efficiency and
zHOST-efficiency maps to use the same set of variables.
For earlier \SNANA\ versions, check associated manual for limitations
with these maps.
...
\clearpage
\subsubsection{Spectroscopic-Confirmation Efficiency ($\simeffspec$) }
\label{sss:eff_spec}
% \bigskip  % \clearpage

The \spec-confirmation \eff\ ($\simeffspec$) cannot be rigorously computed
since it involves complex algorithms or human decision making during a survey.
The \snana\ simulation parametrizes $\simeffspec$ as an arbitrary
function of redshift, peak-mags, peak colors, \hostlib\ variables 
(including mags and colors), and closest time to peak.
...
grid of many variables.
An example of definining the $\simeffspec$ map is shown here,
\begin{verbatim}
  SEARCHEFF_SPEC_FILE:  $SNDATA_ROOT/models/searcheff/SEARCHEFF_SPEC_SDSS.DAT
  #     or
  SEARCHEFF_SPEC_FILE:  ZERO  # force EFF_SPECID=0 without map
  #     or
  SEARCHEFF_SPEC_FILE:  ONE   # force EFF_SPECID=1 without map
  #     or
  SEARCHEFF_SPEC_FILE:  NONE  # force EFF_SPECID=1 without map
 
  # --------------------
  #  !!!!! for systematics !!!!!
  #
  SEARCHEFF_SPEC_MAGSHIFT: 0.2   # shift all peak and host mags by 0.2
  #       or
  SEARCHEFF_SPEC_SHIFT(r): 0.2           # shift only r-band peak by 0.2
  #       or
  SEARCHEFF_SPEC_SHIFT(PEAKMAG_r): 0.2   # same as above

\end{verbatim}
% $
If \verb|SEARCHEFF_SPEC_FILE|  is not specified, then default $\simeffspec=1$.
For each simulated SN, $\simeffspec$ is determined from  multi-dimensional (linear) 
interpolation. 
If more than one grid map is given in the same $\simeffspec$-map file,
...
%
For parameter values outside the grid range, default $\simeffspec=0$;
extrapolation with nearest edge value is enabled with ``\verb|OPT_EXTRAP: 1|''
in the \verb|SEARCHEFF_SPEC_FILE|.
This out-of-range feature allows using different telescopes to cover
...
The second table describes the \eff\ for peak $r$-band magnitudes 
below 22, and the last table defines the \eff\ for
peak $i$ band magnitudes above 22. 
The second map depends on the absolute peak $r$-band magnitude
and the peak $g-r$ color at peak.
The 3rd map depends on the peak $i$-band magnitude and redshift,
and applies only for fields named '\verb|DEEP1|' and '\verb|DEEP2|'.
The 'FIELD:' option allows for field-dependent maps,
and the \verb|FIELD| key must appear before \verb|VARNAMES| key.
There essentially is no limit to the complexity: 
for example, one could define 
``\verb|VARNAMES: g  r  i  g-r  r-i  REDSHIFT SPECEFF|''
assuming that such a function can be determined.

% ----------------
\begin{figure}[hb] 
\begin{center}
\caption{Illustration of \spec\ \eff\ ($\simeffspec$) table format for the \snana\ simulation.
         The same table structure works for {\tt SEARCHEFF\_zHOST\_FILE},
         with {\tt SPECEFF} (key and column name) replaced with {\tt HOSTEFF}. }
\medskip
\label{fig:speceff}
\begin{Verbatim}[frame=single]
  OPT_EXTRAP: 0  # 0 -> EFF=0 outside grid
                 # 1 -> if outside grid, extrap EFF to nearest edge

# TABLE 1 -- ensure SPECEFF=1 at low redshift
  VARNAMES:  REDSHIFT  SPECEFF
  SPECEFF:    0.0      1.00
  SPECEFF:    0.1      1.00

# TABLE 2 -- 4m telescope
# VARNAMES:  PEAKMAG_g-PEAKMAG_r PEAKMAG_r  SPECEFF  # more verbose allowed 
...
# TABLE 3 -- 8m telescope
  FIELD:    DEEP1,DEEP2   # comma-sep or '+' sep list of fields; BEFORE VARNAMES
...
Fig.~\ref{fig:searcheff_varnames}. A color must be constructed
from either 2 peak mags (e.g., \verb|r-i|) or two host mags (e.g., \verb|g_obs-r_obs|).
While a mixed SN-HOST color (e.g., \verb|g-r_obs|) is not allowed,
peak and host mags can appear in different columns; e.g., \\
``\verb|VARNAMES: g-r  g_obs-r_obs SPECEFF|'' is allowed.
...
\caption{ Valid {\tt VARNAMES} to describe $\simeffspec$ in Fig.~\ref{fig:speceff}.
          Also valid for $\simeffzhost$ table in Fig.~??}
\label{fig:searcheff_varnames}
\begin{Verbatim}[frame=single]
  * g r i                # peak mags in any band
  * g-r                  # peak color
  * g-i                  # another peak color
  * PEAKMAG_[band]       # idem for any [band]; e.g. PEAKMAG_r is same as r
  * REDSHIFT             # redshift
  ...
  * SALT2mB, SALT2x1, SALT2c          # SALT2 mB, stretch, color
  * LOGMASS              # hsot galaxy LOGMASS
  * SNRSUM_REST_V        # synthetic rest-frame V band from spectrograph
  * [varname_hostlib]    # any variable from hostlib
  * HOSTMAG_[band]       # mag of host galaxy in [band]
  * HOSTMAG_g-HOSTMAG_r  # hosg g-r color
  * g_obs-r_obs          # same host g-r color
...
For multiple maps in one \verb|SEARCHEFF_SPEC_FILE| file,
...
but additional peakmag-dependent maps can be added to the same file.
...
\begin{Verbatim}[frame=single]
   ...
   SIM_SEARCHEFF_MASK += 8   # zSPEC from either spec-confirmation of zSPEC
\end{Verbatim}
%
Here are some examples to interpret this mask:
%
\begin{Verbatim}[frame=single]
   SIM_SEARCHEFF_MASK = 1    # detected by image-subtr;  spec-unconfirmed
   SIM_SEARCHEFF_MASK = 11   # detected by image-subtr & spec confirmed
   SIM_SEARCHEFF_MASK = 13   # detected by image-subtr, unconfirmed with zHOST
   SIM_SEARCHEFF_MASK = 15   # detected by image-subtr; spec-confirmed and zSPEC
\end{Verbatim}
%
...
The following sim-input key controls event selection:
%
\begin{Verbatim}[frame=single]
   APPLY_SEARCHEFF_OPT: 0  # keep all events (default)
   APPLY_SEARCHEFF_OPT: 1  # keep if software trigger passes
   APPLY_SEARCHEFF_OPT: 3  # keep if software trigger and SPEC-ID -> zSPEC(SN)
   APPLY_SEARCHEFF_OPT: 5  # keep if software trigger and zHOST
   APPLY_SEARCHEFF_OPT: 9  # keep if software trigger and zSPEC(SN or HOST)
\end{Verbatim}
%
When ``{\tt APPLY\_SEARCHEFF\_OPT: 3}'' is set,
then {\tt SIM\_SEARCHEFF\_MASK=11 (3+8)} for all saved SNe
because those that fail either of the search 
criteria (software trigger of specID map) are rejected. 
Setting  ``{\tt APPLY\_SEARCHEFF\_OPT: 1}'' results
in SNe with {\tt SIM\_SEARCHEFF\_MASK=1} or 11;
...
% --------------
%\clearpage
\subsubsection{\Nospec\ Efficiency for Host-Galaxy Spectroscopic Redshift ($\simeffzhost$) }
\label{sss:eff_zHOST}

For the unconfirmed SNe, i.e., those with \verb|SIM_SEARCHEFF_MASK=1|,
...
A redshift-dependent fraction of unconfirmed 
\spec\ (host-galaxy) redshifts ($\simeffzhost$) can be specified with
%
\begin{verbatim}
      ...
      # !!!! systematics !!!
      SEARCHEFF_zHOST_MAGSHIFT: 0.2    # change HOST and SN mags by 0.2
          or
      SEARCHEFF_zHOST_SHIFT(LOGMASS): 0.1  # shift LOGMASS in map
\end{verbatim}
%
where the file contains a $\simeffzhost$ map that is very similar
so the $\simeffspec$ maps in \ref{sss:eff_spec}, except that 
\verb|SPECEFF| keys and colums are replaced with \verb|HOSTEFF|.
Here is an example map that depends on 
\verb|FIELD|,  \verb|PEAKMJD|, and {\tt g} and {\tt i}-band host mags,
%
\begin{Verbatim}[frame=single]
# default is 0=>abort; 1=>use value at grid edge
OPT_EXTRAP: 1               

# decide if every event has FIELD corresponding to a map;
# Detault = 1 -> every event requires map covering FIELD
# option 0 -> if FIELD is not in a map, set EFF_zHOST=0
OPT_FIELDMATCH_REQUIRE:  1  
...
Fig.~\ref{fig:hostlib}, and also SN properties such as {\tt PEAKMAG}
and SALT2 parameters. See Fig.~\ref{fig:searcheff_varnames} for
complete list of allowed {\tt VARNAMES}.
Host color dependence can be specified with ``\verb|g_obs-r_obs|''
or equivalently, ''\verb|HOSTMAG_g-HOSTMAG_r|.''
The last ($N$th) {\tt VARNAME} must be {\tt HOSTEFF}.
...
When an accurate host-galaxy redshift is available from either 
\spec\ confirmation or the host,
the 4-bit is set with {\tt SIM\_SEARCHEFF\_MASK+=8}. 
There is a valid (i.e, positive) redshift when 
\verb|SIM_SEARCHEFF_MASK| has either the 2-bit or 4-bit set.
\verb|SIM_SEARCHEFF_MASK=1| means that the SN is
detected by the pipeline (\S\ref{sss:eff_pipeline})
but there is no valid redshift
(\verb|REDSHIFT_FINAL = -9|).
...
%
\begin{Verbatim}[frame=single]
   APPLY_SEARCHEFF_OPT: 3         # apply DETECTION+SPEC efficiencies
...
%
\begin{Verbatim}[frame=single]
   APPLY_SEARCHEFF_OPT: 1        # apply only DETECTION
\end{Verbatim}
Events with \verb|SIM_SEARCHEFF_MASK=3| or 5 have a \spec\ redshift
with \unc\ given by the \verb|GENSIGMA_REDSHIFT| key.
Events with \verb|SIM_SEARCHEFF_MASK=1| 
(unconfirmed and no host spec-z) have a host-galaxy photo-z
...
\begin{Verbatim}[frame=single]
   APPLY_SEARCHEFF_OPT: 5        # apply DETECTION & zHOST
   SEARCHEFF_zHOST_FILE:  SEARCHEFF_zHOST.DAT
\end{Verbatim}

{\bf zHOST VALIDATION:}
to validate the zHOST efficiency, include \verb|SIM_SEARCHEFF_MASK| in the
\verb|SIMGEN_DUMP| option (\S\ref{sss:simgen_dump}), 
and use command-line override  ``\verb|APPLY_SEARCHEFF_OPT 1|.'' 
The simulated zHOST efficiency is defined to be
\begin{verbatim}
                          N[SIM_SEARCHEFF_MASK=5]
    EFF(zHOST) = --------------------------------------------------- ,
                 N[SIM_SEARCHEFF_MASK=1] + N[SIM_SEARCHEFF_MASK=5] 
\end{verbatim}
and note that spectroscopically-confirmed events (\verb|SIM_SEARCHEFF_MASK=3|)
are ignored here.
...
\end{Verbatim}
%
The first five keys control the $R_V$ value, color law, optional color law params (PARLIST), 
...
#define OPT_MWCOLORLAW_SOMM25         225 // Sommovigo et al. (2025) LtU
#define OPT_MWCOLORLAW_SOMM26         226 // Sommovigo et al. (2026) LtU
...
An auxilary table of extinction mag vs. $R_V,\lambda$ for a few color law options
is created with sim-input command:
\begin{Verbatim}
   SIMGEN_DUMP_MWCL: 1
\end{Verbatim}
which creates {\tt [GENVERSION].MWCL} table file in the same directory as
the sim data files.
...
if {\tt APPLYFLAG\_MWEBV} is set, as was done for the PLASTICC 
...
filter.\footnote{See function {\tt GET\_MWXT8} in {\tt snana.F90.}}
...
The default option is 99 ({\tt FITZ99\_EXACT}(.
Also included are variants of the Fitzpatrick law 
...
The newer four-parameter attenuation curve from LtU \cite{Sommovigo2026} has been added.
This is based on a functional form derived through symbolic regression applied to
Illustris TNG \cite{Pillepich2018, Pillepich2019} galaxies with SKIRT radiative transfer modelling of the dust \cite{Camps2015, Camps2020}. This is the same input datasets as the previous LtU attenuation curve \cite{Sommovigo2025}, but an improved analysis. The model
has four constants $(c_1, c_2, c_3, c_4)$ measured at the population level, and four
free parameters $(B_1, B_2, B_3, B_4)$ that vary galaxy-by-galaxy. 
The `\texttt{GALextinct}' function reads $B0$ through $B3$ from the first four entries of 
\verb!PARLIST_MWCOLORLAW!, and will abort if these are not found.
...
# optional
   ...
   HOSTLIB_SNR_STORE:    xxx          # SNR>xxx to store HOSTLIB row (see below)
...
# debug options
   ...
   HOSTLIB_FIXRAN_WGT:     #.##       # 0-1: fix Sersic wgt 
   HOSTLIB_FIXRAN_RADIUS:  #.##       # 0-1: fix Sersic radius
   HOSTLIB_FIXRAN_PHI:     #.##       # 0-1: fix azimuthal PHI angle 
...
\end{Verbatim}
Only the \verb|HOSTLIB_FILE| key is required, while the
...
a \hostlib. The optional \verb|WGTMAP_FILE| overrides the 
default weight map embedded in the
\hostlib\ file. 
\verb|HOSTLIB_ZPHOTEFF_FILE| specifies a 2-column file with
...
\verb|HOSTLIB_STOREPAR| is a comma-separated
...
\verb|HOSTLIB| variables defined in 
\verb|SEARCHEFF_zHOST_FILE|,
\verb|GENPDF_FILE|, and
\verb|WGTMAP|
are automatically included in the \verb|HOSTLIB_STOREPAR| list.


\verb|HOSTLIB_SNR_DETECT| works only if both mags and mag-uncertainties
are defined in the \hostlib\ as \verb|[band]_obs| and \verb|[band]_obs_err|.
Setting ``\verb|HOSTLIB_SNR_DETECT: 5|'' requires at least one band with
...
passes SNR cut.  \verb|HOSTLIB_MAG_DETECT| works the same way,
except the cut values are based on mag instead of SNR;
e.g. ``\verb|HOSTLIB_MAG_DETECT: 28,28,27|'' requires 2 bands with mag brighter
than 28 and a 3rd band brighter than 27.

\verb|HOSTLIB_SNR_STORE| selects/rejects HOSTLIB rows to store for transient matching.
While galaxies failing \verb|HOSTLIB_SNR_DETECT| can be matched
to a transient (resulting in host-less event), 
galaxies failing \verb|HOSTLIB_SNR_STORE| are excluded from 
matching to a transient, as if they were not in the {\tt HOSTLIB}.

\verb|HOSTLIB_MINDAYSEP_SAMEGAL| specifies the minimum
...
There are two ways to specify photometric redshifts:
(1) \hostlib\ described here, or (2) define a \verb|HEADER_OVERRIDE| file 
as described in \S~\ref{subsec:data_override}.
The $\Zphot$ values are not used in the simulation for calculations,
but are simply written to the data files for analysis.
There are two ways to provide photo-z information into the \hostlib.
The first method is to provide {\tt ZPHOT} and {\tt ZPHOTERR} columns.
The second method is to provide quantile redshift columns 
corresponding to integrated (CDF) probabilities. The \hostlib\ columns are
\begin{verbatim}
    QZPHOT00 QZPHOT01 QZPHOT02 ... QZPHOTnn  # nn+1 quantiles

    #or legacy columns fixed to 11 quantiles:
    ZPHOT_Q000  ZPHOT_Q010   ZPHOT_Q020   ZPHOT_Q030 ... ZPHOT_Q100
\end{verbatim}
One or both methods may be provided in the same \hostlib.
The quantile percentages are internally computed with a percent grid-size 
of 100/nn; for nn=10, the percent grid size is 10\%.
The corresponding output data variables are {\tt HOSTGALz} vectors
%
\begin{verbatim} 
    HOSTGALz_QUANTILE_NZ
    HOSTGALz_QUANTILE_ZPHOT
    HOSTGALz_QUANTILE_PERCENT
\end{verbatim}
%
where \verb|HOSTGALz_QUANTILE_NZ = nn+1|, and is internally determined as the
number of {\tt QZPHOTnn} columns.

To select photo-$z$ in the light curve fitting program,
set {\&FITINP} namelist variable \verb|OPT_PHOTOZ|
as described in \S\ref{subsec:photoz}. 
...
\begin{verbatim}
   ...
   snlc_sim.exe <inputFile> +HOSTNBR   SEPNBR_MAX 10.0  NNBR_WRITE_MAX 10
          or 
   snlc_sim.exe  NOFILE  HOSTLIB_FILE <hostlib>  +HOSTNBR  SEPNBR_MAX 10.0 
\end{verbatim}
%
For the first two command options, the sim-input file must include a {\tt HOSTLIB\_FILE} key,
but no other sim-input keys are required. In the 3rd option, the {\tt NOFILE} arg 
skips the input file and only uses command line args where \verb|HOSTLIB_FILE|
must be is specified.
The optional command-line arguments are 
1) \verb|SEPNBR_MAX|, the maximum angular separation between galaxies
  (default: $10^{\prime\prime}$), and
2) \verb|NNBR_WRITE_MAX|, the max number of neighbors to include
  (default: 10).
...
The output \verb|[HOSTLIB]+HOSTNBR| file contains an additional 
\verb|NBR_LIST| column that is a comma-separated list of row numbers
for neighbors (e.g., 44,43324,878), or $-1$ if there are no neighbors.
%
Row numbers start at 1 (not 0), and to allow for easy verification
the {\tt +HOSTNBR} option includes a screen-dump for several entries, 
where the dump includes the row number and {\tt GALID}.
%
\verb|NBR_LIST| is read as a string, and internally split into integer
row numbers. The reason for storing file row numbers, instead of {\tt GALID}s,
is to enable a fast instant lookup in the internal {\tt HOSTLIB} table, 
rather than a slow event-by-event search for {\tt GALID}s.
{\it BEWARE not to alter \verb|[HOSTLIB]+HOSTNBR| (i.e. do not remove or add rows),
otherwise the \verb|NBR_LIST| row numbers become invalid and the sim
will likely abort on a crazy-large SN-galaxy separation.}
To make modifications (i.e., remove pathological {\tt GALID} rows),
modify the original {\tt HOSTLIB} and re-run the {\tt +HOSTNBR} command above
to recreate the \verb|[HOSTLIB]+HOSTNBR| file.


If Sersic profiles are included in the original \hostlib, 
running the simulation again with the 
newly-created \verb|[HOSTLIB]+HOSTNBR| file
results in computing DLR for the true galaxy and
for each neighbor galaxy; 
the galaxy matches in the output data files are ordered by
{\tt DDLR = DLR/Angsep}, 
even if the smallest {\tt DDLR} is the wrong match.


\verb|SIM_HOSTLIB_GALID| is the true galaxy ID, and is stored in 
output data files and the output tables created by analysis codes.
An incorrect host-match is flagged by
\verb|SIM_HOSTLIB_GALID|$\ne$\verb|HOSTGAL_OBJID|;
in this case, it is likely that
\verb|SIM_HOSTLIB_GALID = HOSTGAL2_OBJID|.
To reject host-matches with a {\tt DDLR} cut
(\S\ref{sss:hostlib_options}),
set sim-input key ``\verb|HOSTLIB_MAXDDLR: <cutval>|''  
where default cut is 4.
For analysis, host information can be added to TEXT-formatted
output tables (\S\ref{subsec:anavar}) using 
\begin{verbatim}
...
% ------------------------------------
\subsubsection{Select weighted subset from {\tt HOSTLIB} ({\tt +HOSTSELECT}) }
\label{sss:hostlib_select}
% ------------------------------------

To generating host galaxies for image[sim] overlays, it may be convenient to pre-select
hosts using the command
%
\begin{verbatim}                                   
   snlc_sim.exe NOFILE  +HOSTAPPEND \
       MAX_HOSTLIB_SELECT 100000   \
       HOSTLIB_FILE <hostlib>      \
       HOSTLIB_WGTMAP_FILE <wgtmap_file>
\end{verbatim} 
%
\verb|MAX_HOSTLIB_SELECT| has default value of 1 million, and can be increased or decreased.
With  \verb|NOFILE| argument, \verb|HOSTLIB_FILE| is required and
\verb|HOSTLIB_WGTMAP_FILE| is optional.  Galaxies are selected with \verb|WGT>RAN|
where \verb|WGT| is computed from the host properties and \verb|WGTMAP| file, and
\verb|RAN| is a random number between 0 and 1. If a \verb|WGTMAP| file is not
provided, every event is selected and there is no reason to use this option
except to select the first \verb|MAX_HOSTLIB_SELECT| galaxies.

The output \verb|HOSTLIB| (with \verb|+SELECT| extension in file name) includes
an extra column \verb|WGT_SELECT|. This output is recommended only to define placement
of events on images, and is NOT recommended to generated to generate SNANA sims for science.
...
\begin{itemize}
  \item \verb|HOSTLIB_FIXRAN_WGT| fixes the random number ($0<r<1$)
    to pick the wgt for multi-Sersic profiles. This option has no
    impact for single-Sersic description.
%
  \item \verb|HOSTLIB_FIXRAN_RADIUS| fixes the random number ($0<r<1$)
         where the reduced radius contains a fraction  
         `$r$' of the total flux.
%
  \item \verb|HOSTLIB_FIXRAN_PHI| fixes random number ($0<r<1$) 
        ...
        ($90^{\circ},270^{\circ}$).
%
 \item \verb|HOSTLIB_FIXSERSIC| fixes the Sersic parameters
...
{\tt SIMLIB\_DUMP} feature (\S\ref{sss:simlib_dump}).
...
\subsubsection{Disable {\tt TAKE\_SPECTRUM} keys}
\label{sss:disable_take_spectrum}

Disable {\tt TAKE\_SPECTRUM} keys without commenting them out or removing them from the 
sim-input file;
\vspace{-0.4cm}
\begin{verbatim}
...
\begin{verbatim}
   GENRANGE_PEAKMJD:          <MJDmin>  <MJDmax>   # required
   GENRANGE_PEAKMJD_EXCLUDE:  <MJDmin>  <MJDmax>   # optional
\end{verbatim}
specifies an MJD window to randomly generate {\tt PEAKMJD},
along with an optional window to exclude.
...
to populate with adequate statistics for a reliable bias correction.
...
% ------------------------------------
  \clearpage
   \subsection{Simulated Output Formats with {\tt FORMAT\_MASK} }
...
\begin{verbatim}
  $SNDATA_ROOT/SIM/[GENVERSION]    # default
     or
  [PATH_SNDATA_SIM]/[GENVERSION]   # optional output path with PATH_SNDATA_SIM key
\end{verbatim}
% $
For FITS format, two sets of FITS files are created: 
...
along with meta-data (sky noise, PSF, zeropoint ...).
Pointers in the HEADER file are used to quickly extract the
appropriate light curve from the PHOT file, without doing a full
search for each PHOT row.
...
multiple format options can be included with a single integer.
...
\begin{Verbatim}[frame=single]
   ...
   FOMAT_MASK:   10   # 2(TEXT)  + 8(no SIM_XXX output, looks like real data)
   FOMAT_MASK:   40   # 32(FITS) + 8(no SIM_XXX output, looks like real data)
\end{Verbatim}
%
and the sub-sections below give more details.
Note that the 2-bit (TEXT) or 32-bit (FITS) must be included for all options,
except for the compact feature (64-bit) that automatically enables FITS format.
...
uses the binary-FITS format that is processed using the 
{\tt cfitsio} library. 
...
a set of {\tt HEAD.FITS} files with a one-row summary for each SN.
...
file; to reduce memory for reading, the FITS files are not merged.
For example, 40 1GB FITS files requires only 1GB of memory to read,
while merging into one file would require 40 GB memory.

The {\tt HEAD} and {\tt PHOT} table columns are shown in \S~\ref{dataFiles}.
...
``{\tt\bf FORMAT\_MASK:  2}'' 
This  option results one outut TEXT file per event.
These files are human-readable, and is thus recommended for testing with small interactive jobs.
For sims created in FITS format, a small number of light curves can be extracted
into TEXT format with command:
\begin{verbatim}
   quick_commands.py -v [GENVERSION] -cidlist_text [cid0],[cid1], ...
     or
   quick_commands.py -v [GENVERSION] --cidlist_text TEN   # extract first 10 events
\end{verbatim}
...
% ------------------------------------
``{\tt\bf FORMAT\_MASK: 10 or 40 (+8 bit)}'' 
...
% ------------------------------------
``{\tt\bf FORMAT\_MASK: 18 or 48 (+16 bit)}'' 
...
Here is an overview of the simulation table-DUMP options that
produce text-formatted table summaries which are easily read with
python. {\tt [GENV]} is the {\tt GENVERSION} argument,
and is the prefix for all table-dump files names that are
stored in the sim-data folder along with other auxiliary files
({\tt [GENV].LIST} and {\tt [GENV].README}).
%
\begin{Verbatim}[frame=single]
  table file               sim-input
  name                     key: value               comment
  ----------------------------------------------------------------------------
  Optional:
  [GENV].DUMP       SIMGEN_DUMP: [below]      user specifies variable list
  [GENV].NOISE      SIMGEN_DUMP_NOISE: 1      noise per obs: sky,galaxy,source
  [GENV].TRAINSALT  SIMGEN_DUMP_TRAINSALT: 1  PEAKMJD table for saltshaker
  [GENV].MWCL       SIMGEN_DUMP_MWCL: 1       Color law vs. lam and RV
  [GENV].RATE       SIMGEN_DUMP_RATE: 1       z & vol-Rate in .01 z bins
  SIMLIB_DUMP_*     SIMLIB_DUMP 1             command line arg; sim quits after dump.

  Auto-generated:
  [GENV].SL          ---                      Strong Lensing
  [GENV].DCR         ---                      Differential Chromatic Correction
  [GENV].SPEC        ---                      TAKE_SPECTRUM 
\end{Verbatim}
%
The most commonly used table-DUMP is {\tt SIMGEN\_DUMP} that produces
a one-row-per-SN summary. This is the only table that requires
a user-input list of variable names (\S~\ref{sss:simgen_dump});
the other tables produce standard columns, and column changes
require a code update. 
{\tt SIMLIB\_DUMP} (\S~\ref{sss:simlib_dump}) is unique because
the key is entered on the command line, and it quits after the dump,
and thus is not used with nominal sim generation.

% ------------------------------------
   \clearpage
   \subsubsection{{\tt SIMLIB\_DUMP} Utility }
   \label{sss:simlib_dump}
% ------------------------------------
...
{\tt SIMLIB\_DUMP\_AVG-[simlib]}. To obtain both the LIBID and MJD
...
%{\tt SIMLIB\_DUMP} option (\S~\ref{sss:simlib_dump}). The FoM is 
...
\begin{enumerate} 
 \item {\bf MINIMIZATION} based on CERNLIB's 
   ...
   options below. For very high-SNR events the first fit-iteration
   can fall into a false minimum, leading to pathological
   fits on successive iterations. This problem is
   alleviated with {\tt \&FITINP} namelist variable
   \verb|FUDGEALL_ITER1_MAXFRAC| (set to few percent);
...
%
 \item {\bf MARGINALIZATION} using multi-dimensional integration
   over the P.D.F; see {\tt snana.F90} subroutine \verb|MARG_DRIVER|.
   {\tt \&FITINP} namelist parameters:
   \begin{enumerate}[noitemsep]
     \item \verb|NGRID_PDF|:  (required) number of integration bins per fit-parameter (11 recommended).
     \item \verb|NSIGMA_PDF|: (required) integration range is $\pm$\verb|NSIGMA_PDF| for each parameter.
     \item \verb|VARLIST_PDF|: (optional) subset of variables to reduce CPU time 
            (e.g., {\tt 'c,x0,zPHOT'}).
     \item \verb|PRIOR_SHAPE_RANGE|: (optional) flat prior on shape/stretch parameter
               (e.g., x1) with Gaussian roll-off at edges ($\sigma=0.1$)
               $\to$ sets PDF=0 outside defined range of stretch.
     \item \verb|PRIOR_COLOR_RANGE|: (optional) flat prior on color par
               (Gauss roll-off with $\sigma=0.01$).
   \end{enumerate}
   The minimizer must run first (\verb|NFIT_ITERATION=2 or 3|)
...
%
\begin{itemize}  
  \item \verb|PRIOR_MJDSIG|:
    sigma on Gaussian prior for MJD at peak brightness.
    Default is 20 days.
%
  \item \verb|PRIOR_SHAPE_RANGE(2)|  and \verb|PRIOR_SHAPE_SIGMA|: 
    range of flat prior for shape parameter to prevent crazy values,
    and Gauss roll-off. Default range is \{$-9,+9$\} and $\sigma=0.1$.
%
  \item \verb|PRIOR_COLOR_RANGE(2)|  and \verb|PRIOR_COLOR_SIGMA|: 
    range of flat prior for color parameter to prevent crazy values,
    and Gauss roll-off. Default range is \{$-9,+9$\} and $\sigma=0.01$.
%
  \item \verb|PRIOR_DMU_RANGE(2)|  and \verb|PRIOR_DMU_SIGMA|: 
    range of flat prior for $\mu-\mu_{\rm theory}$, primarily intended for photo-$z$ fits,
    and Gauss roll-off. Default range is \{$-9,+9$\} and $\sigma=0.1$.
    Since $\mu$ is not a fit parameter for SALT2 model,
    $\mu \equiv m_B + \alpha x1 - \beta c$ with  $\alpha=0.14$ and $\beta=3.2$.
%
  \item \verb|PRIOR_DELTA_PROFILE(4)|:
    ...
    for all $\Delta$ values within \verb|PRIOR_SHAPE_RANGE|.
...
\begin{Verbatim}[frame=single] 
   ...
   ERRFLAG_FIT >  0    -> see error codes with
                      grep  'ERRFLAG_'  \$SNANA_DIR/src/snlc_fit.F90 | grep '&'
\end{Verbatim}
%
...
and has a valid fit whose results are stored in the {\tt SNANA} and {\tt FITRES} table files.
...
\newcommand{\lcfitz}{{\tt LCFITz}}
\newcommand{\ZphotHost}{z_{\rm phot,Host}}

Here we describe light curve fits that determine the
SN~Ia redshift ($z$) from photometry, called ``\lcfitz'' fits.
There are two fundamental methods to perform \lcfitz.
The first method, called a ``cosmology-constrained \lcfitz,'' 
...
\lcfitz\ for \mlcs\ has 4 floated parameters: 
\{$\Zphot,t_0,\Delta,A_V$\}.
\lcfitz\ for \SALTII\ also has 4 floated parameters:
\{$\Zphot,t_0,x_1,c$\}.
%
The distance variable ($\mu$ or $x_0$) is constrained (calculated)
assuming a particular cosmology: $\mu = \mu(\Zphot,\OM,\OL,w_0,w_a)$
where $\Zphot$ is floated in the fit, and $\OM,\OL,w_0,w_a$ are
set by the user.  The cosmology can be specified
with \&SNLCINP namelist parameters
{\tt OMAT\_REF}, {\tt OLAM\_REF}, {\tt W0\_REF}, {\tt WA\_REF}.
For \SALTII\ \lcfitz, the distance modulus is converted
into the $x_0$ parameter, and therefore 
{\tt SALT2alpha} \& {\tt SALT2beta} must be specified
as \&FITINP namelist parameters.


The second method, ``unconstrained \lcfitz,''
floats 5 parameters:
\{$\mu,\Zphot,t_0,\Delta,A_V$\} for \mlcs, and
\{$x_0,\Zphot,t_0,x_1,c$\} for \SALTII.
...
variations that involve using the host-galaxy photo-$z$
($\ZphotHost$) as a prior to help constrain the redshift.
A distance-modulus prior can be applied to the second
method (5-parameter \lcfitz); this is essentially a
constrained-\lcfitz, but the $\Zphot$ error includes uncertainty 
from the cosmological parameters.
Needless to say, {\it never} run a cosmology fit on lightcurve-fit ouputs 
for which a distance-modulus prior is used with zero uncertainty.

There are five {\tt \&FITINP} namelist parameters that
control \lcfitz. The default values are set so
that \lcfitz\ is turned off,
%
\begin{Verbatim}[frame=single]
   DOFIT_PHOTOZ     = F
...
\end{Verbatim}
%
Setting \verb|DOFIT_PHOTOZ=T| and \verb|INISTP_DLMAG=0.0|
results in a 4-parameter cosmology-constrained \lcfitz.
Since \verb|PRIOR_ZERRSCALE=100.0| by default, the $\ZphotHost$
uncertainty is multiplied by 100 and therefore has no impact on \lcfitz.
Setting \verb|PRIOR_ZERRSCALE=1.0| results in using the $\ZphotHost$
prior described by a Gaussian distribution.
If $\ZphotHost$ is described by a zPDF unpacked from quantiles,
\verb|PRIOR_ZERRSCALE<100| is simply a flag to use the host zPDF,
but the value is not applied to the zPDF.

To switch from a cosmology-constrained \lcfitz\ to a 5-parameter
unconstrained \lcfitz, set \verb|INISTP_DLMAG| to any
non-zero value such as 0.1. The use (or non-use) of the
$\ZphotHost$ prior is controlled by the value of 
\verb|PRIOR_ZERRSCALE|. \verb|PRIOR_MUERRSCALE| controls the
cosmology prior as described in  \S~\ref{sss:zphot_cosmology_prior}.
The parameter \verb|OPT_PHOTOZ| controls the source of
...
even if spec-$z$ are available. For quantiles, the mean and stddev are used as Gauss prior.
...
photo-$z$ redshifts.
%
{\tt OPT\_PHOTOZ=4} uses zPDF from quantiles, and ignores accurate spec-$z$.
...
\bigskip
A few other \lcfitz\  issues are:
\begin{itemize}
  \item	To test \lcfitz\ methods in simulated SN~Ia samples,
	the simulation includes an option to include $\ZphotHost$
	from on an externally-supplied library 
	(\S~\ref{subsec:hostlib}). To test SN-only \lcfitz\
	(without host), increase {\tt GENSIGMA\_REDSHIFT}
	so that the initial redshift estimate is poor.
%
\item	To test \lcfitz\ sensitivity to the initial redshift estimate,
...
\begin{center}
\caption{
  Examples of setting \lcfitz\ options within the
  {\tt \&FITINP} namelist.
 }
\label{fig:photoZ}
%
\begin{Verbatim}[frame=single]
 ! constrained LCFITz, ignore host-galaxy photo-z:
   DOFIT_PHOTOZ    = T
 ...
 ! constrained LCFITz fit using host-galaxy photo-z:
   DOFIT_PHOTOZ    = T
   PRIOR_ZERRSCALE = 1.0
   INISTP_DLMAG    = 0.0    ! fix MU = MU(zphot,cosmology)

 ! constrained LCFITz, host-galaxy photo-z errors inflated by 1.3
   DOFIT_PHOTOZ    = T
   PRIOR_ZERRSCALE = 1.3
   INISTP_DLMAG    = 0.0    ! fix MU = MU(zphot,cosmology)

 ! cosmology LCFITz, ignore host-galaxy photo-z:
   DOFIT_PHOTOZ    = T
   PRIOR_ZERRSCALE = 100.0  ! inflate error on photo-z prior
   INISTP_DLMAG    = 0.1    ! float DLMAG

 ! cosmology LCFITz using host-galaxy photo-z:
   DOFIT_PHOTOZ    = T
   PRIOR_ZERRSCALE = 1.0   
   INISTP_DLMAG    = 0.1    ! float DLMAG

 ! cosmology LCFITz with priors on both distance & host-galaxy photo-z:
   DOFIT_PHOTOZ     = T
   PRIOR_ZERRSCALE  = 1.0   ! use host-galaxy photo-z errors   
...
% ---------------------------------------
   \clearpage
   \subsubsection{Redshift-Dependent Selection in \lcfitz\ }
...
\lcfitz\ has finished, yet it is useful
...
For \lcfitz, however, it is not clear which filters \& epochs
are valid until the fit has finished. 
For example, consider \lcfitz\ using  $gri$ bands and \SALTII model: 
if $\Zphot < 0.072$, $i$-band maps to rest-frame wavelengths 
greater than the 7000~\AA\ cutoff in \SALTII. Including $i$-band
in \lcfitz\ results in using an unphysical region of the model,
...
Another example is in DES \& LSST, where $g$-band maps below 3000~\AA\
at redshifts above about 0.5.


To make initial $\Trest$-dependent cuts before \lcfitz\
has started, the cuts are loosened by a factor of
...
for any redshift in the range specified by \verb|PHOTOZ_BOUND|.
For example, consider \verb|PHOTOZ_BOUND = 0.1| and 
and a requirement that the min-$\Trest$ is before $-4$ days;
the initial cut would be a requirement of an epoch before
$-2$~days using whatever \verb|REDSHIFT_FINAL| is in the 
...
to 15-120 days before \lcfitz\ starts.
...
\lcfitz\ with all filters except for those in 
...
$\Zphot$ is then used to determine which filters to exclude
...
As an example, consider \lcfitz\ with $griz$ filters.
For $z>0.49$, $g$-band should be excluded. As a safety margin,
one might exclude $g$-band when the 1st-iteration $\Zphot$ value
...
\end{Verbatim}
The default \verb|PHOTOZ_ITER1_LAMRANGE| cut is set to exclude
...
can be added back after the first fit-iteration if the $\Zphot$
value is low enough.  \verb|PHOTOZ_BOUND| is a hard {\sc minuit}
bound to prevent crazy excursions during the minimization,
and is also used to loosen the $\Trest$-related cuts
before \lcfitz\ has started.
...
The $g$-band is excluded if the 1st-iteration satisfies
...
or $\Zphot < 0.11$.
Note that \verb|PHOTODZ_REJECT| is defined to be positive when adding a 
safety margin, regardless of whether a blue or red band is being tested.
Setting  \verb|PHOTODZ_REJECT| to a large negative value (i.e, $-99$)
disables the cut.
Finally, the \verb|PHOTODZ_REJECT| cut is applied to all observer-frame
...
five variables are include in the {\tt FITRES} table file:
\begin{itemize}
...
but was rejected based on its $\Zphot$ value from the
...
\newcommand{\fluxScale}{S_{F}}


For SN-only \lcfitz, \minuit\ is likely to find a
local minimum, rather than the global minimum. It is therefore
important to provide initial parameter estimates before the
\minuit\ stage. 
After using the recommended \verb|OPT_SETPKMJD=20| (Fmax-clump)
to estimate {\tt PEAKMJD},
the default opton performs a 3D course grid-search over 
redshift (0.04 bin-width), color (4 bins) and stretch/shape (3 bins).
If \verb|NBINt_COURSE_PHOTOZ>1| (see below), 
the 3D grid is expanded to a 4D grid search that includes t0={\tt PEAKMJD}.
The distance parameter is estimated analytically as follows.
For each grid point ($\Zphot$, color, shape) the model fluxes
...
Next we define a flux-scale,
  $\fluxScale = x_0/\bar{x_0} = 10^{0.4(\mu-\bar{\mu})}$.
%
The chi-squared is
 $\chi^2 = \sum_i [\Fdata - \fluxScale\Fmodel]^2/\sigma_i^2$,
where $\sigma_i$ is the quadrature sum of data and model errors
and '$i$' is the epoch index.\footnote{See {\tt R4SN\_FFSUM\_XXX}
   variables in {\tt snlc\_fit.F90}.}
Analytical minimization is from setting $\partial \chi^2/\partial\fluxScale = 0$, 
which results in
%
\begin{equation}
  \fluxScale
...
The four initial values ($\Zphot$, color, stretch, distance)
are set based on the minimum $\chi^2$ from the 3D loop over \{redshift,color,stretch\}. 
A correction to the $\fluxScale$ formula (Eq.~\ref{eq:fluxScale})
is needed when there is a cosmology prior
(Eq.~\ref{eq:chi2_muprior} in \S~\ref{sss:zphot_cosmology_prior})
but this has not yet been implemented.
After the course-grid search, a 1D {\tt PEAKMJD} grid is searched in 0.5~day bins ($-2$ to $+2$ days).

The number of bins and range per variable have default values,
and the default values can be altered with these inputs:
\begin{verbatim}
  &FITINP
    NBINc_COURSE_PHOTOZ = nnn ! default=4  
    NBINs_COURSE_PHOTOZ = nnn ! default=3  
    NBINt_COURSE_PHOTOZ = nnn ! default=1 (fixed at PEAKMJD from OPT_SETPKMJD)
    NBINz_COURSE_PHOTOZ = nnn ! default=43*[zph_max/(1+zph_max) - zph_min/(1_zph_min)]
    SCALE_NBIN_COURSE_PHOTOZ = nnn  ! scale number of bins for each param (default=1)

    RANGEc_COURSE_PHOTOZ = x,y ! default = -0.2, 0.25
    RANGEs_COURSE_PHOTOZ = x,y ! default = -3.0, 1.0
    RANGEt_COURSE_PHOTOZ = x,y ! default = -15, 15 (only if NBINt_COURSE_PHOTOZ>1)
    RANGEz_COURSE_PHOTOZ = x,y ! default = -9,-9 -> computed each event using prior
\end{verbatim}
Beware that increasing the number of bins can result in significantly longer fit times, 
expecially if \verb|NBIN[par]_COURSE_PHOTOZ|
is increased for multiple parameters;
it is therefore good practice to run a CPU benchmark before submitting long jobs.
The ranges for color (\verb|RANGEc_|) and stretch (\verb|RANGEs_|) have literal definitions
for SALT2 (c,x1) or BAYESN (AV,theta). 
\verb|RANGEt_| is w.r.t. the rough estimate determined by {\tt \&SNLCINP} input 
\verb|OPT_SETPKMJD| in \S~\ref{subsec:SETPKMJD}.
\verb|RANGEz_| defines a grid over $\Zphot/(1+\Zphot)$ instead of $\Zphot$, in order to 
search finer $\Zphot$ bins at lower redshifts.

While optimizaing $\Zphot$ performance with different {\tt NBIN} and {\tt RANGE} parameters 
is encouraged,
it is best to let the code internally compute {\tt NBINz\_} and {\tt RANGEz\_} for each event
when a host redshift prior is used.
If there is no host prior, \verb|NBINz_COURSE_PHOTOZ| and \verb|RANGEz_COURSE_PHOTOZ|
are the same for each event and a manual override is applicable to all events.
With a host prior, however, each event has a custom \verb|NBINz_| and \verb|RANGEz_|
based on the event-specific host prior value;
therefore a manual override that is fixed for each event makes no sense and
should be done only for testing individual events.
To see the actual bins used, grep (or search) ``binsize'' from stdout and 
visually examine output that looks like
%
\begin{verbatim}
         7      peakmjd  bins from 56972.31 to 57002.31   (binsize=   5.00)
        23  zPH/(1+zPH)  bins from    0.010 to    0.545   (binsize=  0.024)
         4      color    bins from   -0.200 to    0.250   (binsize=  0.150)
         3      shape    bins from   -3.000 to    1.000   (binsize=  2.000) 
\end{verbatim}


Cheater option: \verb|OPT_PHOTOZ=16|sets initial $\Zphot$
to spectroscopic redshift and skips the redshift grid-search. 
...
\begin{verbatim}
   &FITINP
      UVLAM_EXTRAPFLUX    = 1000          
      RESTLAMBDA_FITRANGE = 2000, 13000 
\end{verbatim}
In this example, the model flux at the bluest wavelength (at each phase)
is linearly extrapolated to zero flux at $\lam=1000$~\AA.
For the initial parameter estimates (\S\ref{sss:photoz_ini}),
the lower bound of \verb|RESTLAMBDA_FITRANGE| is replaced with
\verb|UVLAM_EXTRAPFLUX| (starting with {\tt SNANA} version v12\_01d); 
the original \verb|RESTLAMBDA_FITRANGE|
is restored before the {\tt MINUIT} minimiztion.

For cosmology analyses, \verb|RESTLAMBDA_FITRANGE| should be set
based on the highest quality model precision; 
for rate analyses, this wavelength range can be relaxed.
...
% ------------------------------------
   \subsubsection{Cosmology Prior and {\tt PRIOR\_MUERRSCALE}}
   \label{sss:zphot_cosmology_prior}
% ------------------------------------

For improved redshift fitting, a cosmology prior can be included;
for example,
\begin{verbatim}
  &SNLCINP
     OMAT_REF =  0.3, 0.03  ! value and uncertainty for prior 
     W0_REF   = -1.0, 0.1   ! idem
     WA_REF   = -0.4, 0.2   ! idem
\end{verbatim}
% 
uses $\sigma_{\OM}=0.03$, $\sigma_{w0}=0.1$ and $\sigma_{wa}=0.2$ to calculate
the $\mu$-error for the photoZ at each fit-iteration. 
The $\chi^2$ prior term is
\begin{equation}
   \chi_{\mu{\rm -prior}}^2 = \left[ \frac{ \Delta\mu  }{ \sigma_{\mu} \times {\rm\tt PRIOR\_MUERRSCALE} } \right]^2
   \label{eq:chi2_muprior}
\end{equation}
%
where 
\newcommand{\muTripp}{\mu_{\rm Tripp}}
\newcommand{\muTheory}{\mu_{\rm theory}}
\newcommand{\rhowowa}{\rho_{w0wa}}
\begin{itemize}
  \item $\Delta\mu = \muTripp - \muTheory$, with $\muTripp = m_x + 0.14x_1 - 3.2c$ and 
    $\muTheory$ is computed using $\Zphot$ and cosmology parameters from
    {\tt \&SNLCINP} namelist inputs {\tt OMAT\_REF}, {\tt W0\_REF}, {\tt WA\_REF};
    \item $\sigma_\mu^2 = [\delta_{\mu}(\OM)]^2 + [\delta_{\mu}(w_0)]^2 + [\delta_{\mu}(w_a)]^2 
          + [ 2 \rhowowa \times \delta_{\mu}(w_0) \times \delta_{\mu}(w_a)] $ ~~~where \\
          $\delta_{\mu}(\OM) = \mu({\OM} +\delta_{\OM}) - \mu(\OM)$
          with $\OM = {\rm\tt OMAT\_REF(1)}$ and $\delta_{\OM} = {\rm\tt OMAT\_REF(2)}$,
          and similarly for $\delta_{\mu}(w_0)$ and $\delta_{\mu}(w_a)$,
          and $\rhowowa = -0.8$ is hard-coded (see {\tt RHO\_W0WA} in {\tt snlc\_fit.F90}).
    \item {\rm\tt PRIOR\_MUERRSCALE} is a {\tt \&FITINP} namelist input to soften the cosmology prior
      by inflating the computed distance uncertainty $\sigma_{\mu}$.
\end{itemize}


% ------------------------------------
  \clearpage
   \subsubsection{Host properties: LOGMASS}
   \label{sss:zphot_logmass}
% ------------------------------------

\newcommand{\SEDFIT}{SEDFIT$_{\rm HOST}$}
\newcommand{\lm}{{\tt LOGMASS}}

The determination of host properties with SED fitting codes, 
in partular the log of stellar mass (\lm), depends on an accurate redshift. 
SED fit codes such as {\tt CIGALE} and {\tt FSPS} are generically denoted
here as \SEDFIT.
For events with $\ZphotHost$, the \lm\ can be pre-computed 
in the same way as for events with $\Zspec$. 
However, the combined SN+HOST \lcfitz\ results in a more accurate $\Zphot$ 
estimate compared to the host-only $\Zphot$, 
and obtaining an improved \lm\ estimate requires re-running
\SEDFIT\ after each \lcfitz.

To more efficiently update \lm\ after each \lcfitz,
\SEDFIT\ can be used to pre-compute \lm\ on a redshift grid for 
each event for real data, or for each \hostlib\ entry for simulated data.
\lcfitz\ performs a post-processing interpolation to obtain \lm, 
its \SEDFIT\ uncertainty, and the interpolation uncertanty
from $\sigma_{\Zphot}$.

\bigskip
The tecnical implementation is to use the \verb|[SIM_]HEADER_OVERRIDE_FILE|
feature in \S\ref{sss:header_override}. Rather than specifying
{\tt CID}, the override file should be keyed by {\tt GALID}.

\begin{verbatim}
VARNAMES: GALID HOSTGALz_LOGMASS_ZGRID HOSTGALz_LOGMASS_VALGRID HOSTGALz_LOGMASS_ERRGRID
GAL: 101988  0.10   9.17  0.29
GAL: 101988  0.25   9.94  0.30
GAL: 101988  0.40  10.28  0.24
GAL: 101988  0.55  10.53  0.24
GAL: 101988  0.70  10.67  0.27
GAL: 101988  0.85  10.82  0.32
GAL: 101988  1.00  10.94  0.39
GAL: 101988  1.15  11.06  0.63
GAL: 101988  1.30  11.22  0.66
GAL: 101988  1.45  11.56  0.63

GAL: 112537  0.10    9.28  0.28
GAL: 112537  0.25   10.09  0.25
GAL: 112537  0.40   10.33  0.20
GAL: 112537  0.55   10.59  0.21
GAL: 112537  0.70   10.77  0.25
GAL: 112537  0.85   10.81  0.38
GAL: 112537  1.00   10.83  0.45
GAL: 112537  1.15   10.95  0.56
GAL: 112537  1.30   11.13  0.60
GAL: 112537  1.45   11.57  0.71

etc ...
\end{verbatim}                                                                                         
%
The redshift grid can be non-uniform, and the number of redshift bins
can vary for each galaxy. If a fitted $\Zphot$ is outside the pre-defined
redshift grid ... ??? If an event has multiple nearby hosts, each
host is updated provided the associated {\tt GALID} is in the override file.
...
\end{verbatim}
The {\tt MAGDIF} variable is stored in the output {\tt FITRES} table.
...
\end{Verbatim}
%
and these {\tt USERTAG} indices appear in output table files.
...
\subsubsection{Over-Riding Header Information: \texttt{HEADER\_OVERRIDE\_FILE} }
\label{sss:header_override}

For \snana\ analysis programs 
({\tt snana.exe}, {\tt snlc\_fit.exe}, {\tt psnid.exe}),
header information in the data files can be over-written for analysis,
without re-making the data files.
This feature is useful to study the impact of data changes
without altering a released data set. 
Beware that this feature is intended only for internal 
...
The header information includes any variable in a {\tt *HEAD.FITS} file:
host properties, peculiar velocities, redshifts (spec-$z$ and photo-$z$), 
Galactic extintion, and many more.
This override feature does {\tt NOT} work
on epoch-dependent quantities in the {\tt *PHOT.FITS} files, 
but mag and flux corrections can be applied as described below in Sec.~\ref{sss:magcor}.
...
more myUpdates.dat
VARNAMES: CID  REDSHIFT_HELIO  REDSHIFT_HELIO_ERR  MWEBV   EXTRA
SN:   1346137      0.246        0.001              0.021    45
SN:   1346387      0.533        0.0011             0.038    46
etc ...
\end{verbatim}

In the first example ({\tt muUpdates.dat}), three header values are updated:
helio redshift, the uncertainty, and Galactic extintion. The {\tt EXTRA} column
does not correspond to a known data variable and is thus ignored, which means
that extra diagnostic variables are allowed to be mixed with data variables.
For SNe missing from the update file, the original values in the 
data file header are preserved. 
The first column name can be either {\tt CID} or {\tt SNID}.
The second example illustrates a comma-separated list of override table files;
...
Quantiles can be overwritten if they already exist in the
original data files, or appended if the original data files
do not include quantile information. Quantile overrides can be specified
in two distinct formats. The first format is using the same {\tt QZPHOTnn}
column names as in the \hostlib\ (\S\ref{sss:header_override_dir}),
even though these variables names do not appear in the data files:
\begin{verbatim}
  VARNAMES:  GALID  QZPHOT00 QZPHOT01 ... QZPHOTnn ANOTHTER_VAR ANOTHER_VAR2 ...
\end{verbatim}
While this {\hostlib}-style format {\it implicitly} defines equal-prob spacing of the
percentiles, {\it explicit} (non-uniform) percentiles can be defined with a second 
(more verbose) format:
\clearpage
\begin{verbatim}
  VARNAMES:  GALID  HOSTGALz_QUANTILE_ZPHOT  HOSTGALz_QUANTILE_PERCENT
  GAL: 10307000163519  0.35   0
  GAL: 10307000163519  0.38   10
  GAL: 10307000163519  0.41   28
  GAL: 10307000163519  0.44   35
  GAL: 10307000163519  0.47   48
  GAL: 10307000163519  0.50   55
  GAL: 10307000163519  0.53   62
  GAL: 10307000163519  0.56   70
  GAL: 10307000163519  0.59   83
  GAL: 10307000163519  0.62   91
  GAL: 10307000163519  0.80   100
\end{verbatim}
%
With the 2nd format option, additional override variables (e.g., {\tt ZPHOT, ZPHOTERR})
cannot be specified in this file, but a 2nd override file can be used for other variables.

A common  mistake is to accidentally replace data variable names with names in the
output FITRES tables: unfortunately the variable naming is not consistent.
Such substituion usually results in an abort since no
valid override variables are found.

\bigskip\noindent
{\bf Missing Events in {\tt HEADER\_OVERRIDE\_FILE}:} \\
By default, missing events ({\tt CID} or {\tt GALID}) in the 
\verb|HEADER_OVERRIDE_FILE| result in reading the value in the data file.
This default behavior may not be suitable in cases such as photo-$z$ (PZ) overrides
(point estimate and/or quantiles) with a different PZ-syst algorithm;
if a PZ-syst method drops a few events, the analysis should not necessarily
use the baseline PZ values for these events. To force using only values
in the \verb|HEADER_OVERRIDE_FILE|, add the following key above the {\tt VARNAMES}
key in the \verb|HEADER_OVERRIDE_FILE|:
\begin{verbatim}
   NULL_ON_MISSING_EVENT:  1  # do NOT read data values for VARNAMES in this file
\end{verbatim}
resulting in NULL values for missing override events. NULL values are typically 0 for
counters (e.g.. number of quantiles), and -9 for values such as redshifts.
Beware that \verb|NULL_ON_MISSING_EVENT| applies to all columns in the 
\verb|HEADER_OVERRIDE_FILE|; to have a mix of columns with/without the
\verb|NULL_ON_MISSING_EVENT| feature, separate the columns into different
\verb|HEADER_OVERRIDE_FILE|s.


\subsubsection{Over-Riding Header Information: {\tt HEADER\_OVERRIDE\_DIR} }
\label{sss:header_override_dir}
%
If using \verb|HEADER_OVERRIDE_FILE| with long comma-separated lists becomes
difficult to manage, an alternative approach is to specify an override
{\it directory} with a list file indicating which override files to use.
For example,
\begin{verbatim}
   &SNLCINP
      HEADER_OVERRIDE_DIR = '/project/mysurvey/OVERRIDES/V02'
    !   beware that /project/mysurvey/overrides/V02/V02.LIST   must exist
\end{verbatim}
reads a list of overrides files from the LIST file, and internally
constructs a comma-separated list of override files. This strategy
is similar to reading data by specifying a data folder with
\verb|VERSION_PHOTOMETRY| rather than specifying a list of files.


% - - - - - - - - 
%\clearpage
\subsubsection{Over-Riding by {\tt HOSTGAL\_OBJID} instead of {\tt CID}}
\label{sss:header_override_galid}

{\tt OVERRIDE} table row identification by {\tt CID} (or {\tt SNID}) is robust for real data
because real data event names should not change (or change rarely with reprocessing).
To modify host properties for simulated data, however, would require an intermediate
script to create a new {\tt HEADER\_OVERRIDE\_FILE} table for each simulated data
set and for each bias-correction sim. To simplify altering host properties for sims,
particularly photo-z information, the {\tt HEADER\_OVERRIDE\_FILE} can be keyed
by {\tt HOSTGAL\_OBJID} or {\tt GALID} for galaxies in an entire {\tt HOSTLIB\_FILE}, e.g. 

\begin{verbatim}                                                                                                   
VARNAMES: GALID  HOSTGAL_PHOTOZ HOSTGAL_PHOTOZ_ERR
SN:  104754   .111   0.011
SN:  104049   .222   0.022
SN:  463249   .333   0.033
etc ...
\end{verbatim}
This single table override file is valid for simulated data regardless of which
random {\tt CID}s are chosen. This galaxy-based identification can be used for real
data as well, but beware that host-less events cannot be modified using the {\tt GALID} key.


% - - - - - - - - 
\subsubsection{Over-Riding Table Information in BBC/SALT2mu Input }
\label{sss:bbc_override}
...
Copy any subroutine from {\tt \$SNANA\_DIR/src/snana.F90} or 
{\tt snlc\_fit.F90},
...
(see {\tt psnid.F90}) to control the detailed behavior
...
\begin{Verbatim}[frame=single]
   ...
   PRIOR_DELTA_PROFILE = xxx, xxx, xxx, xxx  ! grep snlc_fit.F90 for details
\end{Verbatim}
...
The \BAYESN\ SED model is based on the Hsiao template \cite{Hsiao2007}, modified by a
$t$- and $\lambda$-dependent warping surface $\propto 10^{-0.4 W(t,\lambda)}$, and extinguished by dust.
Extrapolation of a \BAYESN\ model outside its defined phase and wavelength range is achieved
by extrapolating $W(t,\lambda)$.
...
\begin{itemize}
  ...
  \item \verb!GENMODEL_MSKOPT+=32!: Enable extrapolation beyond the \BAYESN\ phase range
   by setting $W(t,\lambda) \to 0$. This reverts to the Hsiao template.
  \item \verb!GENMODEL_MSKOPT+=64!: Enable extrapolation beyond the \BAYESN\ phase range
   by setting $W(t,\lambda) \to W(t_\mathrm{edge},\lambda)$. This gives Hsiao-like behavior,
   but re-normalized to minimize any jumps in magnitude.
  \item \verb!GENMODEL_MSKOPT+=128!: Enable a ``verbose'' mode for debugging purposes.
  \item \verb!GENMODEL_MSKOPT+=256!: Enable a ``super verbose'' mode for debugging splines.
  \item \verb!GENMODEL_MSKOPT+=512!: Enable extrapolation beyond the \BAYESN\ wavelength range
   by setting $W(t,\lambda) \to 0$. This reverts to the Hsiao template.
  \item \verb!GENMODEL_MSKOPT+=1024!: Enable extrapolation beyond the \BAYESN\ wavelength range
   by setting $W(t,\lambda) \to W(t,\lambda_\mathrm{edge})$. This reverts to the Hsiao template, re-scaled
   to match the normalization of the \BAYESN\ model at the edge of its defined wavelength range.
  \item \verb!GENMODEL_MSKOPT+=2048!: Generic development mask for testing new code.
\end{itemize}
...
Please note that extrapolation in wavelength, or by $>10$~d in phase, is not recommended in most
circumstances. Without setting any of the extrapolation-related \verb!GENMODEL_MSKOPT! bits, the
model will \emph{not} be extrapolated in wavelength, and will be extended in phase by linearly
extrapolating $W(t,\lambda)$. This phase-dependent extrapolation can lead to surprising results
(e.g.\ re-brightening at late times) if pushed too far! If extensive extrapolation of the model is
needed, setting \verb!GENMODEL_MSKOPT=576! is probably the best option, but this is experimental
and should be used with caution!
It is currently not possible to extrapolate beyond the phase ($t\in[-20,85]$~d) 
and wavelength ($\lambda\in[1000,25000]$~\AA) ranges of the Hsiao \cite{Hsiao2007} template.
...
%
\begin{verbatim}
   GENMODEL:  fixmag  20      # mag=20 for every obs & event
   GENMODEL:  fixmag  20:24   # random mag (20-24) per event (same mag per obs)
   GENMODEL:  ranmag  20:24   # random mag (20-24) oer observation

   GENMODEL:  FIXMAG  -19     # obs mag = -19 + MU
   GENMODEL:  FIXMAG  -19:-17 # obs mag = ran(-19:-17) + MU per event
   GENMODEL:  RANMAG  -19:-17 # random mag per observations
\end{verbatim}
%
In the first case, all observer-frame magnitudes are 20.
For the 2nd case, the observer-frame mag 
...
For the 3rd case ({\tt ranmag}), a random mag is selected per for each observation.
The range of fixed-mag (random-mag) epochs is determined by the sim-input key 
\verb|GENRANGE_TREST|.
For these observer-frame models, 
\verb|GENRANGE_REDSHIFT| is ignored unless a \hostlib\ is specfied.

The upper case {\tt FIXMAG} specifies rest-frame mag for which
the distance modulus is added. 
For this option, \verb|GENRANGE_REDSHIFT| must be specified.
...
The CMB prior is based on the $R$ parametrization using simulated cosmology parameters
...
\begin{verbatim}
   ...
   for full list of options, type program name with no arguments:
   wfit.exe
\end{verbatim}
where {\tt HubbleDiagram} and {\tt mucov\_file} are created from 
the {\crcov} program (\S~\ref{subsec:create_cov}).
...
% -----------------------------------------------
   \clearpage
   \subsection{Statistical+Systematic Covariance Matrix}
   \label{subsec:create_cov}
% -----------------------------------------------

The program {\crcov} reads {\tt SALT2mu/BBC} output directories that include
systematics, and computes a stat+syst covariance matrix, a Hubble diagram file,
and an {\tt INFO.YAML} file with lots of meta data such as which biasCor sim was used,
flag to indicate real/sim data, etc. The input config file is defined by command
\begin{verbatim}
   create_covariance.py -H   # config file options
   create_covariance.py -h   # command line options
\end{verbatim}
%
Since the cosmology fitting codes need the inverted matrix, the default
behavior is to write only the inverted matrix. To reduce disk space,
the output format is npz, and only the upper-triangular part. 
For special tests or debugging, the total and syst-only cov matrices
can also be written with command-line option ``{\tt --write\_mask\_cov 7};''
the default npz format can be replaced with text using
``{\tt write\_format\_cov text}.''
...
The fitres text-file output from the light curve fit program {\tt snlc\_fit.exe}
contains a small fraction of the analysis variables.
The entire set of analysis variables is stored in a more
compact ``{\tt SNTABLE}'' structure using ROOT
...
\begin{verbatim}  
    ...
    ZP_FLUXCAL_REFORMAT = 31.4  ! update FLUXCAL[ERR] to this ZP instead of original 27.5
\end{verbatim}
%
...
\begin{verbatim}  
    ...
    OPT_REFORMAT_SPECTRA = 8 ! specialized output for SNID-SAGE code
\end{verbatim}
...
The following variables are automatically included in the output
{\tt FITRES} table file:
...
overlapping a patch of sky or variation with CCD, it is sometimes convenient
...
To fit with the usual ``\verb|FILTLIST_FIT = 'ugriz'|'' input,
...
Another example for combined low-z samples is
\begin{verbatim} 
   &SNLCINP
      FILTER_REPLACE = 'abcdefghijkl -> BBBVVVRRRIII'
           ! or
      FILTER_REPLACE = 'abc def ghi jkl -> BBB VVV RRR III'  ! more human readable
          ! or 
      FILTER_REPLACE = 'abc def ghi jkl->BBB VVV RRR III'  ! arrow without pad spaces
\end{verbatim}
% 
where 'a,b,c' bands in the data are remapped to a common B band,
'd,e,f' bands are remapped to V, etc.
Only B,V,R,I need to be defined in the kcor/calib file. Pad spaces are 
optional for human readability. The arrow string (\verb|->|)
cannot have a pad space (i.e., \verb|- >| is not recognized).
...
the code, e.g.,
\begin{verbatim}                                                               
   grep cutvar_name $SNANA_DIR/src/snana.F90 | grep CUTBIT
\end{verbatim}
...
To compare {\tt SNANA} analyses with non-SNANA codes, 
the  \verb|ERALYLC_STRING| option can be used with 
\verb|VERSION_REFORMAT_FITS| to re-write data samples
that contain only the early observations.
