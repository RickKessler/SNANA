...
Starting with \snana\ {v9\_30G} there is a host-galaxy
package designed to simulate the following effects:
\begin{itemize}
%
...
\noindent {\bf Notes on CPU Resources:} \\
...
%\clearpage
\noindent {\bf Anomalous Flux-Scatter on Bright Galaxies :} \\

The simulation supports a model of anomalous flux-scatter
related to subtractions near bright galaxies. 
The true flux uncertainty is increased as a function of 
surface brightness, while the nominal uncertainty is 
...
% ------------------------------------
   \clearpage
   \subsection{Initial Fit-Parameter Estimation}
   \label{subsec:initial_fitpar}
% ------------------------------------

For {\sc minuit} to converge,
initial fit-parameters must be chosen so that the
initial model light curve roughly overlaps the data.
The color and shape parameter are fixed
to an average value, and the distance modulus
(or $x_0$ for SALT-II) is adjusted so that the model
matches the data.

The estimate of the epoch of peak brightness ($t_0$)
is usually read from the data file from the keyword
{\tt SEARCH\_PEAKMJD}. If this keyword is missing,
then \snana\ will try to estimate $t_0$ by fitting
a general function of the form 
(see \snana\ function {\tt SET\_PEAKMJD})
%
\begin{equation}
...
\end{equation}
%
as suggested in the SNLS CC rate paper (Bazin et al, 2008).
The fitted parameters for each filter are 
$A, a_1, a_2, \bar{t}, \Tfall, \Trise$.
The time at peak is obtained by setting the derivative equal
to zero: $t_0 = \bar{t} + \Trise \ln(\Tfall/\Trise-1)$.
The initial $\bar{t}$ value is estimated to be the epoch
with maximum flux. Each filter is fit independently and
the final $t_0$ is the filter-weighted average.
A filter's $t_0$ is dropped from the average if:
(1) its max-flux measurement has the smallest $S/N$ 
ratio among filters and has $S/N<10$, or
(2) its $t_0$ value is more than 30 days away from the average
of the other filter-$t_0$ values 
(tested only if there are 3 or more filters).
The max-flux epoch must have $S/N > 4$ to make an estimate
of $t_0$. 
...
Users should compare the initial $t_0$ (from above)
to the final $t_0$ from the light curve fit
and check for outliers;
outliers can be fixed by visual inspection of the light curve
and setting the {\tt SEARCH\_PEAKMJD} keyword in the data file.
To avoid clogging up the standard log-file,
the {\sc minuit} output for these fits is dumped to a
separate log-file specified by namelist variable
{\tt MNFIT\_PKMJD\_LOGFILE}: the default filename is
{\tt MNFIT\_PKMJD.LOG}.

\bigskip
WARNING (Jun 2010):
The SN classifier challenge has uncovered two serious problems
with the initial $t_0$ estimate for SNe~Ia using Eq.~\ref{eq:t0fun}.
First, the filter-dependence of $t_0$ is not accounted for.
Second, Eq.~\ref{eq:t0fun} can sometimes be very nonIa-like
(even for a perfectly good fit), leading to $t_0$ estimates off
by more than a week.
Will need to add an option to use a more Ia-like function.
...
\noindent
Reference: J. Guy et al., {\bf A\&A 466}, 11 (2007). 
...
The 2nd and 3rd cases are identical. The observer-frame mag for 
each event is randomly selected between 20 and 24; the same mag
is used at all epochs, but a different random mag
...
performs a global fit for $\alpha$ and $\beta$, and computes
distance moduli.
The program also implements the 
``BEAMS with Bias Corrections'' (BBC) method in \cite{JM11,BBC},
to produce bias-corrected distances in redshift bins.
To include bias corrections, {\tt SALT2mu} reads fit parameters
for the data and for a large simulation.
%
...
%$
% -----------------------------------------
   \subsection{\SALTII\ Training Scripts}
   \label{sec:SALT2train}
% -----------------------------------------
Scripts are {\tt \&SNANA\_DIR/util/SALT2train\_*};
...
\begin{itemize}
\item
{\tt SNANA+EPOCHS} and {\tt SPECTRA} options work with 
