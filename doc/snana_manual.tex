...
the default opton performs a 3D grid-search over 
...
\begin{verbatim}
   &FITINP
      NBINc_COURSE_PHOTOZ = nnn  ! default is 4  what about BAYESN ???
      NBINs_COURSE_PHOTOZ = nnn  ! default is 3  
      NBINt_COURSE_PHOTOZ = nnn  ! default is 1 (fixed at PEAKMJD from OPT_SETPKMJD)
      NBINz_COURSE_PHOTOZ = nnn  ! default is 43*(zph_max/(1+zph_max) - zph_min/(1_zph_min)
      SCALE_NBIN_COURSE_PHOTOZ  = nnn  ! scale number of bins for each param (default=1)

      RANGEc_COURSE_PHOTOZ = x, y  ! default is -0.2, 0.25
      RANGEs_COURSE_PHOTOZ = x, y  ! default is -3.0, 1.0
      RANGEt_COURSE_PHOTOZ = x, y  ! default is -15, 15 (used only if NBINt_COURSE_PHOTOZ>1)
      RANGEz_COURSE_PHOTOZ = x, y  ! default is -9, -9 (auto-computed based on prior)
\end{verbatim}
...
If there is no host prior, then it is ok to modify 
{\tt NBINz\_COURSE\_PHOTOZ} and {\tt RANGEz\_COURSE\_PHOTOZ}
and re-process a large sample. However, a host prior internally alters
...
\begin{verbatim}                                                                                                          7      peakmjd  bins from 56972.31 to 57002.31   (binsize=   5.00)
