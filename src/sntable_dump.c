...
void write_IGNORE_FILE(void) {
  ...
  for(irow=0; irow < NROW; irow++ ) {
    ...
    NROW_MATCH = SNTABLE_AUTOSTORE_READ(CCID, "MJD",      &ISTAT, &DVAL, CVAL ); 
    MJD = DVAL ;

    NROW_MATCH = SNTABLE_AUTOSTORE_READ(CCID, "IFILTOBS", &ISTAT, &DVAL, CVAL ); 
    IFILTOBS = (int)DVAL ;

    NROW_MATCH = SNTABLE_AUTOSTORE_READ(CCID, "REJECT", &ISTAT, &DVAL, CVAL ); 
    REJECT = (int)DVAL ;

    NROW_MATCH = SNTABLE_AUTOSTORE_READ(CCID, "PSF", &ISTAT, &DVAL, CVAL ); 
    PSF = DVAL ;

    NROW_MATCH = SNTABLE_AUTOSTORE_READ(CCID, "TOBS", &ISTAT, &DVAL, CVAL ); 
    TOBS = DVAL ;

    NROW_MATCH = SNTABLE_AUTOSTORE_READ(CCID, VARNAME_CHI2, &ISTAT, &DVAL, CVAL ); 
    CHI2 = DVAL ;
