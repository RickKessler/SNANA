...
def build_doc_header(snana_cols, cigale_results_path):
    ...
    return (
        "DOCUMENTATION:\n"
        "    PURPOSE: Grid of pre-computed host logmasses to use in photo-z analyses\n"
        f"    INPUT_HOST_INFORMATION:\n"
        f"    CIGALE_RESULTS_FILE: {os.getcwd()}/{cigale_results_path}\n"
        "    REF:\n"
        "    - AUTHOR: Boquien et al. 2019\n"
        "    - ADS:    https://adsabs.harvard.edu/abs/2019A%26A...622A.103B\n"
        "    VALIDATE_SCIENCE: used in PASTRY SNIa-cosmology analysis\n"
        "    NOTES:\n"
        "    - Host logmasses generated by cigale\n"
        "    VERSIONS:\n"
...
def write_snana_output(output_path, snana_cols, galids, redshifts, logmass, logmass_err, cigale_results_path, num_pad_rows=0):
    ...
    with opener(output_path, mode) as f:
        f.write(build_doc_header(snana_cols, cigale_results_path))
