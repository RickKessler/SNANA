...
respectively. 
If there are more than two Sersic profiles per galaxy, 
...
TAKE_SPECTRUM_HOSTSNFRAC: 0.1  # scale FLAM(HOST) so Sum(HOST)/Sum(SN) = 0.1
...
\newcommand{\dFlamSN}{dF_{\rm SN}/d\lam}
...
The 2nd method ({\tt TAKE\_SPECTRUM\_HOSTSNFRAC}) does not depend
on correct host normalization because it scales $\dFlamHost$ such that
\begin{equation}
   {\rm\tt HOSTSNFRAC} = 
     \frac{  \int d\lam [\dFlamHost] } { \int d\lam [\dFlamSN ] }~.
\end{equation}
% .xyz
...
\begin{Verbatim}[frame=single]
   ...
   RANSYSTPAR_GENMODEL_WILDCARD: "/path/to/genmodels*" # picks random model out of globbed list
...
\end{verbatim}
%
In the first example, four header values are updated:
...
the variables in each file must be unique. (except for CID)
...
(class=\SALTII,\SIMSED,\NONIA,\BYOSED\footnote{https://byosed.readthedocs.io/en/latest}) 
...
\begin{verbatim} 
  # fit-input file
   &FITINP
      ENABLE_SALT2_MAGSHIFT  = F  ! disable MAGSHIFT 
      ENABLE_SALT2_WAVESHIFT = F  ! disable WAVESHIFT
...
SIMSED_GRIDONLY: SEQUENTIAL  # sequential generation of grid values
     or
SIMSED_GRIDONLY: KN_INDEX    # pick random KN_INDEX
    or
SIMSED_GRIDONLY: KN_INDEX(3,5,6,9) # pick random among these KN_INDEX values
...
\begin{itemize}
  ...
  \item SNLCPAK: used by {\tt mkfitplots.pl} (\S\ref{sss:lcplot})
...
classifier\footnote{https://arxiv.org/abs/1901.06384}
