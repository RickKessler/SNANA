...
in the Milky Way (\S\ref{sss:GALextinc_curves}).
...
\cite{Fitzpatrick1999, Fitzpatrick2004}, and a new UV--optical--NIR--MIR
curve from Gordon et al.~\cite{Gordon2023}. All of these are $R_V$-dependent.
The `\texttt{GALextinct}' function will abort if called 
outside the valid wavelength range of these models.
Also included is the two-parameter $(R_V^A,f_A)$-dependent curve
from Gordon et al.~\cite{Gordon2016}. This gives a mixture of MW-like \cite{Fitzpatrick1999}
and SMC-bar-like \cite{Gordon2003} dust. At the time of writing, $f_A$ 
is not available as a free parameter, and is hardcoded as $f_A=0$. This
reverts to the SMC bar dust law with $R_V=2.74$ \cite{Gordon2003}. This 
can also be accessed via its own key, although it will abort for $R_V\neq2.74$.
Other than this example, $R_V$ restrictions are not enforced, 
but they are defined via the variables \verb!RVMIN_*! and \verb!RVMAX_*! in 
...
\subsubsection{Simulating Intergalactic Extinction}
\label{sss:IGM}
...
\begin{verbatim}
   ...
   OPT_PHOTOZ = 16  ! CHEAT: set initial zPhot=zSpec and skip grid search (for debug)
...
\begin{verbatim}  
    ...
    # options to re-write only the spectra (no photometry); e.g., to prepare
    # input to spectroscopic analyzer for redshifts or classification
