...
void INIT_FLUXERRMODEL(int OPTMASK, char *fileName, 
  ...
  // Aug 26 2020: set OPENMASK options for snana_openTextFile
  ...
  // set OPENMASK options for snana_open
  // Note that the OPTMASK bit (passed above) is different than 
  // the OPENMASK bit passed to snana_openTextFile.
  OPENMASK = OPENMASK_VERBOSE;
  if ( (OPTMASK & MASK_REQUIRE_DOCANA_FLUXERRMAP)>0 ) 
...
void get_FLUXERRMODEL(int OPT, double FLUXERR_IN, char *BAND, char *FIELD, 
  ...
  char callFun[200];  sprintf(callFun,"%s(%s-%s)", fnam, FIELD, BAND); 
  ...
  *FLUXERR_DATA = FLUXERR_IN ;
  ...
  istat = interp_GRIDMAP( &FLUXERRMAP[imap].MAP, parList, callFun,  &errModelVal);
