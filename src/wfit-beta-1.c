...
int main(int argc,char *argv[]){
  ...
  /********************************************************/
    ...
    for( i=0; i < w_steps; i++){
      ...
      cpar.wa = 0. ; // Look here AM, 3rd loop
   // for( k = 0; k < z_steps; k++){    // indent
   // cpar.wa = (wa_min + i*w_stepsize)*(1-(a_min + k*z_stepsize))

      for(j=0; j < omm_steps; j++){
        cpar.omm = omm_min + j*omm_stepsize;
        cpar.ome = 1 - cpar.omm;

        get_chi2wOM ( cpar.w0, cpar.omm, sqsnrms,  // inputs  AM --> insert cpar.wa ? 
