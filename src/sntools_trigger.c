...
int gen_SEARCHEFF_zHOST(int ID, double *EFF_zHOST) {
  ...
  *EFF_zHOST = 0.0 ;
  ...
  if ( REFAC_SEARCHEFF ) {
    ...
    // legacy
    LFIND = gen_SEARCHEFF_zHOST_LEGACY(ID,EFF_zHOST); 
  }
...
} // end read_zHOST_FILE_LEGACY


// ***************************************
void read_zHOST_FILE_LEGACY2(FILE *fp) {
  ...
  double VAL[2];
  char fnam[] = "read_zHOST_FILE_LEGACY2" ;
...
} // end of read_zHOST_FILE_LEGACY2
...
int parse_VARNAMES_zHOST_LEGACY(FILE *fp, int *ivar_HOSTLIB, 
  ...
  for ( ivar=0; ivar < NVAR; ivar++ ) {
    ...
    *ivar_H  = IVAR_HOSTLIB_STORE(varName_H,0, fnam);  // sntools_host.c
