...
is not included. The DCR effects are enabled with sim-input
 \begin{verbatim}
   ...
   [astrom noise param(s) ??]
\end{verbatim}
%
While the default simulation sets measured RA,DEC to their exact values
without measurement noise, {\tt ATMOSPHERE\_OPTMASK}$>0$ changes the sim
to determine a measured RA,DEC for each observation using a noise model
based on ??, and including calculated DCR shifts. A weighted average 
of RA,DEC measurements from all {\it detections} is reported in the
data files as the measured location for the event. 
Since the DCR effect shifts measured coordinates as a function of band pass, 
a magnitude shift (calibration error) for each observations is 
calculated assuming a PSF-fitted flux centered on the
measured RA,DEC-average.

The simulation of atmosphere effects automatically appends the following
columns in the photometry table per observations:
\begin{verbatim}
 dRA             # RA - RA_AVG (arcsec)
 dDEC            # DEC - DEC_AVG (arcsec)
 ...
 SIM_DCR_dMAGOBS # calculated mag shift from using RA_AVG and DEC_AVG for PSF fit
\end{verbatim}
%
To compute {\tt AIRMASS}, the geo location of the telescope must be
included in the {\tt \$SNDATA\_ROOT/SURVEY.DEF} file; e.g., 
\begin{verbatim}
   SURVEY:  LSST   12   geo:-30.244639,-70.749417 
\end{verbatim}
The sim aborts if {\tt ATMOSPHERE\_OPTMASK}$>0$ 
and the geo-location is not provided.
...
%
  \item For an arbitrary set of table variables, 
       ...
       \begin{verbatim}
       sntable_dump.pl  myjob.root  SNANA  -v RA DEC SNRMAX_r
       sntable_dump.pl  myjob.root  FITRES -v m0obs_g m0obs_r
       \end{verbatim}
%
  \item Append an existing text file with variables from a {\tt SNTABLE}:
       \begin{verbatim}
 sntable_dump.pl  myjob.root  SNANA  -v RA DEC SNRMAX_r -a MYJOB.FITRES
 sntable_dump.pl  myjob.root  FITRES -v m0obs_g m0obs_r  -a MYJOB.FITRES
       \end{verbatim}
%
