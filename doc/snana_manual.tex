...
The number of bins per variable can be scaled with 
\begin{verbatim}
   &FITINP
      ISCALE_NBINc_COURSE_PHOTOZ = [ISCALE]  ! scale default number of color bins
      ISCALE_NBINs_COURSE_PHOTOZ = [ISCALE]  ! scale default number of stretch bins
      ISCALE_NBINt_COURSE_PHOTOZ = [ISCALE]  ! scale default number of PEAKMJD bins
      ISCALE_NBINz_COURSE_PHOTOZ = [ISCALE]  ! scale default number of zphot bins
      ISCALE_NBIN_COURSE_PHOTOZ  = [ISCALE]  ! scale number of bins for each parameter
\end{verbatim}
and each default ISCALE=1.  The default number of {\tt PEAKMJD} bins is 1 since it is fixed,
and setting {\tt ISCALE\_NBINt\_COURSE\_PHOTOZ>1} results in a grid between $-15$ and $+15$ days.
Beware that setting {\tt ISCALE\_XXX > 1} can result in significantly longer fit times, 
so run a CPU benchmark before running long jobs.
