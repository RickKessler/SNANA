...
% ------------------------------------
   \clearpage
   \subsection{Initial Estimate of Peak-MJD ($t_0$) }
   \label{subsec:initial_peakmjd}
% ------------------------------------

Many light curve fitting programs rely on an approximate estimate 
of peak-MJD ($t_0$). Several \SNANA\ options are described here
using the {\tt \&SNLCINP} namelist variable {\tt OPT\_SETPKMJD}.
Options are based on analyzing fluxes, and/or a light curve fit
in each pass-band using a generic Bazin function.
To extract $t_0$ into a TEXT-table (column {\tt PKMJDINI})
for a non-\SNANA\ code, 
\begin{verbatim}
    snana.exe NOFILE \
...
A summary of the {\tt \&SNLCINP} bit-mask options is as fillows
%
\begin{Verbatim}[frame=single]
   ...
   MJDWIN_SETPKMJD  = 50   # for Fmax-clump method only (default=50)
...
{\tt OPT\_SETPKMJD=32} sets $t_0$ to the MJD when the survey
...
The max-flux epoch must have $S/N > 3$ to make an estimate of $t_0$. 
The CPU time os ${\sim}170$~millsec per event.

The Bazin fit needs and initial $\bar{t}$ value, which by default
is based on MJD of maximum flux. However, the Fmax-clump method
can be used instead by combining bit-mask options,
\begin{verbatim}
   OPT_SETPKMJD = 17  # 1(Bazin) + 16(Fmax-clump)
\end{verbatim}
%
If $t_0$ canot be detemermined, \SNANA\ programs abort by default.
To avoid aborts, {\tt OPT\_SETPKMJD += 4}.
The Bazin fit parameters for each band can be included in output
tables with {\tt OPT\_SETPKMJD += 512}.
Users should compare the $t_0$ estimate ({\tt PKMJDINI} in output tables) 
